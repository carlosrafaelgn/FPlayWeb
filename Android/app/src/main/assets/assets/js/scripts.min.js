...
a=this._playlist.currentItem;!a&&this._playlist.length&&(this._playlist.moveCurrentToNext(),a=this._playlist.currentItem);if(!a){this.stop();return}this._loadedSong=a;this._resumeTimeS=this._lastTimeS;this._lastTimeS=-1;this._songStartedAutomatically=!!b;this._currentPlaylistSong!==a&&(this._currentPlaylistSong=a,queueMicrotask(this._boundNotifySongChange));this._nextSongToPlayAfterLoading=null;if(this._nextAudioBundle){var d=this._nextAudioBundle;if(d.nextSong===a&&!this._nextSongLoadedWithError){c=
this._audio;this._resumeTimeS=-1;this._songToResumeTime=null;this._lastObjectURL&&URL.revokeObjectURL(this._lastObjectURL);this._lastObjectURL=this._nextSongObjectURL;this._nextAudioBundle=this._nextSongObjectURL=null;this._nextSongLoading?(this.pause(),this.playbackLoadStart(),this._nextSongToPlayAfterLoading=a):this._audio.pause();this._sourceNode&&this._sourceNode.disconnectFromDestination();this._sourceNode=d.sourceNode;this._sourceNode.connectToDestination(this._intermediateNodes.length?this._intermediateNodes[0]:
this._destinationNode);this._audio=d.audio;this.volume=this._volume;this._nextSongLoading||this.nextSongPerformFinalPlaybackSteps();for(zeroObject(d,!0);c.firstChild;)c.removeChild(c.firstChild);c.load();c.parentNode&&c.parentNode.removeChild(c);return}}this.destroyNextAudioBundle();for(this.resumeAudioContext();this._audio.firstChild;)this._audio.removeChild(this._audio.firstChild);d=document.createElement("source");if(a.url&&!a.isLocalURL)d.src=a.url;else if(a.file)this._lastObjectURL&&URL.revokeObjectURL(this._lastObjectURL),
this._lastObjectURL=URL.createObjectURL(a.file),d.src=this._lastObjectURL;else{if(a.url){this.pause();const e=this._playlist.currentIndex,f=this._playlist.currentItem;FileSystemAPI.getFile(a.absolutePath).then(g=>{f&&!f.file&&g&&(f.file=g);this._playlist&&e===this._playlist.currentIndex&&f===this._playlist.currentItem&&(g?(this._lastTimeS=this._resumeTimeS,this.play(e,b)):this.handleError(Strings.FileNotFoundOrNoPermissionError))},()=>{this._playlist&&e===this._playlist.currentIndex&&f===this._playlist.currentItem&&
this.handleError(Strings.FileNotFoundOrNoPermissionError)})}else this.handleError(Strings.MissingSongError);return}d.onerror=this._boundPlaybackError;this._audio.appendChild(d);this._audio.load();if(this._songToResumeTime!==a||!this._resumeTimeS||0>=this._resumeTimeS)c=this._audio.play()}c&&c.catch(Player._nop)}}playSongId(a,b){if(!this._alive||!this._audio||!this._playlist)return!1;a=this._playlist.findIndexById(a);return 0<=a?(this.play(a,b),!0):!1}playPause(){this._alive&&this._audio&&(!this._loadedSong||
this._audio.paused?this.play():this.pause())}stop(){if(this._alive&&this._audio){this._songToResumeTime=this._loadedSong=null;this._lastTimeS=-1;this.destroyNextAudioBundle();if(this._currentPlaylistSong){for(;this._audio.firstChild;)this._audio.removeChild(this._audio.firstChild);this._audio.load();this._lastObjectURL&&(URL.revokeObjectURL(this._lastObjectURL),this._lastObjectURL=null);if(this._loading){this._loading=!1;if(this.onloadingchange)this.onloadingchange(!1);this._mediaSession&&this._mediaSession.setLoading(!1,
0,0)}if(!this._paused){this._paused=!0;if(this.onpausedchange)this.onpausedchange(!0);this._mediaSession&&this._mediaSession.setPaused(!0,0,0)}this._currentPlaylistSong=null;this.notifySongChange()}this._songToResumeTime=this._loadedSong=null;this._lastTimeS=-1;this.suspendAudioContext(!1)}}seekForward(){this._alive&&this.seekTo(1E4,!0)}next(a){this._alive&&this._playlist&&(this._playlist.moveCurrentToNext(),this.play(-1,a))}seekTo(a,b){if(this._alive&&this._currentPlaylistSong&&this._currentPlaylistSong.isSeekable&&
!(0>=this._currentPlaylistSong.lengthMS))if(!this._loadedSong)this._songToResumeTime||(this._songToResumeTime=this._currentPlaylistSong),this._songToResumeTime===this._currentPlaylistSong&&(this._lastTimeS=Math.min(Math.max(0,this._currentPlaylistSong.lengthMS),Math.max(0,a+(b?1E3*Math.max(0,this._lastTimeS)|0:0)))/1E3,this.notifyCurrentTimeSChange());else if(this._audio&&this._audio.seekable&&this._audio.seekable.length){try{const c=Math.min(this._audio.duration||0,Math.max(0,a/1E3+(b?this._audio.currentTime||
0:0)));this._lastTimeS=c;this._audio.currentTime=c}catch(c){}this.notifyCurrentTimeSChange()}}headsetRemoved(){this._alive&&this._audio&&(this._audio.paused?this._loadedSong&&(this._mutedByHeadsetRemoval=this._audio.muted=!0):this.pause())}}Player.minVolume=-40;Player._nop=function(){};
