...
LibMikMod.init(this._audioContext,"assets/lib/libmikmod/").then(async()=>{let a;try{if(a=await this._song.readAsArrayBuffer(),!a){queueMicrotask(()=>{if(this._alive&&this.onerror)this.onerror(Strings.LibMikModErrorReadingModuleFile)});return}}catch(b){queueMicrotask(()=>{if(this._alive&&this.onerror)this.onerror(Strings.LibMikModErrorReadingModuleFile+": "+b)});return}this._alive&&LibMikMod.loadModule({audioContext:this._audioContext,source:a,onload:b=>{if(this._alive){this._loading=!1;this._loaded=
!0;this._sourceAudioNode=b;this._sourceAudioNode.connect(this._volumeAudioNode);this._audioContextStartTime=this._audioContext.currentTime;if(this.oncanplay)this.oncanplay();this._playAfterLoading&&this.play()}},onerror:b=>{if(this._alive&&this.onerror)this.onerror(Strings.LibMikModErrorLoadingModule+": "+b)},onended:()=>{if(this._alive){this._loaded=!1;this._paused=!0;this._currentTimeInterval&&(clearInterval(this._currentTimeInterval),this._currentTimeInterval=0);this._duration=this._currentTime;
if(this._song.lengthMS<this.duration&&this.ondurationchange)this.ondurationchange();if(this.onended)this.onended()}}})},a=>{queueMicrotask(()=>{if(this._alive&&this.onerror)this.onerror(Strings.LibMikModErrorInitializing+": "+a)})})):queueMicrotask(()=>{if(this._alive&&this.onerror)this.onerror(Strings.LibMikModNotSupported)}))}play(){if(!this._alive||!this._paused||!this._loaded)return this._playAfterLoading=!0,Promise.resolve();this._resumeAudioContext();this._paused=!1;this.updateInterval();this.onplaying&&
queueMicrotask(this.onplaying);return Promise.resolve()}pause(){this._alive&&!this._paused&&(this._suspendAudioContext(),this._paused=!0,this.updateInterval(),this.onpause&&queueMicrotask(this.onpause))}destroy(){this._alive&&(this._playAfterLoading=this._loaded=this._alive=!1,this._paused=!0,this._suspendAudioContext(),this._sourceAudioNode&&this._sourceAudioNode.disconnect(),this._volumeAudioNode.disconnect(),this.updateInterval(),zeroObject(this),this._paused=!0)}}
