...
(AppUI._topMessageFading=0,AppUI._topMessageTimeout=0,AppUI._topMessage.style.visibility="",AppUI._topMessage.innerHTML="",AppUI._titleLabel&&AppUI._titleLabel.classList.remove("behind"),AppUI._artistLabel&&AppUI._artistLabel.classList.remove("behind"))}))}static showTopMessage(a,b){AppUI._topMessage&&(AppUI._topMessageTimeout&&clearTimeout(AppUI._topMessageTimeout),AppUI._topMessageFading=1,AppUI._titleLabel&&AppUI._titleLabel.classList.add("behind"),AppUI._artistLabel&&AppUI._artistLabel.classList.add("behind"),
AppUI._topMessage.innerHTML=a,b&&b(AppUI._topMessage),AppUI._topMessage.style.visibility="visible",AppUI._topMessageTimeout=DelayControl.delayShortCB(function(){0>=AppUI._topMessageFading||1!==AppUI._topMessageFading||(AppUI._topMessageFading=2,AppUI._topMessage.classList.add("in"),AppUI._topMessageTimeout=DelayControl.delayFadeCB(function(){2===AppUI._topMessageFading&&(AppUI._topMessageFading=0,AppUI._topMessageTimeout=0)}))}))}static async addFiles(a){if(App.player&&!AppUI._loading&&!Modal.visible&&
!FilePicker.visible){var b=App.hostType===App.hostTypeElectron;if(FilePicker.isSupported()){a=null;var c=FilePicker.showAddPlay(AppUI._playlistControl)}else a=await App.showOpenDialogWeb(a),c=null;if((a||c)&&App.player)try{App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const d=App.player.playlist,e=b?null:new Uint8Array(BufferedReader.minBufferLength),f=b?null:[new Uint8Array(2048)];let g=!1;do{let h=!1;if(c){const k=await c();if(!k)break;h=k.play;a=k.files}if(!a)break;
AppUI._loading=!0;App.updateLoadingIcon();for(let k=0;k<a.length&&App.player;k++)if(b)await d.addSong(a[k])&&h&&(h=!1,App.player&&App.player.play(d.length-1));else{const p=d.length;await d.addSongWeb(a[k],e,f)&&(p==d.length?g=!0:h&&(h=!1,App.player&&App.player.play(p)))}AppUI._loading=!1;App.updateLoadingIcon()}while(c);g&&AppUI._playlistControl&&AppUI._playlistControl.refreshVisibleItems()}catch(d){Modal.show({text:"addFiles error: "+(d.message||d.toString()),returnFocusElement:AppUI._playlistControl})}finally{AppUI._loading=
