...
if(!HostMediaSession.browserWrapper){const a="mediaSession"in navigator&&"MediaMetadata"in window?navigator.mediaSession:null;if(a&&"setActionHandler"in a){HostMediaSession.setUpMediaSession(a);let b=!0,c=!1;HostMediaSession.browserWrapper={setPaused:function(d){b=d;a.playbackState=b||c?"paused":"playing"},setLoading:function(d){c=d;a.playbackState=b||c?"paused":"playing"},setMetadata:function(d,e,f,g,h,k,m){0<d?(a.metadata=new window.MediaMetadata({title:e,artist:f,album:g,artwork:[{src:"assets/images/albumArts/64x64.png",
sizes:"64x64",type:"image/png"},{src:"assets/images/albumArts/96x96.png",sizes:"96x96",type:"image/png"},{src:"assets/images/albumArts/192x192.png",sizes:"192x192",type:"image/png"},{src:"assets/images/albumArts/256x256.png",sizes:"256x256",type:"image/png"},{src:"assets/images/albumArts/512x512.png",sizes:"512x512",type:"image/png"}]}),"setPositionState"in a&&a.setPositionState({duration:0<k?k/1E3:0,position:0,playbackRate:1}),"none"==a.playbackState&&(a.playbackState="paused")):(a.metadata=null,
a.playbackState="none")},cleanUpMediaSession:function(){HostMediaSession.cleanUpMediaSession(a)}}}}return HostMediaSession.browserWrapper}}HostMediaSession.browserWrapper=null;
...
null;this._volume=0;this.lastObjectURL=null;this.lastTimeS=-1;this.songStartedAutomatically=!0;this.songToResumeTime=null;this.resumeTimeS=0;this.haltOnAllErrors=!1;this.volume=a||0;this.sourceNode=this.audio=null;this.recreateAudioPath();this.mediaSession=c||null;b&&(this.playlist=b)}recreateAudioPath(){const a=this.intermediateNodes;this.sourceNode&&this.sourceNode.disconnectFromDestination();for(var b=a.length-1;0<=b;b--)a[b].disconnectFromDestination();this.audio&&document.body.removeChild(this.audio);
...
!1;this._playlist&&this._playlist.length?0>b&&(c=!this.songStartedAutomatically):c=!0;if(c&&this.onerror)this.onerror(a);else queueMicrotask(this.boundAutoNext)}}audioContextStateChange(){this._alive&&"running"!==this.audioContext.state&&!this.audioContextSuspended&&(this.audioContextSuspended=!0,this.playbackPaused())}playbackLoadStart(){if(this._alive&&!this._loading){this._loading=!0;if(this.onloadingchange)this.onloadingchange(!0);this.mediaSession&&this.mediaSession.setLoading(!0)}}playbackLoadEnd(){if(this._alive&&
this._loading){this._loading=!1;if(this.songToResumeTime){if(this.audio&&this.songToResumeTime===this._currentSong&&0<this.resumeTimeS){this.lastTimeS=this.resumeTimeS;try{this.audio.currentTime=this.resumeTimeS}catch(b){}const a=this.audio.play();a&&a.catch(Player.nop)}this.songToResumeTime=null;this.resumeTimeS=0}if(this.onloadingchange)this.onloadingchange(!1);this.mediaSession&&this.mediaSession.setLoading(!1)}}playbackEnd(){this._alive&&this.next(!0)}playbackError(a,b,c,d,e){this._alive&&this.handleError(e?
e.message?e.message:"string"===typeof e?e:e.toString():"string"===typeof a?a:Strings.UnknownError)}suspendAudioContext(a){this.audioContextTimeout&&(clearTimeout(this.audioContextTimeout),this.audioContextTimeout=0);this.audioContextSuspended||(a?this.audioContextTimeout=window.setTimeout(this.boundCheckAudioContext,5E3):(this.audioContextSuspended=!0,this.audioContext.suspend()))}resumeAudioContext(){this.audioContextTimeout&&(clearTimeout(this.audioContextTimeout),this.audioContextTimeout=0);if(this.audioContextSuspended||
"running"!==this.audioContext.state)this.audioContextSuspended=!1,this.audioContext.resume()}checkAudioContext(){this.audioContextTimeout=0;this._paused?this.audioContextSuspended||(this.audioContextSuspended=!0,this.audioContext.suspend()):this.audioContextSuspended&&(this.audioContextSuspended=!1,this.audioContext.resume())}playbackPaused(){if(this._alive&&!this._paused){this._paused=!0;this.suspendAudioContext(!0);if(this.onpausedchange)this.onpausedchange(!0);this.mediaSession&&this.mediaSession.setPaused(!0)}}playbackStarted(){if(this._alive&&
this._paused){this._paused=!1;this.resumeAudioContext();if(this.onpausedchange)this.onpausedchange(!1);this.mediaSession&&this.mediaSession.setPaused(!1)}}playbackAborted(){if(this._alive){if(this._loading){this._loading=!1;if(this.onloadingchange)this.onloadingchange(!1);this.mediaSession&&this.mediaSession.setLoading(!1)}if(!this._paused){this._paused=!0;this.suspendAudioContext(!0);if(this.onpausedchange)this.onpausedchange(!0);this.mediaSession&&this.mediaSession.setPaused(!0)}}}playbackLengthChange(){this._alive&&
this.audio&&this._playlist&&this._currentSong&&this._playlist.updateSongLength(this._currentSong,this.audio.duration)&&this.notifyMediaSessionChange()}currentTimeChange(){if(this._alive&&this.audio&&this._currentSong){var a=this.audio.currentTime;this.lastTimeS=a;if(this.oncurrenttimeschange)this.oncurrenttimeschange(a)}}notifyMediaSessionChange(){if(this._alive&&this.mediaSession){var a=this._currentSong;a?this.mediaSession.setMetadata(a.id,a.title,a.artist,a.album,a.track,a.lengthMS,a.year):this.mediaSession.setMetadata(0,
null,null,null,0,0,0)}}notifySongChange(){if(this._alive){if(this.onsongchange)this.onsongchange(this._currentSong);this.notifyMediaSessionChange()}}setPlaylistData(){const a=this._playlist;a&&(a.currentIndexResumeTimeS=this.currentIndexResumeTimeS)}previous(a){this._alive&&this._playlist&&(this._playlist.moveCurrentToPrevious(),this.play(-1,a))}seekBackward(){this._alive&&this.seekTo(-1E4,!0)}pause(){this._alive&&(this.suspendAudioContext(!1),this._currentSong&&this.audio&&this.audio.pause())}play(a,
b){if(this._alive&&this.audio){if(this._currentSong&&void 0===a){this.resumeAudioContext();var c=this.audio.play()}else if(this._playlist){void 0!==a&&0<=a&&(this._playlist.currentIndex=a);a=this._playlist.currentItem;!a&&this._playlist.length&&(this._playlist.moveCurrentToNext(),a=this._playlist.currentItem);if(!a){this.stop();return}this.lastTimeS=-1;this.songStartedAutomatically=!!b;this._currentSong!==a&&(this._currentSong=a,queueMicrotask(this.boundNotifySongChange));for(this.resumeAudioContext();this.audio.firstChild;)this.audio.removeChild(this.audio.firstChild);
const d=document.createElement("source");if(a.url&&!a.url.startsWith(FileUtils.localURLPrefix))d.src=a.url;else if(a.file)this.lastObjectURL&&URL.revokeObjectURL(this.lastObjectURL),this.lastObjectURL=URL.createObjectURL(a.file),d.src=this.lastObjectURL;else{if(a.url){this.pause();const e=this._playlist.currentIndex,f=this._playlist.currentItem;FileSystemAPI.getFile(a.url.substring(FileUtils.localURLPrefix.length)).then(g=>{f&&!f.file&&g&&(f.file=g);this._playlist&&e===this._playlist.currentIndex&&
f===this._playlist.currentItem&&(g?this.play(e,b):this.handleError(Strings.FileNotFoundOrNoPermissionError))},()=>{this._playlist&&e===this._playlist.currentIndex&&f===this._playlist.currentItem&&this.handleError(Strings.FileNotFoundOrNoPermissionError)})}else this.handleError(Strings.MissingSongError);return}d.onerror=this.boundPlaybackError;this.audio.appendChild(d);this.audio.load();if(this.songToResumeTime!==a||!this.resumeTimeS||0>=this.resumeTimeS)c=this.audio.play()}c&&c.catch(Player.nop)}}playSongId(a,
b){if(!this._alive||!this.audio||!this._playlist)return!1;a=this._playlist.findIndexById(a);return 0<=a?(this.play(a,b),!0):!1}playPause(){this._alive&&this.audio&&(!this._currentSong||this.audio.paused?this.play():this.pause())}stop(){if(this._alive&&this.audio){if(this._currentSong){for(;this.audio.firstChild;)this.audio.removeChild(this.audio.firstChild);this.audio.load();this.lastObjectURL&&(URL.revokeObjectURL(this.lastObjectURL),this.lastObjectURL=null);if(this._loading){this._loading=!1;if(this.onloadingchange)this.onloadingchange(!1);
this.mediaSession&&this.mediaSession.setLoading(!1)}if(!this._paused){this._paused=!0;if(this.onpausedchange)this.onpausedchange(!0);this.mediaSession&&this.mediaSession.setPaused(!0)}this._currentSong=null;this.lastTimeS=-1;this.songToResumeTime=null;this.resumeTimeS=0;this.notifySongChange()}this.suspendAudioContext(!1)}}seekForward(){this._alive&&this.seekTo(1E4,!0)}next(a){this._alive&&this._playlist&&(this._playlist.moveCurrentToNext(),this.play(-1,a))}seekTo(a,b){this._alive&&this.audio&&this._currentSong&&
this._currentSong.isSeekable&&!(0>=this._currentSong.lengthMS)&&this.audio.seekable&&this.audio.seekable.length&&(this.lastTimeS=a=Math.min(this.audio.duration||0,Math.max(0,a/1E3+(b?this.audio.currentTime:0))),this.audio.currentTime=a)}}Player.minVolume=-40;Player.nop=function(){};
