...
a&&(a=0);var b=a|0;AppUI.currentTimeS!==b?(AppUI.currentTimeS=b,AppUI.seekSlider.manuallyChangeAll(1E3*a,Formatter.formatTimeS(b),SliderControlValueChild.LeftChild)):AppUI.seekSlider.value=1E3*a}}static playerError(a){Modal.show({text:a,returnFocusElement:AppUI.playlistControl.element})}static playlistSongLengthChange(a){App.player&&(a===App.player.currentSong&&(Strings.changeText(AppUI.songLengthLabel,a.length),Strings.changeText(AppUI.assistiveSongLengthLabel,a.length),AppUI.seekSlider&&(0<a.lengthMS?
(AppUI.seekSlider.disabled=!1,AppUI.seekSlider.max=a.lengthMS):(AppUI.seekSlider.value=0,AppUI.seekSlider.disabled=!0))),AppUI.playlistControl&&AppUI.playlistControl.refreshVisibleItems())}static seekSliderValueChange(a){AppUI.seekSlider&&AppUI.seekSlider.dragging&&(a=a/1E3|0,AppUI.currentTimeS!==a&&(AppUI.currentTimeS=a,AppUI.seekSlider.manuallyChangeAria(Formatter.formatTimeS(a),SliderControlValueChild.LeftChild)))}static seekSliderDragEnd(a){App.player&&App.player.seekTo(a)}static volumeSliderValueChange(a){App.player&&
(App.player.volume=a)}static playlistControlItemClick(a,b,c){App.player&&!c&&App.player.play(b)}static playlistControlItemContextMenu(a,b){}static graphicalFilterControlTypeClick(){App.graphicalFilterControl&&(App.graphicalFilterControl.simpleMode=!App.graphicalFilterControl.simpleMode,ButtonControl.setText(AppUI.graphicalFilterControlType,App.graphicalFilterControl.simpleMode?Strings.TraditionalFilter:Strings.AdvancedFilter))}static graphicalFilterControlEnabledClick(){App.graphicalFilterControl&&
(App.graphicalFilterControl.enabled=AppUI.graphicalFilterControlEnabled.checked)}static stereoPannerControlEnabledClick(){App.stereoPannerControl&&(App.stereoPannerControl.enabled=AppUI.stereoPannerControlEnabled.checked)}static monoDownMixerControlEnabledClick(){App.monoDownMixerControl&&(App.monoDownMixerControl.enabled=AppUI.monoDownMixerControlEnabled.checked)}static hideTopMessage(){0>AppUI.topMessageFading||!AppUI.topMessage||(AppUI.topMessageTimeout&&clearTimeout(AppUI.topMessageTimeout),AppUI.topMessageFading=
-1,AppUI.topMessage.classList.remove("in"),AppUI.topMessageTimeout=DelayControl.delayFadeCB(function(){0<=AppUI.topMessageFading||(AppUI.topMessageFading=0,AppUI.topMessageTimeout=0,AppUI.topMessage.style.visibility="",AppUI.topMessage.innerHTML="",AppUI.titleLabel&&AppUI.titleLabel.classList.remove("behind"),AppUI.artistLabel&&AppUI.artistLabel.classList.remove("behind"))}))}static showTopMessage(a,b){AppUI.topMessage&&(AppUI.topMessageTimeout&&clearTimeout(AppUI.topMessageTimeout),AppUI.topMessageFading=
1,AppUI.titleLabel&&AppUI.titleLabel.classList.add("behind"),AppUI.artistLabel&&AppUI.artistLabel.classList.add("behind"),AppUI.topMessage.innerHTML=a,b&&b(AppUI.topMessage),AppUI.topMessage.style.visibility="visible",AppUI.topMessageTimeout=DelayControl.delayShortCB(function(){0>=AppUI.topMessageFading||1!==AppUI.topMessageFading||(AppUI.topMessageFading=2,AppUI.topMessage.classList.add("in"),AppUI.topMessageTimeout=DelayControl.delayFadeCB(function(){2===AppUI.topMessageFading&&(AppUI.topMessageFading=
0,AppUI.topMessageTimeout=0)}))}))}static addFiles(a){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading&&!Modal.visible&&!FilePicker.visible){var b=App.hostType===App.hostTypeElectron;if(FilePicker.isSupported()){var c=null;var d=FilePicker.showAddPlay(AppUI.playlistControl.element)}else c=yield App.showOpenDialogWeb(a),d=null;if((c||d)&&App.player)try{App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const e=App.player.playlist,f=b?null:
new Uint8Array(BufferedReader.minBufferLength),g=b?null:[new Uint8Array(2048)];let k=!1;do{let h=!1;if(d){const m=yield d();if(!m)break;h=m.play;c=m.files}if(!c)break;AppUI._loading=!0;App.updateLoadingIcon();for(let m=0;m<c.length&&App.player;m++)if(b)(yield e.addSong(c[m]))&&h&&(h=!1,App.player&&App.player.play(e.length-1));else{const l=e.length;if(yield e.addSongWeb(c[m],f,g))l==e.length?k=!0:h&&(h=!1,App.player&&App.player.play(l))}AppUI._loading=!1;App.updateLoadingIcon()}while(d);k&&AppUI.playlistControl&&
...
!0,AppUI.focusBlocker=new FocusBlocker,AppUI.focusBlocker.block(AppUI.middlePanel),AppUI.showTopMessage(Strings.DeleteModeHTML,function(b){const c=document.createElement("div");ButtonControl.create({color:"red",icon:"icon-delete-all",text:Strings.DeleteAllSongs,parent:c,onclick:function(){App.player&&App.player.playlist&&App.player.playlist.clear();AppUI.playlistControl&&AppUI.playlistControl.deleteMode&&AppUI.toggleDeleteMode(!0)}});ButtonControl.create({color:"green",icon:"icon-check",text:Strings.Done,
parent:c,onclick:function(){AppUI.playlistControl&&AppUI.playlistControl.deleteMode&&AppUI.toggleDeleteMode(!0)}});b.appendChild(c)}),HistoryHandler.pushState());try{AppUI.playlistControl.element.focus()}catch(b){}}}static checkPanelContainerToggleState(){if(!(875>=window.innerWidth)&&AppUI.fixedPanel&&AppUI.optionalPanel){if(AppUI.panelContainerToggling){AppUI.panelContainerToggleVersion++;AppUI.panelContainerToggling=!1;const a=AppUI.cover.parentNode;a&&a.removeChild(AppUI.cover);AppUI.cover.classList.remove("in")}AppUI.panelContainerToggled&&
(AppUI.panelContainerToggled=!1,AppUI.panelContainer.classList.remove("toggled"),HistoryHandler.popState())}}static toggleView(a){if(AppUI.panelContainer){AppUI.panelContainerToggleVersion++;const b=AppUI.panelContainerToggleVersion,c=function(d,e){AppUI.panelContainerToggling=!0;AppUI.cover.classList.remove("in");AppUI.cover.parentNode||document.body.appendChild(AppUI.cover);DelayControl.delayShortCB(function(){return __awaiter(this,void 0,void 0,function*(){if(AppUI.panelContainerToggleVersion===
b&&(AppUI.cover.classList.add("in"),yield DelayControl.delayFade(),AppUI.panelContainerToggleVersion===b&&(e?AppUI.panelContainer.classList.add("toggled"):AppUI.panelContainer.classList.remove("toggled"),AppUI.cover.classList.remove("in"),yield DelayControl.delayFade(),AppUI.panelContainerToggleVersion===b))){var f=AppUI.cover.parentNode;f&&f.removeChild(AppUI.cover);AppUI.panelContainerToggling=!1;try{d.focus()}catch(g){}}})})};AppUI.panelContainerToggled?(AppUI.panelContainerToggled=!1,c(AppUI.playlistControl.element,
!1),a&&HistoryHandler.popState()):(AppUI.panelContainerToggled=!0,c(ButtonControl.getDefaultFocusElement(AppUI.toggleViewButton),!0),HistoryHandler.pushState())}}static showSongInfo(a){a||(a=App.player&&App.player.currentSong);a?Modal.show({html:`<div class="selectable-text">
