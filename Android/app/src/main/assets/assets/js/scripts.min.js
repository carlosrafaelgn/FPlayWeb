...
this.oncurrenttimeschange=this.onpausedchange=this.onloadingchange=this.onsongchange=null;this.boundPlaybackError=this.playbackError.bind(this);this.boundNotifySongChange=this.notifySongChange.bind(this);this.boundCheckAudioContext=this.checkAudioContext.bind(this);this.audio=new Audio;this.audio.loop=!1;this.audio.controls=!1;this.audio.volume=1;b=this.playbackLoadStart.bind(this);this.audio.onwaiting=b;this.audio.onloadstart=b;this.audio.oncanplay=this.playbackLoadEnd.bind(this);this.audio.onended=
this.playbackEnd.bind(this);this.audio.onerror=this.boundPlaybackError;this.audio.onpause=this.playbackPaused.bind(this);b=this.playbackStarted.bind(this);this.audio.onplay=b;this.audio.onplaying=b;b=this.playbackAborted.bind(this);this.audio.onabort=b;this.audio.onemptied=b;this.audio.ondurationchange=this.playbackLengthChange.bind(this);this.audio.ontimeupdate=this.currentTimeChange.bind(this);this.source=this.audioContext.createMediaElementSource(this.audio);this._alive=!0;this._loading=!1;this._paused=
!0;this._currentSong=this._playlist=null;this._volume=0;this.lastObjectURL=null;this.volume=void 0===a?0:a;this.sourceNode=new SourceNode(this.source);this.sourceNode.enabled=!0;this.destinationNode=new DestinationNode(this.audioContext.destination);this.destinationNode.enabled=!0;if(c.length){this.sourceNode.connectToDestination(c[0]);for(a=0;a<c.length-1;a++)c[a].connectToDestination(c[a+1]);c[c.length-1].connectToDestination(this.destinationNode)}else this.sourceNode.connectToDestination(this.destinationNode);
...
AppUI.toggleDeleteMode(!0):App.player&&App.player.playPause()),cancelEvent(a);case "F1":return!a.repeat&&App.player&&App.player.previous(),cancelEvent(a);case "F2":return!a.repeat&&App.player&&App.player.playPause(),cancelEvent(a);case "F3":return!a.repeat&&App.player&&App.player.stop(),cancelEvent(a);case "F4":return!a.repeat&&App.player&&App.player.next(),cancelEvent(a)}}static historyStatePopped(){if(!AppUI.playlistControl||!AppUI.playlistControl.deleteMode){if(!AppUI.panelContainerToggled)return null;
AppUI.toggleView(!1);return!0}AppUI.toggleDeleteMode(!1);return!0}static preInit(a,b){window.addEventListener("resize",AppUI.adjustCover);AppUI.adjustCover();AppUI.webFrame=a;window.addEventListener("keydown",AppUI.globalKeyHandler,!0);a&&b.devicePixelRatio&&b.devicePixelRatio!==devicePixelRatio&&a.setZoomFactor(b.devicePixelRatio);FilePicker.isSupported()&&(FilePicker.lastPath=b.filePickerLastPath||null,FilePicker.lastRootLength=b.filePickerRootLength||0,a=document.getElementById("add-folder-button"),
a.parentNode.removeChild(a));ButtonControl.init();CheckboxControl.init();AppUI.seekSlider=new SliderControl("seek-slider",Strings.Seek,null,SliderControlValueChild.None,!1,!1,0,1,0,null,null,!0);AppUI.seekSlider.disabled=!0;AppUI.seekSlider.onvaluechange=AppUI.seekSliderValueChange;AppUI.seekSlider.ondragend=AppUI.seekSliderDragEnd;a=document.createElement("span");a.className="seek-label";AppUI.currentTimeS=0;Strings.changeText(a,Formatter.none);AppUI.seekSlider.leftChild=a;AppUI.songLengthLabel=
document.createElement("span");AppUI.songLengthLabel.className="seek-label";Strings.changeText(AppUI.songLengthLabel,Formatter.none);Strings.changeText(AppUI.assistiveSongLengthLabel,Formatter.none);AppUI.seekSlider.rightChild=AppUI.songLengthLabel;AppUI.volumeSlider=new SliderControl("volume-slider",Strings.Volume,function(c){return(c<=Player.minVolume?GraphicalFilterEditorStrings.MinusInfinity:c?c:"-0")+" dB"},SliderControlValueChild.RightChild,!0,!1,Player.minVolume,0,b.playerVolume);AppUI.volumeSlider.leftChild=
Icon.createLarge("icon-volume","green small-right-margin");a=document.createElement("span");a.className="small-left-margin";a.setAttribute("id","volume-label");AppUI.volumeSlider.rightChild=a;AppUI.volumeSlider.onvaluechange=AppUI.volumeSliderValueChange;AppUI.playlistControl=new ListControl("playlist-control",Strings.Playlist,!0);AppUI.playlistControl.onitemclick=AppUI.playlistControlItemClick;AppUI.playlistControl.onitemcontextmenu=AppUI.playlistControlItemContextMenu;AppUI.graphicalFilterControlType=
document.getElementById("graphical-filter-control-type");ButtonControl.setText(AppUI.graphicalFilterControlType,b.graphicalFilterControlSimpleMode?Strings.TraditionalFilter:Strings.AdvancedFilter);AppUI.graphicalFilterControlType.onclick=AppUI.graphicalFilterControlTypeClick;AppUI.graphicalFilterControlEnabled=document.getElementById("graphical-filter-control-enabled");AppUI.graphicalFilterControlEnabled.checked=b.graphicalFilterControlEnabled||!1;AppUI.graphicalFilterControlEnabled.onclick=AppUI.graphicalFilterControlEnabledClick;
AppUI.stereoPannerControlEnabled=document.getElementById("stereo-panner-control-enabled");AppUI.stereoPannerControlEnabled.checked=b.stereoPannerControlEnabled||!1;AppUI.stereoPannerControlEnabled.onclick=AppUI.stereoPannerControlEnabledClick;MonoDownMixerControl.isSupported()?(AppUI.monoDownMixerControlEnabled=document.getElementById("mono-down-mixer-control-enabled"),AppUI.monoDownMixerControlEnabled.checked=b.monoDownMixerControlEnabled||!1,AppUI.monoDownMixerControlEnabled.onclick=AppUI.monoDownMixerControlEnabledClick):
(b=document.getElementById("mono-down-mixer-control-container"),b.parentNode.removeChild(b));App.frameless&&AppUI.panelContainer.classList.remove("web");window.addEventListener("resize",AppUI.adjustWindow,{passive:!0});AppUI.adjustWindow()}static init(a){if(App.player){AppUI.volumeSlider.value=App.player.volume;App.player.onsongchange=AppUI.playerSongChange;App.player.onloadingchange=AppUI.playerLoadingChange;App.player.onpausedchange=AppUI.playerPausedChange;App.player.oncurrenttimeschange=AppUI.playerCurrentTimeSChange;
App.player.onerror=AppUI.playerError;AppUI.preparePlaylist();AppUI.playerSongChange(App.player.currentSong);AppUI.centerCurrentSongIntoView();try{AppUI.playlistControl.element.focus()}catch(b){}HistoryHandler.init(Menu.historyStatePopped,Modal.historyStatePopped,AppUI.historyStatePopped);AppUI.cover.classList.remove("in");DelayControl.delayShortCB(function(){AppUI.centerCurrentSongIntoView();DelayControl.delayFadeCB(function(){AppUI.cover&&document.body.removeChild(AppUI.cover)})})}}static adjustCover(){if(AppUI.cover){const a=
document.body.getBoundingClientRect();AppUI.cover.style.transform=`scale(${Math.ceil(.5*a.right)}, ${Math.ceil(.5*a.bottom)})`}}static centerCurrentSongIntoView(){App.player&&App.player.playlist&&0<=App.player.playlist.currentIndex&&AppUI.playlistControl.centerItemIntoView(App.player.playlist.currentIndex)}static destroy(){zeroObject(AppUI)}static get loading(){return AppUI._loading}static get fontScale(){return AppUI._fontScale}static get smallIconSizePX(){return AppUI._smallIconSizePX}static get largeIconSizePX(){return AppUI._largeIconSizePX}static get thinBorderPX(){return AppUI._thinBorderPX}static get thickBorderPX(){return AppUI._thickBorderPX}static get buttonSizePX(){return AppUI._buttonSizePX}static get contentsSizePX(){return AppUI._contentsSizePX}static get playlistItemSizePX(){return AppUI._playlistItemSizePX}static get playlistControlElement(){return AppUI.playlistControl.element}static addZoomHandler(a){a&&
...
App.graphicalFilterControl.saveSettings();App.stereoPannerControl&&App.stereoPannerControl.saveSettings();if(a&&a.length)for(let d=a.length-1;0<=d;d--)if(a[d]){const e=a[d]();e&&b.push(e)}c&&InternalStorage.savePlaylistWeb(c);App.hostInterface&&(b.length?Promise.all(b).then(App.destroy,App.destroy):App.destroy())}}static destroy(){App.player&&App.player.destroy();AppUI.destroy();const a=App.ipcRenderer;zeroObject(App);a&&a.invoke("mainWindowFinalClose")}static updateLoadingIcon(){App.iconLoading&&
