...
d=1-d;d*=d;d+=.5*d;.55>d?d=0:1>d&&(d=smoothStep(.55,1,d));k=1-k;k=smoothStep(0,1,k);k*=k;k+=k;1<k&&(k=1);d+=.5*k;d=255*d|0;c[64*e+g]=255<=d?255:d}}a.texImage2D(a.TEXTURE_2D,0,a.ALPHA,64,64,0,a.ALPHA,a.UNSIGNED_BYTE,c)}analyze(a){let b=a-this.lastTime;33<b&&(b=33);this.lastTime=a;a=this.program.gl;var c=.00390625*b,d=1-c;const e=this.processedData,f=this.processedDataR,g=this.fft,k=this.BG_COLUMNS,h=this.BG_PARTICLES_BY_COLUMN,m=this.COLORS,l=this.program,n=this.bgPos,q=this.bgSpeedY,p=this.bgColor,
...
class SimpleFilterEditorControl{constructor(a,b){this.element=a;this.editor=b;a.classList.add("simple-filter");b=Array(GraphicalFilterEditor.shelfEquivalentZoneCount);const c=function(d){return Formatter.formatDB(.5*d)};this.sliders=b;for(let d=0;d<b.length;d++){const e=document.createElement("span"),f=document.createElement("span"),g=document.createElement("span"),k=this.filterToSlider(d);let h;f.className="simple-filter-freq small-bottom-margin";switch(d){case 0:f.innerHTML="31<br/>62";h="31 / 62 Hz";
break;case 1:f.innerHTML="<br/>125";h="125 Hz";break;case 2:f.innerHTML="<br/>250";h="250 Hz";break;case 3:f.innerHTML="500<br/>1k";h="500 / 1000 Hz";break;case 4:f.innerHTML="2k<br/>4k";h="2000 / 4000 Hz";break;case 5:f.innerHTML="<br/>8k";h="8000 Hz";break;default:f.innerHTML="<br/>16k",h="16000 Hz"}g.className="small-top-margin";e.className="simple-filter-slider"+(!d||AppUI.primaryInputIsTouch?"":" left-margin");a.appendChild(e);b[d]=new SliderControl(e,h,c,SliderControlValueChild.LeftChild,!0,
!0,-30,30,k,g,f);this.createHandler(b[d],d)}}static sliderToY(a){return GraphicalFilterEditor.zeroChannelValueY-(a<<1)}static yToSlider(a){return Math.max(-30,Math.min(30,-(a-GraphicalFilterEditor.zeroChannelValueY)>>1))}filterToSlider(a){return SimpleFilterEditorControl.yToSlider(this.editor.getShelfZoneY(a))}createHandler(a,b){const c=this.editor,d=function(e){c.changeShelfZoneY(b,SimpleFilterEditorControl.sliderToY(e));c.commitChanges()};a.ondragend=d;a.onkeyboardchange=d}updateSliders(){const a=
...
c);d.className="modal"+(a.fullHeight?" full-height":"");d.onsubmit=this.submit.bind(this);var e=document.createElement("h1");this.modalHeaderElement=e;(a.titleHeader||e).setAttribute("id",c);e.className="modal-header padding"+(a.rightHeader?" right":"");a.title?"string"===typeof a.title?e.innerHTML=a.title:e.appendChild(a.title):e.innerHTML=a.titleStringKey?Strings.translate(a.titleStringKey):Strings.Oops;this.modalBodyElement=c=document.createElement("div");c.className="modal-body scrollable padding top-margin"+
(a.leftBody?" left":"");a.html||(a.html=document.createElement("div"),a.text&&Strings.changeText(a.html,a.text));if("string"===typeof a.html)c.innerHTML=a.html;else if(Array.isArray(a.html)){var f=a.html;for(var g=0;g<f.length;g++)c.appendChild(f[g])}else c.appendChild(a.html);if((f=c.getElementsByTagName("button"))&&f.length)for(g=f.length-1;0<=g;g--){const k=f[g];ButtonControl.prepare(k);k.onclick=()=>{if(!this.fading&&this.options.onbuttonclick)this.options.onbuttonclick(k.id,k)}}this.modalFooterElement=
f=document.createElement("div");f.className="modal-footer padding top-border left-margin right-margin";this.defaultSubmitButton=this.defaultCancelButton=null;if(a.buttons)for(g=0;g<a.buttons.length;g++){const k=a.buttons[g],h=ButtonControl.create(k,!0);k.defaultCancel&&(this.defaultCancelButton=h);k.defaultSubmit&&(this.defaultSubmitButton=h);h.onclick=()=>{if(!this.fading){if(k.onclick)k.onclick(k.id,h);if(this.options.onbuttonclick)this.options.onbuttonclick(k.id,h)}};f.appendChild(h)}d.appendChild(e);
if(a.skipBody)for(;c.firstChild;)e=c.firstChild,c.removeChild(e),d.appendChild(e);else d.appendChild(c);d.appendChild(f);b.appendChild(d);document.body.appendChild(b);this.boundDocumentKeyDown=this.documentKeyDown.bind(this);document.addEventListener("keydown",this.boundDocumentKeyDown,!0);this.fading=!0;DelayControl.delayShortCB(()=>{if(a.onshowing)a.onshowing();this.containerElement.classList.add("in");DelayControl.delayFadeCB(()=>{this.fading=!1;const k=this.options.onshown&&this.options.onshown()||
this.modalElement;try{k.focus()}catch(h){}})})}static get visible(){return!!Modal.modal}static get currentModalElement(){return Modal.modal?Modal.modal.modalElement:null}static get currentModalHeaderElement(){return Modal.modal?Modal.modal.modalHeaderElement:null}static get currentModalBodyElement(){return Modal.modal?Modal.modal.modalBodyElement:null}static get currentModalFooterElement(){return Modal.modal?Modal.modal.modalFooterElement:null}static show(a){if(Modal.modal)return!1;a.okCancel?a.buttons=
[{id:"cancel",defaultCancel:!0,icon:"icon-clear",text:a.cancelText||Strings.Cancel,color:"red",onclick:a.oncancel||Modal.hide},{id:"ok",defaultSubmit:a.okCancelSubmit,icon:"icon-check",text:a.okText||Strings.OK,color:"green",onclick:a.onok||Modal.hide}]:a.buttons&&a.buttons.length||(a.buttons=[{id:"cancel",defaultCancel:!0,icon:"icon-clear",text:Strings.Close,color:"red",onclick:Modal.hide}]);Modal.modal=new Modal(a);return!0}static hide(){Modal.modal&&Modal.modal.hideInternal()}static defaultCancelAction(){return Modal.modal?
...
class FilePicker{constructor(a,b,c){const d=document.createElement("div"),e=Strings.createSrOnlyText(Strings.AddSongs);d.appendChild(e);ButtonControl.create({color:"orange",icon:"icon-up",text:Strings.Up,parent:d,onclick:this.navigateUp.bind(this)});ButtonControl.create({icon:"icon-checkbox-1",text:Strings.All,parent:d,onclick:this.toggleAll.bind(this)});const f=document.createElement("div");f.className="file-picker-path padding bottom-border left";f.setAttribute("aria-atomic","true");f.setAttribute("aria-live",
"assertive");f.setAttribute("aria-relevant","all");const g=Icon.createLarge("icon-folder-open","orange hidden",null,Strings.CurrentPathLabel);f.appendChild(g);const k=document.createElement("i");k.className="icon loading large";k.appendChild(Strings.createSrOnlyText(Strings.LoadingCurrentPathLabel));f.appendChild(k);const h=document.createElement("div");h.className="file-picker-path-element small-left-padding";f.appendChild(h);const m=document.createElement("div");m.className="file-picker-list fade";
...
FilePicker.filePicker=null;this.provider&&this.provider.destroy();return!0}modalHidden(){const a=this.resolve,b=this.selectedFiles;this.selectedFiles=null;zeroObject(this);a&&a(b)}modalOk(a){if(FilePicker.filePicker===this&&!this.gettingFiles&&!this.fading){var b=[],c=[],d=this.list,e=d.length;if(this.path)for(let g=0;g<e;g++){const k=d.item(g);k.selected&&(k.dir?b:c).push(k)}if(this.path&&(b.length||c.length)){FilePicker.lastPath=this.path;FilePicker.lastRootLength=this.rootLength;this.gettingFiles=
!0;this.providerVersion++;var f=this.providerVersion;this.updateIconLoading(!0);this.provider.getFiles(b,c).then(g=>{if(FilePicker.filePicker===this&&this.provider&&f===this.providerVersion)if(this.callback){const k=this.list;for(let h=k.length-1;0<=h;h--)k.item(h).selected=!1;this.listControl.refreshVisibleItems();this.updateIconLoading(!1);this.gettingFiles=!1;this.callback({play:a,files:g})}else this.selectedFiles=g,Modal.hide()},g=>{FilePicker.filePicker===this&&this.provider&&f===this.providerVersion&&
