...
e)return;this._firstIndex=e;var f=this._visibleCount+(0===e?2:1===e?3:4);f>b.length&&(f=b.length);0>(e-=2)&&(e=0);if(e>d){if((d=e-d)&&d<f)for(let m=d,l=0;m<f;m++,l++)d=b[l],b[l]=b[m],b[m]=d}else if((d-=e)&&d<f)for(let m=f-1,l=m-d;0<=l;l--,m--)d=b[m],b[m]=b[l],b[l]=d;var g=a.list,h=g.length,k=a.list.currentItem,n=this._keyboardIndex;d=0;for(let m=e*c;d<f&&e<h;d++,e++,m+=c){const l=g.item(e),p=b[d];if(p.item!==l){let r=k===l;p.current!==r&&((p.current=r)?p.element.classList.add("current"):p.element.classList.remove("current"));
r=n===e;p.keyboard!==r&&((p.keyboard=r)?(p.element.classList.add("keyboard"),p.element.ariaSelected="true",this.setAttribute("aria-activedescendant",p.element.id)):(this.getAttribute("aria-activedescendant")===p.element.id&&this.removeAttribute("aria-activedescendant"),p.element.classList.remove("keyboard"),p.element.ariaSelected="false"));a.prepareElement(l,e,h,p.element);p.element.ariaRowIndex=e.toString();p.item||this.appendChild(p.element);p.item=l}p.index!==e&&(p.index=e,p.element.style.transform=
"translateY("+m+"px)")}for(a=b.length;d<a;d++)c=b[d],c.item&&(c.item=null,this.removeChild(c.element));this._lastIndex=e-1;this._lastIndex<this._firstIndex&&(this._lastIndex=this._firstIndex)}}refreshVisibleItemsInternal(){this._refreshVisibleItemsEnqueued=!1;var a=this._adapter;if(a){this.adjustContainerHeight();var b=this._items,c=this._itemHeightAndMargin,d=a.list,e=d.length,f=a.list.currentItem,g=this._keyboardIndex;if(this._useVirtualItems){this._currentListItem=f;var h=0,k=this._firstIndex,
n=this._visibleCount+(0===k?2:1===k?3:4);n>b.length&&(n=b.length);0>(k-=2)&&(k=0);for(let m=k*c;h<n&&k<e;h++,k++,m+=c){const l=d.item(k),p=b[h];let r=f===l;p.current!==r&&((p.current=r)?p.element.classList.add("current"):p.element.classList.remove("current"));r=g===k;p.keyboard!==r&&((p.keyboard=r)?(p.element.classList.add("keyboard"),p.element.ariaSelected="true",this.setAttribute("aria-activedescendant",p.element.id)):(this.getAttribute("aria-activedescendant")===p.element.id&&this.removeAttribute("aria-activedescendant"),
...
"false"),g?(g.element.classList.add("keyboard"),g.element.ariaSelected="true",this.setAttribute("aria-activedescendant",g.element.id)):this.removeAttribute("aria-activedescendant"),this._keyboardItem=g);for(let m=0,l=this.indexFromY(0);m<=h&&l<e;m++,l++)g=b[l].element,a.prepareElement(d.item(l),l,e,g),g.ariaRowIndex=l.toString()}}}resize(){const a=this._adapter;if(a){var b=this._clientHeight,c=this.clientHeight;0>c&&(c=0);var d=Math.ceil(c/this._itemHeightAndMargin)+1;if(this._visibleCount!==d&&(this._visibleCount=
...
const n=document.createElement("f-list");n.className="file-picker-list fade";this._callback=c||null;this._resolve=a;this._provider=App.hostType===App.hostTypeAndroid?new FilePickerAndroidProvider:new FilePickerFileSystemAPIProvider;this._list=new List;this._listAdapter=new FilePickerListAdapter(this._list);this._listControl=n;this._listControl.addEventListener("keydown",this.listKeyDown.bind(this));this._listControl.onitemclick=this.itemClick.bind(this);this._listControl.onitemcontrolclick=this.itemControlClick.bind(this);
