...
this.firstIndex=f;var g=this.visibleCount+(0===f?2:1===f?3:4);g>c.length&&(g=c.length);0>(f-=2)&&(f=0);if(f>e){if((e=f-e)&&e<g)for(let n=e,r=0;n<g;n++,r++)e=c[r],c[r]=c[n],c[n]=e}else if((e-=f)&&e<g)for(let n=g-1,r=n-e;0<=r;r--,n--)e=c[n],c[n]=c[r],c[r]=e;var h=a.list,k=h.length,m=a.list.currentItem,l=this.keyboardIndex;e=0;for(let n=f*d;e<g&&f<k;e++,f++,n+=d){const r=h.item(f),p=c[e];if(p.item!==r){let q=m===r;p.current!==q&&((p.current=q)?p.element.classList.add("current"):p.element.classList.remove("current"));
q=l===f;p.keyboard!==q&&((p.keyboard=q)?(p.element.classList.add("keyboard"),p.element.setAttribute("aria-selected","true"),this.element.setAttribute("aria-activedescendant",p.element.getAttribute("id"))):(this.element.getAttribute("aria-activedescendant")===p.element.getAttribute("id")&&this.element.removeAttribute("aria-activedescendant"),p.element.classList.remove("keyboard"),p.element.setAttribute("aria-selected","false")));a.prepareElement(r,f,k,p.element);p.element.setAttribute("aria-rowindex",
f.toString());p.item||b.appendChild(p.element);p.item=r}p.index!==f&&(p.index=f,p.element.style.transform="translateY("+n+"px)")}for(a=c.length;e<a;e++)d=c[e],d.item&&(d.item=null,b.removeChild(d.element));this.lastIndex=f-1;this.lastIndex<this.firstIndex&&(this.lastIndex=this.firstIndex)}}refreshVisibleItemsInternal(){this.refreshVisibleItemsEnqueued=!1;var a=this._adapter,b=this.element;if(a&&b){this.adjustContainerHeight();var c=this.items,d=this.itemHeightAndMargin,e=a.list,f=e.length,g=a.list.currentItem,
h=this.keyboardIndex;if(this.useVirtualItems){this.currentListItem=g;var k=0,m=this.firstIndex,l=this.visibleCount+(0===m?2:1===m?3:4);l>c.length&&(l=c.length);0>(m-=2)&&(m=0);for(let n=m*d;k<l&&m<f;k++,m++,n+=d){const r=e.item(m),p=c[k];let q=g===r;p.current!==q&&((p.current=q)?p.element.classList.add("current"):p.element.classList.remove("current"));q=h===m;p.keyboard!==q&&((p.keyboard=q)?(p.element.classList.add("keyboard"),p.element.setAttribute("aria-selected","true"),this.element.setAttribute("aria-activedescendant",
p.element.getAttribute("id"))):(this.element.getAttribute("aria-activedescendant")===p.element.getAttribute("id")&&this.element.removeAttribute("aria-activedescendant"),p.element.classList.remove("keyboard"),p.element.setAttribute("aria-selected","false")));a.prepareElement(r,m,f,p.element);p.element.setAttribute("aria-rowindex",m.toString());p.item||b.appendChild(p.element);p.item=r;p.index!==m&&(p.index=m,p.element.style.transform="translateY("+n+"px)")}for(a=c.length;k<a;k++)e=c[k],e.item&&(e.item=
