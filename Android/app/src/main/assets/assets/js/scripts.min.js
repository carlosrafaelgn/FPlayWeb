...
b=a.lastIndexOf("/"),c=0<=b?a.substring(b+1):a,this.isHttpURL||(b=c.lastIndexOf("."),0<b&&(c=c.substring(0,b))));this.title=c||Formatter.none;this.artist=d||Formatter.none;this.album=e||Formatter.none;this.track=f&&0<f?f:Formatter.noneInt;this.lengthMS=g&&0<g?g:Formatter.noneInt;this.year=h&&0<h?h:Formatter.noneInt;this.sampleRate=k&&0<k?k:Formatter.noneInt;this.channels=n&&0<n?n:Formatter.noneInt;this.length=Formatter.formatTimeMS(this.lengthMS);this.fileName=m||null;this.fileSize=l||0;this.customProvider=
this.fileExtension&&FileUtils.getCustomProviderByExtension(this.fileExtension)||null}get isSeekable(){return this.flags&MetadataFlags.Seekable?!0:!1}get absolutePath(){return this.isLocalURL?this.url.substring(FileUtils.localURLPrefix.length):this.isFileURL?decodeURI(this.url.substring(FileUtils.fileURLPrefix.length)):this.url}estimateSerializedLength(){return(this.url.length+this.title.length+this.artist.length+this.album.length+(this.fileName?this.fileName.length:0)<<1)+17}serialize(a){return a.writeUint8(0).writeString(this.url).writeInt32(this.flags).writeString(this.title).writeString(this.artist).writeString(this.album).writeInt16(this.track).writeInt32(this.lengthMS).writeInt16(this.year).writeInt32(this.sampleRate).writeInt8(this.channels).writeString(this.fileName||
...
this.album!==Formatter.none&&b.push(this.album),this.year&&0<this.year&&b.push(this.year.toString()),this.url&&b.push(this.url),this._normalizedInfo=b=Strings.removeDiacritics(b.join(" ")));for(let c=a.length-1;0<=c;c--)if(!b.includes(a[c]))return!1;return!0}async readAsArrayBuffer(){if(!this.file)if(this.isLocalURL){var a=await FileSystemAPI.getFile(this.absolutePath);if(!a)return null;this.file=a}else return a=await fetch(this.url),a.ok?a.arrayBuffer():null;return this.file.arrayBuffer()}}
Song._lastId=0;
