...
FilePickerAndroidProvider.callback&&(FilePickerAndroidProvider.callback(0,null),FilePickerAndroidProvider.callback=null);return new Promise(b=>{if(!App.hostInterface)return null;FilePickerAndroidProvider.callback=(c,d)=>{c!==this.enumerationVersion?b(null):(FilePickerAndroidProvider.callback=null,b(d))};App.hostInterface.enumerateFiles(this.enumerationVersion,a)})}async getDirectoryFiles(a,b,c){if((c=await this.enumerate(c))&&a===this.version){var d=Strings.comparer;c.sort(function(e,f){const g=47===
e.charCodeAt(0),k=47===f.charCodeAt(0);return g===k?d(e,f):g?1:-1});for(let e=0;e<c.length&&a===this.version;e++){const f=c[e];47===f.charCodeAt(0)?b.push(f):await this.getDirectoryFiles(a,b,f)}}}async getFiles(a,b){this.cancel();const c=[];var d=this.version,e=App.showOpenDialogWeb(!1,!0);if(a)for(let f=0;f<a.length&&d===this.version;f++)await this.getDirectoryFiles(d,c,a[f].path);if(b&&d===this.version)for(a=0;a<b.length;a++)c.push(b[a].path);if(!App.hostInterface||!c.length)return null;for(b=c.length-
1;0<=b;b--)c[b]=FileUtils.fileURLPrefix+encodeURI(c[b]);App.hostInterface.setFileURLs(c);e=await e;if(!e||e.length!==c.length||d!==this.version)return null;for(d=c.length-1;0<=d;d--)e[d]["data-path"]=c[d];return e}async navigate(a){this.cancel();var b=this.version;if(!FilePickerAndroidProvider.permissionGranted&&!await FilePickerAndroidProvider.checkPermission()||b!==this.version)return null;const c=await this.enumerate(a);if(!c||!c.length||b!==this.version)return null;b=Array(c.length);if(a)for(a=
b.length-1;0<=a;a--){var d=c[a];b[a]={name:FileUtils.nameFromLinuxPath(d),dir:47===d.charCodeAt(0)?0:1,path:d}}else for(a=b.length-1;0<=a;a--){d=c[a];const f=d.indexOf(FileUtils.separator);b[a]={name:0<f?d.substring(0,f):Formatter.none,dir:1,path:0<f?d.substring(f+1):d,root:!0}}const e=Strings.comparer;b.sort(function(f,g){return f.dir===g.dir?e(f.name,g.name):g.dir-f.dir});return b}cancel(){this.version++;this.enumerationVersion++}destroy(){App.hostInterface&&App.hostInterface.cancelFileEnumeration(this.enumerationVersion);
this.cancel()}}FilePickerAndroidProvider.callback=null;FilePickerAndroidProvider.callbackPermissionResolve=null;FilePickerAndroidProvider.permissionGranted=!1;
...
if(d){var e=1<devicePixelRatio?devicePixelRatio:1;e!==this.pixelRatio&&(this.scaleChanged(),e=this.pixelRatio);var f=this.editor,g=f.filter;e*=f.scale;var k=this.element;f=this.leftMargin;var h=k.width,m=k.height,l=Math.round(8*e),p=Math.round(4*e),n=(h/l|0)+1,q=GraphicalFilterEditor.maximumChannelValueY*e|0,t=1>e?e:e|0;k=.5*t;d.fillStyle="#303030";d.fillRect(0,0,h,m);d.lineWidth=t;d.strokeStyle="#5a5a5a";d.beginPath();t=h+(p>>1);var u=(GraphicalFilterEditor.zeroChannelValueY*e|0)+k;d.moveTo(t,u);
for(let r=n-1;0<=r;r--)d.lineTo(t-p,u),t-=l,d.moveTo(t,u);d.stroke();d.beginPath();t=h+(p>>1);u=(GraphicalFilterEditor.validYRangeHeight*e|0)+k;d.moveTo(t,u);for(h=n-1;0<=h;h--)d.lineTo(t-p,u),t-=l,d.moveTo(t,u);d.stroke();if(a)for(a=g.equivalentZonesFrequencyCount.length-2;0<a;a--){t=((g.equivalentZonesFrequencyCount[a]+f)*e|0)+k;u=q;d.beginPath();for(d.moveTo(t,u);u<m;)d.lineTo(t,u+p),u+=l,d.moveTo(t,u);d.stroke()}t=1>e?2*e:2*e|0;k=.5*t;d.lineWidth=t;a=GraphicalFilterEditor.visibleBinCount-1;for(m=
b?1:0;0<=m;m--){l=m||!b?g.channelCurves[c]:g.actualChannelCurve;d.strokeStyle=m?"#707070":this.rangeImage;d.beginPath();d.moveTo(f*e|0,l[0]*e+k);for(t=1;t<a;t++)d.lineTo((f+t)*e|0,l[t]*e+k);d.lineTo((f+t+1)*e|0,l[t]*e+k);d.stroke()}}}}
...
d:2*d|0);const f=this.leftMargin,g=GraphicalFilterEditor.visibleBinCount;for(let k=b?1:0;0<=k;k--){const h=k||!b?a.channelCurves[c]:a.actualChannelCurve,m=k?this.svgGrayCurve:this.svgCurve;let l=1,p=0,n=h[0],q="M "+(f*d|0)+","+(n*d+e);for(;l<g;l++)h[l]!==n&&(l>p+1&&1<l&&(q+=" "+((f+l-1)*d|0)+","+(h[l-1]*d+e)),p=l,n=h[l],q+=" "+((f+p)*d|0)+","+(n*d+e));q+=" "+((f+l)*d|0)+","+(h[l-1]*d+e);m.setAttribute("d",q)}this.isActualChannelCurveNeeded!==b&&(this.isActualChannelCurveNeeded=b,this.svgGrayCurve.style.display=
b?"":"none")}}
...
"GEMNUSEP";return k};c=function(k){const h=document.createElement("div");h.className="GEMNULBL";h.appendChild(document.createTextNode(k));return h};d=function(k,h,m,l,p,n){const q=document.createElement("div");q.className=n?"GEMNUIT GECLK "+n:"GEMNUIT GECLK";if(h)if(f&&(l&&f.radioHTML||!l&&f.checkHTML))q.innerHTML=l?f.radioHTML:f.checkHTML,h=q.firstChild,h.style.marginRight=l?f.radioMargin||"2px":f.checkMargin||"2px",m||(h.style.visibility="hidden");else{h=document.createElement("span");n=l?"\u25cf ":
"\u25a0 ";let t=null;if(f){let u=!1;l?f.radioCharacter&&(u=!0,n=f.radioCharacter,t=f.radioMargin||"2px"):f.checkCharacter&&(u=!0,n=f.checkCharacter,t=f.checkMargin||"2px");u&&(f.checkFontFamily&&(h.style.fontFamily=f.checkFontFamily),f.checkFontSize&&(h.style.fontSize=f.checkFontSize))}t&&(h.style.marginRight=t);h.appendChild(document.createTextNode(n));m||(h.style.visibility="hidden");q.appendChild(h)}q.appendChild(document.createTextNode(k));p&&(q.onclick=p);return q};this.element=a;a.className=
...
d=1-d;d*=d;d+=.5*d;.55>d?d=0:1>d&&(d=smoothStep(.55,1,d));k=1-k;k=smoothStep(0,1,k);k*=k;k+=k;1<k&&(k=1);d+=.5*k;d=255*d|0;c[64*e+g]=255<=d?255:d}}a.texImage2D(a.TEXTURE_2D,0,a.ALPHA,64,64,0,a.ALPHA,a.UNSIGNED_BYTE,c)}analyze(a){let b=a-this.lastTime;33<b&&(b=33);this.lastTime=a;a=this.program.gl;var c=.00390625*b,d=1-c;const e=this.processedData,f=this.processedDataR,g=this.fft,k=this.BG_COLUMNS,h=this.BG_PARTICLES_BY_COLUMN,m=this.COLORS,l=this.program,p=this.bgPos,n=this.bgSpeedY,q=this.bgColor,
t=this.bgTheta,u=Math.max;let r,y=44,x=116;b*=.001;this.analyzerL.getByteFrequencyData(e);this.analyzerR.getByteFrequencyData(f);for(r=0;256>r;r++){let v=u(e[r],f[r]);8>v&&(v=0);const w=g[r];v<w&&(v=c*v+d*w);g[r]=v;e[r]=255<=v?255:v}a.clear(a.COLOR_BUFFER_BIT);r=2;for(let v=0,w=0;v<k;v++){if(6>r)c=.00390625*e[r],r++;else if(20>r)c=.005859375*u(e[r],e[r+1]),r+=2;else if(36>r)c=.005859375*u(e[r],e[r+1],e[r+2],e[r+3]),r+=4;else if(100>r){for(c=0;r<y;r++)c=u(c,e[r]);c*=.0078125;y+=8}else{for(c=0;r<x;r++)c=
...
b-e)&&e<f)for(let l=e,p=0;l<f;l++,p++)e=c[p],c[p]=c[l],c[l]=e}else if((e-=b)&&e<f)for(let l=f-1,p=l-e;0<=p;p--,l--)e=c[l],c[l]=c[p],c[p]=e;var g=a.list,k=g.length,h=a.list.currentItem;e=this.container;var m=0;for(let l=b*d;m<f&&b<k;m++,b++,l+=d){const p=g.item(b),n=c[m];if(n.item!==p){const q=h===p;n.current!==q&&((n.current=q)?n.element.classList.add("current"):n.element.classList.remove("current"));a.prepareElement(p,b,k,n.element);n.item||e.appendChild(n.element);n.item=p}n.index!==b&&(n.index=
...
(this.refreshVisibleItemsEnqueued=!0,queueMicrotask(this.boundRefreshVisibleItemsInternal))}notifyCleared(){this.clear()}notifyItemsAdded(a,b){if(this.useVirtualItems)this.refreshVisibleItems();else{const m=this.adapter,l=b-a+1;if(m&&!(0>=l)){var c=this.container,d=this.items,e=m.list,f=e.length,g=e.currentItem,k=Array(l);for(var h=b;h>=a;h--){const p=e.item(h),n=new ListControlItem(m.createEmptyElement("list-item real"));m.prepareElement(p,h,f,n.element);k[h-a]=n;g===p&&(this.currentItem&&(this.currentItem.current=
!1,this.currentItem.element.classList.remove("current")),this.currentItem=n,n.current=!0,n.element.classList.add("current"))}g=a>=d.length?null:d[a].element;a>=d.length?d.push(...k):d.splice(a,0,...k);if(this.items.length!==e.length)throw Error("Assertion error: this.items.length !== list.length");this.adjustContainerHeight();for(h=0;h<l;h++)c.insertBefore(k[h].element,g);for(c=d.length-1;c>b;c--)m.prepareElementIndexOrLengthChanged(e.item(c),c,f,d[c].element);for(--a;0<=a;a--)m.prepareElementIndexOrLengthChanged(e.item(a),
...
1!==AppUI.topMessageFading||(AppUI.topMessageFading=2,AppUI.topMessage.classList.add("in"),AppUI.topMessageTimeout=DelayControl.delayUICB(function(){2===AppUI.topMessageFading&&(AppUI.topMessageFading=0,AppUI.topMessageTimeout=0)}))}))}static async addFiles(a){if(App.player&&!AppUI._loading&&!Modal.visible&&!FilePicker.visible){var b=App.hostType===App.hostTypeElectron;if((a=FilePicker.isSupported()?await FilePicker.show():await App.showOpenDialogWeb(a))&&App.player)try{AppUI._loading=!0;App.updateLoadingIcon();
App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const c=App.player.playlist,d=b?null:new Uint8Array(BufferedFileHandle.minBufferLength<<1),e=b?null:[new Uint8Array(256)];let f=!1;for(let g=0;g<a.length&&App.player;g++)if(b)await c.addSong(a[g]);else{const k=c.length;await c.addSongWeb(a[g],d,e)&&k===c.length&&(f=!0)}f&&AppUI.playlistControl&&AppUI.playlistControl.refreshVisibleItems()}catch(c){Modal.show({text:"addFiles error: "+(c.message||c.toString())})}finally{AppUI._loading=
!1,App.updateLoadingIcon()}}}static toggleDeleteMode(a){AppUI.playlistControl&&(AppUI.playlistControl.deleteMode?(AppUI.playlistControl.deleteMode=!1,AppUI.hideTopMessage(),a&&HistoryHandler.popState()):(AppUI.playlistControl.deleteMode=!0,AppUI.showTopMessage(Strings.DeleteModeHTML,function(b){const c=document.createElement("div");ButtonControl.create({color:"red",icon:"icon-delete-all",text:Strings.DeleteAllSongs,parent:c,onclick:function(){App.player&&App.player.playlist&&App.player.playlist.clear();
...
"<br/><br/><small>Base latency: "+(App.player.audioContext.baseLatency||0).toFixed(4)+" s<br/>Output latency: "+(isNaN(App.player.audioContext.outputLatency)?"-":App.player.audioContext.outputLatency.toFixed(4)+" s")+"</small>":""),title:Strings.About+" (v"+window.CACHE_VERSION+")"})}}AppUI.primaryInputIsTouch="matchMedia"in window?window.matchMedia("(pointer: coarse)").matches:0<navigator.maxTouchPoints||0<navigator.msMaxTouchPoints;AppUI.rem=4;AppUI.buttonRegularSizeREM=8;
...
Strings.toFixed=GraphicalFilterEditorStrings.toFixed;if(!App.hostInterface&&FileSystemAPI.isSupported())try{await FileSystemAPI.init()}catch(d){Alert.show(d.message||d.toString())}const a=InternalStorage.loadAppSettings(),b=window.electronWebFrame;App.ipcRenderer&&delete window.electronIpcRenderer;b&&delete window.electronWebFrame;if("serviceWorker"in navigator&&!location.href.startsWith("file://")){window.addEventListener("beforeinstallprompt",App.beforeInstallPrompt);var c=navigator.serviceWorker.register("sw.js");
