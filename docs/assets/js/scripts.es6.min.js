...
Strings.DeleteModeHTML="Click songs to delete them.";class DelayControl{static delayShortCB(a){return setTimeout(a,DelayControl.shortDelayMS)}static delayFadeCB(a){return setTimeout(a,DelayControl.fadeDelayMS)}static delayShort(){return new Promise(function(a){setTimeout(a,DelayControl.shortDelayMS)})}static delayFade(){return new Promise(function(a){setTimeout(a,DelayControl.fadeDelayMS)})}}DelayControl.shortDelayMS=20;DelayControl.fadeDelayMS=320;
...
class FocusBlocker{constructor(){this.cover=this.parentContainer=this.oldBodyOverflow=this.changedElements=null;this.coverTimeout=0}block(a){this.unblock();const b=(a||document).querySelectorAll("button,input,select,textarea,.slider-control,.list"),c=[];for(let d=b.length-1;0<=d;d--){const e=b[d],f=e.getAttribute("tabindex");"-1"!==f&&(e.setAttribute("tabindex","-1"),c.push({element:e,tabindex:f}))}this.changedElements=c;a?(this.parentContainer=a,this.cover=document.createElement("div"),this.cover.className=
...
Alert.element.className="alert fade",Alert.element.onclick=Alert.hide,document.body.appendChild(Alert.element),DelayControl.delayShortCB(function(){Alert.show(a,b)}))}}Alert.shortDelayMS=2500;Alert.longDelayMS=4500;Alert.timeout=0;Alert.element=null;
...
class CheckboxControl{static init(){const a=document.querySelectorAll("input[type=checkbox]");if(a)for(let b=a.length-1;0<=b;b--)CheckboxControl.prepare(a[b])}static isChecked(a){return(a=a.childInput)?a.checked:!1}static setChecked(a,b){if(a=a.childInput)a.checked=!!b}static create(a){var b=document.createElement("input");b.setAttribute("type","checkbox");b.setAttribute("value",a.stringKey);a.square&&b.setAttribute("data-square","1");a.id&&b.setAttribute("id",a.id);let c="btn";a.color&&(c+=" "+a.color);
a.square&&(c+=" square");a.opaque||(c+=" transparent");a.className&&(c+=" "+a.className);b.className=c;b=CheckboxControl.prepare(b,a.icon,a.iconColor,a.checkbox0Color,a.checkbox1Color);!1===a.keyboardFocusable&&b.setAttribute("tabindex","-1");a.parent&&a.parent.appendChild(b);return b}static prepare(a,b,c,d,e){const f=document.createElement("button"),g=document.createElement("span"),h=a.parentNode,k=b?null:a.getAttribute("data-square"),m=Strings.translate(a.getAttribute("title")||a.value);f.setAttribute("type",
"button");f.setAttribute("role","checkbox");f.setAttribute("aria-label",m);g.setAttribute("tabindex","-1");a.setAttribute("tabindex","-1");a.setAttribute("aria-label",m);f.className=a.className;a.className="";b||(b=a.getAttribute("data-icon"));b&&!c&&(c=a.getAttribute("data-icon-color"));d||(d=a.getAttribute("data-checkbox0-color"));e||(e=a.getAttribute("data-checkbox1-color"))||(e="orange");h&&(h.insertBefore(f,a),h.removeChild(a));f.appendChild(g);g.appendChild(a);g.appendChild(Icon.createLarge("icon-checkbox-0",
(k?"":"margin")+(d?" "+d:"")));g.appendChild(Icon.createLarge("icon-checkbox-1",k?e:"margin "+e));b&&g.appendChild(Icon.createLarge(b,c?"margin "+c:"margin"));k||g.appendChild(document.createTextNode(m));a.removeAttribute("value");a.removeAttribute("title");f.onclick=CheckboxControl.click;f.childInput=a;return f}static click(a){(a=this.childInput)&&a.click()}}
...
this.boundAdjustScrollbarPaddingFromResize=this.adjustScrollbarPaddingFromResize.bind(this);this.element.addEventListener("focus",this.elementFocus.bind(this));this.element.addEventListener("keydown",this.elementKeyDown.bind(this));this.element.addEventListener("keyup",this.elementKeyUp.bind(this));this.element.addEventListener("blur",this.elementBlur.bind(this));b?this.element.addEventListener("scroll",this.elementScroll.bind(this),{passive:!0}):this.boundNotifyCurrentItemChangedInternal=this.notifyCurrentItemChangedInternal.bind(this);
this.resizeObserver=new ResizeObserver(this.resize.bind(this));this.resizeObserver.observe(this.element,{box:"border-box"});this.itemMargin=this.itemHeight=1;this.itemHeightAndMargin=2;this.lastIndex=this.firstIndex=0;this.currentItem=this.currentListItem=null;this.clientHeight=1;this.containerHeight=0;this.scrollbarPaddingScheduled=this.scrollbarPadding=!1;this.visibleCount=1;this.items=[];this.focused=this.notifyCurrentItemChangedEnqueued=this.refreshVisibleItemsEnqueued=!1;this.lastKeyboardIndex=
this.keyboardIndex=-1;this.deleteMode=!1;this.onitemcontextmenu=this.onitemcontrolclicked=this.onitemclicked=null;a.onclick=this.containerClick.bind(this);a.oncontextmenu=this.containerContextMenu.bind(this);this.resize();AppUI.addZoomHandler(this.boundZoomChanged)}destroy(){this._adapter&&(this._adapter.control=null);this.resizeObserver&&this.resizeObserver.unobserve(this.element);this.items&&this.items.fill(null);this.boundZoomChanged&&AppUI.removeZoomHandler(this.boundZoomChanged);zeroObject(this)}get adapter(){return this._adapter}set adapter(a){this._adapter!==
...
this.items;for(let b=a.length-1;0<=b;b--)a[b].index=-1;this.useVirtualItems&&this.elementScroll()}}adjustScrollbarPadding(){const a=(this.containerHeight|0)>this.clientHeight;this.scrollbarPadding!==a&&(this.scrollbarPadding=a,this.element.style.padding=this.scrollbarPadding?"":"0")}adjustScrollbarPaddingFromResize(){this.scrollbarPaddingScheduled=!1;this.adjustScrollbarPadding()}adjustContainerHeight(){var a=this._adapter;const b=this.container;a&&b&&(a=(a=a.list.length)?a*this.itemHeightAndMargin-
this.itemMargin:0,this.containerHeight!==a&&(this.containerHeight=a,b.style.height=a+"px",this.adjustScrollbarPadding()))}clear(){var a=this.container;if(a){for(;a.firstChild;)a.removeChild(a.firstChild);a=this.items;if(this.useVirtualItems)for(var b=a.length-1;0<=b;b--)a[b].reset();else{for(b=a.length-1;0<=b;b--)a[b].zero();a.splice(0)}this.lastIndex=this.firstIndex=0;this.currentItem=this.currentListItem=null;this.containerHeight=this.element.scrollTop=0;this.container.style.height="0";this.lastKeyboardIndex=
0<this.keyboardIndex?this.keyboardIndex=0:-1;this.adjustScrollbarPadding()}}elementScroll(){var a=this._adapter,b=this.element;if(a&&b){var c=this.items,d=this.itemHeightAndMargin,e=this.firstIndex-2;b=b.scrollTop/d|0;0>e&&(e=0);this.firstIndex=b;var f=this.visibleCount+(0===b?2:1===b?3:4);f>c.length&&(f=c.length);0>(b-=2)&&(b=0);if(b>e){if((e=b-e)&&e<f)for(let p=e,q=0;p<f;p++,q++)e=c[q],c[q]=c[p],c[p]=e}else if((e-=b)&&e<f)for(let p=f-1,q=p-e;0<=q;q--,p--)e=c[p],c[p]=c[q],c[q]=e;var g=a.list,h=g.length,
k=a.list.currentItem,m=this.keyboardIndex;e=this.container;var l=0;for(let p=b*d;l<f&&b<h;l++,b++,p+=d){const q=g.item(b),n=c[l];if(n.item!==q){let r=k===q;n.current!==r&&((n.current=r)?n.element.classList.add("current"):n.element.classList.remove("current"));r=m===b;n.keyboard!==r&&((n.keyboard=r)?n.element.classList.add("keyboard"):n.element.classList.remove("keyboard"));a.prepareElement(q,b,h,n.element);n.item||e.appendChild(n.element);n.item=q}n.index!==b&&(n.index=b,n.element.style.transform=
"translateY("+p+"px)")}for(a=c.length;l<a;l++)d=c[l],d.item&&(d.item=null,e.removeChild(d.element));this.lastIndex=b-1;this.lastIndex<this.firstIndex&&(this.lastIndex=this.firstIndex)}}refreshVisibleItemsInternal(){this.refreshVisibleItemsEnqueued=!1;var a=this._adapter,b=this.element;if(a&&b){this.adjustContainerHeight();b=this.items;var c=this.itemHeightAndMargin,d=a.list,e=d.length,f=a.list.currentItem,g=this.keyboardIndex,h=this.container;if(this.useVirtualItems){this.currentListItem=f;var k=
0,m=this.firstIndex,l=this.visibleCount+(0===m?2:1===m?3:4);l>b.length&&(l=b.length);0>(m-=2)&&(m=0);for(let p=m*c;k<l&&m<e;k++,m++,p+=c){const q=d.item(m),n=b[k];let r=f===q;n.current!==r&&((n.current=r)?n.element.classList.add("current"):n.element.classList.remove("current"));r=g===m;n.keyboard!==r&&((n.keyboard=r)?n.element.classList.add("keyboard"):n.element.classList.remove("keyboard"));a.prepareElement(q,m,e,n.element);n.item||h.appendChild(n.element);n.item=q;n.index!==m&&(n.index=m,n.element.style.transform=
"translateY("+p+"px)")}for(a=b.length;k<a;k++)d=b[k],d.item&&(d.item=null,h.removeChild(d.element))}else{h=this.visibleCount;for(let p=0,q=this.indexFromY(0);p<=h&&q<e;p++,q++)a.prepareElement(d.item(q),q,e,b[q].element)}}}resize(){const a=this._adapter;var b=this.element;if(a&&b){var c=this.clientHeight;b=b.clientHeight;0>b&&(b=0);var d=Math.ceil(b/this.itemHeightAndMargin)+1;if(this.visibleCount!==d&&(this.visibleCount=d,this.useVirtualItems)){const e=this.items;for(;d+4>e.length;)e.push(new ListControlItem(a.createEmptyElement("list-item virtual")))}c!==
b&&(this.clientHeight=b,this.useVirtualItems&&this.elementScroll(),this.scrollbarPaddingScheduled||(this.scrollbarPaddingScheduled=!0,requestAnimationFrame(this.boundAdjustScrollbarPaddingFromResize)))}}containerClick(a){if(this.element&&this.adapter&&a.target){var b=this.element.getBoundingClientRect();b=this.indexFromY(a.clientY-b.top);0<=this.keyboardIndex&&(this.keyboardIndex=-1,this.refreshVisibleItems());if(0<=b){this.lastKeyboardIndex=b;const c=this.adapter.list.item(b);if(c)if(a.target.classList.contains("list-item"))if(this.deleteMode)this.adapter.list.removeItems(b,
b);else{if(this.onitemclicked)this.onitemclicked(c,b,a.button)}else if(this.onitemcontrolclicked)this.onitemcontrolclicked(c,b,a.button,a.target)}}}containerContextMenu(a){if(!(this.element&&this.adapter&&this.onitemcontextmenu&&a.target))return cancelEvent(a);var b=this.element.getBoundingClientRect();b=this.indexFromY(a.clientY-b.top);0<=this.keyboardIndex&&(this.keyboardIndex=-1,this.refreshVisibleItems());if(0<=b){this.lastKeyboardIndex=b;const c=this.adapter.list.item(b);if(c)this.onitemcontextmenu(c,
b)}return cancelEvent(a)}elementFocus(){this.focused=!0}elementKeyDown(a){if(!(!this.adapter||a.ctrlKey||a.metaKey||a.shiftKey||a.altKey)){var b=0,c=0;switch(a.key){case "ArrowDown":case "ArrowRight":b=1;break;case "ArrowUp":case "ArrowLeft":b=-1;break;case "PageDown":b=this.clientHeight/this.itemHeightAndMargin|0;break;case "PageUp":b=-(this.clientHeight/this.itemHeightAndMargin|0);break;case "Home":c=-1;break;case "End":c=1;break;case "Enter":a.repeat||(c=2);break;case " ":a.repeat||(c=3);break;
default:return}var d=this.adapter.list.length;if(0<d){let e=this.keyboardIndex;0>e&&(e=this.lastKeyboardIndex);this.isItemVisible(e)||(e=this.isItemVisible(this.adapter.list.currentIndex)?this.adapter.list.currentIndex:this.indexFromY(this.clientHeight>>1));e=Math.min(d-1,Math.max(0,e));if(-1===c)e=0;else if(1===c)e=d-1;else{const f=e;e+=b;0>e?e=f?0:d-1:e>=d&&(e=f===d-1?0:d-1)}this.keyboardIndex=e;this.bringItemIntoView(e);this.refreshVisibleItems();if(1<c&&(b=this.adapter.list.item(e)))switch(c){case 2:if(this.deleteMode)this.adapter.list.removeItems(e,
e);else if(this.onitemclicked)this.onitemclicked(b,e,0);break;case 3:if(this.deleteMode)this.adapter.list.removeItems(e,e);else if(this.onitemcontrolclicked&&(c=this.listControlItemFromIndex(e)))this.onitemcontrolclicked(b,e,0,c.element)}}else this.keyboardIndex=-1;a.preventDefault();return!1}}elementKeyUp(a){this.adapter&&!a.ctrlKey&&!a.metaKey&&!a.altKey&&"Tab"===a.key&&this.focused&&(a=this.adapter.list.length)&&(a=Math.min(a-1,Math.max(0,this.lastKeyboardIndex)),this.isItemVisible(a)?this.keyboardIndex=
a:(this.keyboardIndex=a=this.isItemVisible(this.adapter.list.currentIndex)?this.adapter.list.currentIndex:this.indexFromY(this.clientHeight>>1),this.bringItemIntoView(a)),this.refreshVisibleItems())}elementBlur(){this.focused=!1;this.lastKeyboardIndex=this.keyboardIndex;0<=this.keyboardIndex&&(this.keyboardIndex=-1,this.refreshVisibleItems())}listControlItemFromIndex(a){if(!this.adapter||!this.items)return null;const b=this.items;for(let c=b.length-1;0<=c;c--)if(b[c].index===a&&b[c].item&&b[c].element)return b[c];
return null}yFromIndex(a){return this.element?a*this.itemHeightAndMargin-this.element.scrollTop:0}indexFromY(a){if(!this.element||!this.adapter)return-1;a=(a+this.element.scrollTop+.9)/this.itemHeightAndMargin|0;return 0>a||a>=this.adapter.list.length?-1:a}isItemVisible(a){a=this.yFromIndex(a);return a>-this.itemHeight&&a<this.clientHeight}bringItemIntoView(a,b){const c=this.yFromIndex(a);0>c?b?this.centerItemIntoView(a):this.element.scrollTop+=c:c>this.clientHeight-this.itemHeight&&(b?this.centerItemIntoView(a):
this.element.scrollTop+=c-(this.clientHeight-this.itemHeight))}centerItemIntoView(a){this.element.scrollTop=a*this.itemHeightAndMargin-(this.clientHeight-this.itemHeight>>1)}refreshVisibleItems(){this.refreshVisibleItemsEnqueued||(this.refreshVisibleItemsEnqueued=!0,queueMicrotask(this.boundRefreshVisibleItemsInternal))}notifyCleared(){this.clear()}notifyItemsAdded(a,b){if(this.useVirtualItems)this.refreshVisibleItems();else{const m=this.adapter,l=b-a+1;if(m&&!(0>=l)){var c=this.container,d=this.items,
e=m.list,f=e.length,g=e.currentItem,h=Array(l);for(var k=b;k>=a;k--){const p=e.item(k),q=new ListControlItem(m.createEmptyElement("list-item real"));m.prepareElement(p,k,f,q.element);h[k-a]=q;g===p&&(this.currentItem&&(this.currentItem.current=!1,this.currentItem.element.classList.remove("current")),this.currentItem=q,q.current=!0,q.element.classList.add("current"))}g=a>=d.length?null:d[a].element;a>=d.length?d.push(...h):d.splice(a,0,...h);if(this.items.length!==e.length)throw Error("Assertion error: this.items.length !== list.length");
...
0;g<f.length;g++)e.appendChild(f[g])}else e.appendChild(a.html);if((f=e.getElementsByTagName("button"))&&f.length)for(g=f.length-1;0<=g;g--){const h=f[g];ButtonControl.prepare(h);h.onclick=()=>{if(!this.fading&&this.options.onbuttonclick)this.options.onbuttonclick(h.id,h)}}this.modalFooterElement=f=document.createElement("div");f.className="modal-footer padding left-margin right-margin"+(a.buttons&&1===a.buttons.length?" right":"");this.defaultSubmitButton=this.defaultCancelButton=null;if(a.buttons)for(g=
0;g<a.buttons.length;g++){const h=a.buttons[g],k=ButtonControl.create(h,!0);h.defaultCancel&&(this.defaultCancelButton=k);h.defaultSubmit&&(this.defaultSubmitButton=k);g&&(k.style.float="right");k.onclick=()=>{if(!this.fading){if(h.onclick)h.onclick(h.id,k);if(this.options.onbuttonclick)this.options.onbuttonclick(h.id,k)}};f.appendChild(k)}c.appendChild(d);if(a.skipBody)for(;e.firstChild;)d=e.firstChild,e.removeChild(d),c.appendChild(d);else c.appendChild(e);c.appendChild(f);b.appendChild(c);document.body.appendChild(b);
this.boundDocumentKeyDown=this.documentKeyDown.bind(this);document.addEventListener("keydown",this.boundDocumentKeyDown,!0);this.fading=!0;DelayControl.delayShortCB(()=>{if(a.onshowing)a.onshowing();this.containerElement.classList.add("in");DelayControl.delayFadeCB(()=>{this.fading=!1;if(this.options.onshown)this.options.onshown()})})}static get visible(){return!!Modal.modal}static get currentModalElement(){return Modal.modal?Modal.modal.modalElement:null}static get currentModalHeaderElement(){return Modal.modal?
...
this.options.onhiding&&!1===this.options.onhiding()||(document.removeEventListener("keydown",this.boundDocumentKeyDown,!0),this.fading=!0,this.containerElement.classList.remove("in"),DelayControl.delayFadeCB(()=>{Modal.modal=null;if(this.options.onhidden)this.options.onhidden();document.body.removeChild(this.containerElement);HistoryHandler.popState();this.focusBlocker&&this.focusBlocker.unblock();if(this.options.buttons)for(let a=this.options.buttons.length-1;0<=a;a--)zeroObject(this.options.buttons[a]);
...
e.className="icon loading large";c.appendChild(e);const f=document.createElement("div");f.className="file-picker-path-element small-left-padding";c.appendChild(f);const g=document.createElement("div");g.className="file-picker-list fade bottom-margin";this.resolve=a;this.provider=App.hostType===App.hostTypeAndroid?new FilePickerAndroidProvider:new FilePickerFileSystemAPIProvider;this.list=new List;this.listAdapter=new FilePickerListAdapter(this.list);this.listControl=new ListControl(g,!0);this.listControl.element.addEventListener("keydown",
this.listKeyDown.bind(this));this.listControl.onitemclicked=this.itemClicked.bind(this);this.listControl.onitemcontrolclicked=this.itemControlClicked.bind(this);this.listControl.adapter=this.listAdapter;this.selectedFiles=null;this.fading=this.gettingFiles=!1;this.providerVersion=0;this.path=FilePicker.lastPath;this.rootLength=FilePicker.lastRootLength;this.iconFolder=d;this.iconLoading=e;this.pathElement=f;this.boundRefreshRoot=this.refreshRoot.bind(this);this.updatePathElement(this.path);if(!Modal.show({html:[c,
g],title:b,rightHeader:!0,leftBody:!0,skipBody:!0,fullHeight:!0,okCancel:!0,onshown:this.modalShown.bind(this),onhiding:this.modalHiding.bind(this),onhidden:this.modalHidden.bind(this),onok:this.modalOk.bind(this)}))throw Error("Assertion error: Modal.show() === false");}static isSupported(){return!!App.hostInterface||FileSystemAPI.isSupported()}static get visible(){return!!FilePicker.filePicker}static show(){return FilePicker.filePicker||Modal.visible?Promise.resolve(null):new Promise(function(a){FilePicker.filePicker=
new FilePicker(a)})}updatePathElement(a){a&&a.endsWith("/")&&(a=a.substring(0,a.length-1));Strings.changeText(this.pathElement,a||Strings.Storage);return a}updateIconLoading(a){this.iconFolder&&this.iconLoading&&(a?(this.iconFolder.classList.add("hidden"),this.iconLoading.classList.remove("hidden")):(this.iconLoading.classList.add("hidden"),this.iconFolder.classList.remove("hidden")))}listKeyDown(a){a.ctrlKey||a.metaKey||a.shiftKey||a.altKey||a.repeat||"Backspace"!==a.key||this.navigateUp()}itemClicked(a,
b,c){this.gettingFiles||this.fading||(a.root&&!a.path&&this.provider.editableRoots()?(this.updateIconLoading(!0),this.provider.addNewRoot().then(this.boundRefreshRoot,this.boundRefreshRoot)):a.dir?this.navigate(a.path):this.listControl&&(a.selected=!a.selected,this.listControl.refreshVisibleItems()))}itemControlClicked(a,b,c,d){if(!this.gettingFiles&&!this.fading){if("SPAN"===d.tagName&&(d=d.parentNode,!d))return;if(d.classList.contains("list-item"))for(b=d.childNodes,c=0;c<b.length;c++)if(b[c]&&
...
AppUI.toggleView(!1);return!0}AppUI.toggleDeleteMode(!1);return!0}static preInit(a,b){AppUI.webFrame=a;window.addEventListener("keydown",AppUI.globalKeyHandler,!0);a&&b.devicePixelRatio&&b.devicePixelRatio!==devicePixelRatio&&a.setZoomFactor(b.devicePixelRatio);FilePicker.isSupported()&&(FilePicker.lastPath=b.filePickerLastPath||null,FilePicker.lastRootLength=b.filePickerRootLength||0,a=document.getElementById("add-folder-button"),a.parentNode.removeChild(a));ButtonControl.init();CheckboxControl.init();
...
window.addEventListener("resize",AppUI.adjustWindow,{passive:!0});AppUI.adjustWindow()}static init(a){if(App.player){AppUI.volumeSlider.value=App.player.volume;App.player.onsongchanged=AppUI.playerSongChanged;App.player.onloadingchanged=AppUI.playerLoadingChanged;App.player.onpausedchanged=AppUI.playerPausedChanged;App.player.oncurrenttimeschanged=AppUI.playerCurrentTimeSChanged;App.player.onerror=AppUI.playerError;AppUI.preparePlaylist();AppUI.playerSongChanged(App.player.currentSong);AppUI.centerCurrentSongIntoView();
...
a&&a.artist||Formatter.none),App.player.playlist&&0<=App.player.playlist.currentIndex&&!AppUI.playlistControl.deleteMode&&a&&AppUI.playlistControl.bringItemIntoView(App.player.playlist.currentIndex),AppUI.currentTimeS=0,a?(Strings.changeText(AppUI.currentTimeLabel,Formatter.zero),Strings.changeText(AppUI.songLengthLabel,a.length),AppUI.seekSlider&&(AppUI.seekSlider.value=0,0<a.lengthMS?(AppUI.seekSlider.disabled=!1,AppUI.seekSlider.max=a.lengthMS):AppUI.seekSlider.disabled=!0)):(Strings.changeText(AppUI.currentTimeLabel,
...
AppUI.topMessage.innerHTML=a,b&&b(AppUI.topMessage),AppUI.topMessage.style.visibility="visible",AppUI.topMessageTimeout=DelayControl.delayShortCB(function(){0>=AppUI.topMessageFading||1!==AppUI.topMessageFading||(AppUI.topMessageFading=2,AppUI.topMessage.classList.add("in"),AppUI.topMessageTimeout=DelayControl.delayFadeCB(function(){2===AppUI.topMessageFading&&(AppUI.topMessageFading=0,AppUI.topMessageTimeout=0)}))}))}static addFiles(a){return __awaiter(this,void 0,void 0,function*(){if(App.player&&
!AppUI._loading&&!Modal.visible&&!FilePicker.visible){var b=App.hostType===App.hostTypeElectron,c=FilePicker.isSupported()?yield FilePicker.show():yield App.showOpenDialogWeb(a);if(c&&App.player)try{AppUI._loading=!0;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const d=App.player.playlist,e=b?null:new Uint8Array(BufferedFileHandle.minBufferLength<<1),f=b?null:[new Uint8Array(256)];let g=!1;for(let h=0;h<c.length&&App.player;h++)if(b)yield d.addSong(c[h]);
else{const k=d.length;(yield d.addSongWeb(c[h],e,f))&&k===d.length&&(g=!0)}g&&AppUI.playlistControl&&AppUI.playlistControl.refreshVisibleItems()}catch(d){Modal.show({text:"addFiles error: "+(d.message||d.toString())})}finally{AppUI._loading=!1,App.updateLoadingIcon()}}})}static toggleDeleteMode(a){if(AppUI.playlistControl)if(AppUI.focusBlocker&&(AppUI.focusBlocker.unblock(),AppUI.focusBlocker=null),AppUI.playlistControl.deleteMode)AppUI.playlistControl.deleteMode=!1,AppUI.hideTopMessage(),a&&HistoryHandler.popState();
else{AppUI.playlistControl.deleteMode=!0;AppUI.focusBlocker=new FocusBlocker;AppUI.focusBlocker.block(AppUI.middlePanel);AppUI.showTopMessage(Strings.DeleteModeHTML,function(b){const c=document.createElement("div");ButtonControl.create({color:"red",icon:"icon-delete-all",text:Strings.DeleteAllSongs,parent:c,onclick:function(){App.player&&App.player.playlist&&App.player.playlist.clear();AppUI.playlistControl&&AppUI.playlistControl.deleteMode&&AppUI.toggleDeleteMode(!0)}});ButtonControl.create({color:"green",
icon:"icon-check",text:Strings.Done,parent:c,onclick:function(){AppUI.playlistControl&&AppUI.playlistControl.deleteMode&&AppUI.toggleDeleteMode(!0)}});b.appendChild(c)});HistoryHandler.pushState();try{AppUI.playlistControl.element.focus()}catch(b){}}}static checkPanelContainerToggleState(){875>=window.innerWidth||!AppUI.fixedPanel||!AppUI.optionalPanel||(AppUI.panelContainerToggling&&(AppUI.panelContainerToggleVersion++,AppUI.panelContainerToggling=!1,AppUI.fixedPanel.classList.remove("fade"),AppUI.fixedPanel.classList.remove("in"),
AppUI.optionalPanel.classList.remove("fade"),AppUI.optionalPanel.classList.remove("in")),AppUI.panelContainerToggled&&(AppUI.panelContainerToggled=!1,AppUI.panelContainer.classList.remove("toggled"),HistoryHandler.popState()))}static toggleView(a){if(AppUI.panelContainer){AppUI.panelContainerToggleVersion++;const b=AppUI.panelContainerToggleVersion,c=function(d,e,f){AppUI.panelContainerToggling=!0;e.classList.remove("in");e.classList.add("fade");d.classList.add("in");d.classList.add("fade");DelayControl.delayShortCB(function(){return __awaiter(this,
void 0,void 0,function*(){AppUI.panelContainerToggleVersion===b&&(d.classList.remove("in"),yield DelayControl.delayFade(),AppUI.panelContainerToggleVersion===b&&(f?AppUI.panelContainer.classList.add("toggled"):AppUI.panelContainer.classList.remove("toggled"),d.classList.remove("fade"),yield DelayControl.delayShort(),AppUI.panelContainerToggleVersion===b&&(e.classList.add("in"),yield DelayControl.delayFade(),AppUI.panelContainerToggleVersion===b&&(e.classList.remove("fade"),e.classList.remove("in"),
AppUI.panelContainerToggling=!1))))})})};AppUI.panelContainerToggled?(AppUI.panelContainerToggled=!1,c(AppUI.optionalPanel,AppUI.fixedPanel,!1),a&&HistoryHandler.popState()):(AppUI.panelContainerToggled=!0,c(AppUI.fixedPanel,AppUI.optionalPanel,!0),HistoryHandler.pushState())}}static showAbout(){Modal.show({html:Strings.AboutHTML+(App.player&&App.player.audioContext?"<br/><br/><small>Base latency: "+(App.player.audioContext.baseLatency||0).toFixed(4)+" s<br/>Output latency: "+(isNaN(App.player.audioContext.outputLatency)?
"-":App.player.audioContext.outputLatency.toFixed(4)+" s")+"</small>":""),title:Strings.About+" (v"+window.CACHE_VERSION+")"})}}AppUI.primaryInputIsTouch="matchMedia"in window?window.matchMedia("(pointer: coarse)").matches:0<navigator.maxTouchPoints||0<navigator.msMaxTouchPoints;AppUI.baseSizePX=4;AppUI.s1PX=AppUI.baseSizePX;AppUI.s2PX=2*AppUI.baseSizePX;AppUI.s3PX=3*AppUI.baseSizePX;AppUI.s4PX=4*AppUI.baseSizePX;AppUI.s5PX=5*AppUI.baseSizePX;AppUI.s6PX=6*AppUI.baseSizePX;AppUI.s7PX=7*AppUI.baseSizePX;
AppUI.s8PX=8*AppUI.baseSizePX;AppUI.smallFontSizeREM=3;AppUI.smallContentsSizeREM=4;AppUI.fontSizeREM=3.5;AppUI.contentsSizeREM=6;AppUI.baseThinBorderPX=1;AppUI.baseThickBorderPX=2;AppUI.zoomHandlers=[];AppUI.rootVariables=document.getElementById("root-variables");AppUI.panelContainer=document.getElementById("panel-container");AppUI.fixedPanel=document.getElementById("fixed-panel");AppUI.middlePanel=document.getElementById("middle-panel");AppUI.optionalPanel=document.getElementById("optional-panel");
AppUI.iconPlay=document.getElementById("i-play");AppUI.iconPause=document.getElementById("i-pause");AppUI.ruler=document.getElementById("ruler");AppUI.titleLabel=document.getElementById("title-label");AppUI.artistLabel=document.getElementById("artist-label");AppUI.topMessage=document.getElementById("top-message");AppUI._loading=!1;AppUI.devicePixelRatio=-1;AppUI._1remInPX=1;AppUI._fontScale=1;AppUI._smallIconSizePX=Icon.baseSizePX;AppUI._largeIconSizePX=Icon.baseSizePX<<1;AppUI._thinBorderPX=AppUI.baseThinBorderPX;
AppUI._thickBorderPX=AppUI.baseThickBorderPX;AppUI._buttonSizePX=1;AppUI._contentsSizePX=1;AppUI._playlistItemSizePX=1;AppUI.directoryCancelled=!1;AppUI.topMessageFading=0;AppUI.topMessageTimeout=0;AppUI.focusBlocker=null;AppUI.panelContainerToggled=!1;AppUI.panelContainerToggling=!1;AppUI.panelContainerToggleVersion=0;
...
document.getElementById("cover");if(a){const b=document.body.getBoundingClientRect();a.style.transform=`scale(${Math.ceil(.5*b.right)}, ${Math.ceil(.5*b.bottom)})`}}static sortFiles(a){if(a.length){const b=Strings.comparer;a[0]["data-path"]?a.sort(function(c,d){return b(c["data-path"],d["data-path"])}):a[0].webkitRelativePath?a.sort(function(c,d){return b(c.webkitRelativePath,d.webkitRelativePath)}):a.sort(function(c,d){return b(c.name,d.name)})}return a}static preInit(){return __awaiter(this,void 0,
void 0,function*(){Strings.init();Icon.init();GraphicalFilterEditorStrings.init(Strings.language);Strings.toFixed=GraphicalFilterEditorStrings.toFixed;if(!App.hostInterface&&FileSystemAPI.isSupported())try{yield FileSystemAPI.init()}catch(c){Alert.show(c.message||c.toString())}const a=InternalStorage.loadAppSettings(),b=window.electronWebFrame;App.ipcRenderer&&delete window.electronIpcRenderer;b&&delete window.electronWebFrame;if("serviceWorker"in navigator&&!location.href.startsWith("file://")){window.addEventListener("beforeinstallprompt",
App.beforeInstallPrompt);const c=navigator.serviceWorker.register("sw.js");c&&"then"in c&&c.then(function(d){d&&"onupdatefound"in d&&d.active&&(d.onupdatefound=function(){Modal.show({html:Strings.PleaseRefresh,title:Strings.UpdateAvailable,okText:Strings.Refresh,okCancel:!0,onok:function(){window.location.reload()}})})})}window.addEventListener("resize",App.adjustCover);App.adjustCover();App.ipcRenderer?(App.ipcRenderer.on("mainWindowStateChanged",App.mainWindowStateChanged),App.ipcRenderer.on("mainWindowClosing",
App.mainWindowClosing),App.triggerMainWindowStateChanged()):App.hostInterface||window.addEventListener("onpagehide"in window?"pagehide":"unload",App.mainWindowClosing);App.frameless&&(App.titleBar=document.createElement("h1"),App.titleBar.innerHTML=`
				<div></div>
				<i id="icon-main"></i>
				FPlay
...
MonoDownMixerControl.isSupported()&&!!a.monoDownMixerControlEnabled;return App.monoDownMixerControl});var b=InternalStorage.loadPlaylistWeb();b&&(App.player.playlist=b);AppUI.init(a);(b=document.getElementById("cover"))&&b.classList.remove("in");DelayControl.delayShortCB(function(){AppUI.centerCurrentSongIntoView();DelayControl.delayFadeCB(function(){window.removeEventListener("resize",App.adjustCover);const c=document.getElementById("cover");c&&document.body.removeChild(c)})});App.loading=!1}}static triggerMainWindowStateChanged(){App.ipcRenderer&&
