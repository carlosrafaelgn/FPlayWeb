...
null;g&&(f=JSON.parse(g))}catch(h){}if(f&&f.length===e.length)for(g=e.length-1;0<=g;g--)e[g]=App.fileInput.files[g],e[g]["data-path"]=f[g];else for(f=e.length-1;0<=f;f--)e[f]=App.fileInput.files[f];App.fileInput.value="";d(App.sortFiles(e))}else d(null)}},c.appendChild(App.fileInput),document.getElementById("top-panel").appendChild(c));AppUI.preInit(b,a)}static init(){return __awaiter(this,void 0,void 0,function*(){if(!App.player){var a=InternalStorage.loadAppSettings();App.player=new Player(a.playerVolume,
function(c){App.graphicalFilterControl=new GraphicalFilterControl(document.getElementById("filter-container"),document.getElementById("optional-panel-container"),c,a.graphicalFilterControlSimpleMode);App.graphicalFilterControl.enabled=!!a.graphicalFilterControlEnabled;return App.graphicalFilterControl},function(c){App.stereoPannerControl=new StereoPannerControl(document.getElementById("stereo-panner-slider"),c);App.stereoPannerControl.enabled=!!a.stereoPannerControlEnabled;App.stereoPannerControl.onappliedgainchanged=
function(d){App.monoDownMixerControl&&(App.monoDownMixerControl.multiplier=1/(1+d))};return App.stereoPannerControl},function(c){App.monoDownMixerControl=new MonoDownMixerControl(c);App.monoDownMixerControl.enabled=MonoDownMixerControl.isSupported()&&!!a.monoDownMixerControlEnabled;return App.monoDownMixerControl});try{var b=App.insideNativeHost?yield InternalStorage.loadPlaylist():InternalStorage.loadPlaylistWeb();if(!App.player||!App.player.alive)return;b&&(App.player.playlist=b)}catch(c){}App.player&&
App.player.alive&&(AppUI.init(a),(b=document.getElementById("cover"))&&b.classList.remove("in"),setTimeout(function(){window.removeEventListener("resize",App.adjustCover);const c=document.getElementById("cover");c&&document.body.removeChild(c)},320),App.loading=!1)}})}static triggerMainWindowStateChanged(){App.ipcRenderer&&App.ipcRenderer.invoke("triggerMainWindowStateChanged")}static mainWindowMinimize(){App.ipcRenderer&&App.ipcRenderer.invoke("mainWindowMinimize")}static mainWindowRestoreOrMaximize(){App.ipcRenderer&&
App.ipcRenderer.invoke(App.isMaximized?"mainWindowRestore":"mainWindowMaximize")}static mainWindowClose(){App.ipcRenderer&&App.ipcRenderer.invoke("mainWindowClose")}static showOpenFileDialog(){return __awaiter(this,void 0,void 0,function*(){if(App.fileInput||App._showingDialog)return null;try{return App._showingDialog=!0,yield App.ipcRenderer.invoke("showOpenFileDialog")}finally{App._showingDialog=!1}})}static convertFilePathsToAppFiles(a){return App.ipcRenderer.invoke("convertFilePathsToAppFiles",
a)}static showOpenDirectoryDialog(){return __awaiter(this,void 0,void 0,function*(){if(App._showingDialog)return null;try{return App._showingDialog=!0,yield App.ipcRenderer.invoke("showOpenDirectoryDialog")}finally{App._showingDialog=!1}})}static listDirectory(a){return App.ipcRenderer.invoke("listDirectory",a)}static extractMetadata(a,b){return App.ipcRenderer.invoke("extractMetadata",a,b)}static showOpenDialogWeb(a){if(!App.fileInput)return Promise.resolve(null);App.fileInputPromiseResolve&&(App.fileInputPromiseResolve(null),
