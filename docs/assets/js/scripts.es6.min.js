...
class FileUtils{static isTypeSupported(a){const b=a.lastIndexOf(".");if(0>b)return!1;a=a.substring(b);return FileUtils.supportedExtensions[a]||FileUtils.supportedExtensions[a.toLowerCase()]||!1}static pathToURL(a){return!a||a.startsWith(FileUtils.httpURLPrefix)||a.startsWith(FileUtils.fileURLPrefix)||a.startsWith(FileUtils.localURLPrefix)?a:encodeURI(47===a.charCodeAt(0)?FileUtils.fileURLPrefix+a:FileUtils.fileURLPrefixWindows+a.replace(FileUtils.regExpSepWindows,"/"))}static nameFromLinuxPath(a){if(!a)return a;
let b=a.lastIndexOf("/");return 0<=b?a.substring(b+1):a}}FileUtils.separator="\u0004";FileUtils.httpURLPrefix="http";FileUtils.fileURLPrefix="file://";FileUtils.localURLPrefix="local://";FileUtils.fileURLPrefixWindows="file:///";FileUtils.regExpSepWindows=/\\/g;FileUtils.supportedExtensions={".aac":!0,".mp3":!0,".wav":!0};FileUtils.concatenatedSupportedExtensions=function(){let a="";for(let b in FileUtils.supportedExtensions)a=a?a+","+b:b;return a}();
...
93+e))))}c&MetadataExtractor.TRACK_B||d[125]||!d[126]||(a.track=d[126]&255)}}catch(g){}})}static extractID3v2Andv1(a,b,c){return __awaiter(this,void 0,void 0,function*(){const d={url:FileUtils.pathToURL(a["data-path"])||""};d.url?d.url.startsWith(FileUtils.localURLPrefix)&&(d.file=a):(d.file=a,d.fileSize=a.size,d.fileName=a.name);var e=b.readByte()<<24|b.readByte()<<16|b.readByte()<<8;if(1229206272!==e)if(e|=b.readByte(),1380533830===e){e=yield MetadataExtractor.extractRIFF(d,b,c[0]);if(!e)return null;
...
class FilePickerAndroidProvider{constructor(){this.enumerationVersion=this.version=0}static callbackPermission(a){FilePickerAndroidProvider.permissionGranted=!!a;FilePickerAndroidProvider.callbackPermissionResolve&&(FilePickerAndroidProvider.callbackPermissionResolve(FilePickerAndroidProvider.permissionGranted),FilePickerAndroidProvider.callbackPermissionResolve=null)}static checkPermission(){FilePickerAndroidProvider.callbackPermissionResolve&&(FilePickerAndroidProvider.callbackPermissionResolve(!1),
FilePickerAndroidProvider.callbackPermissionResolve=null);return App.hostInterface?App.hostInterface.checkFilePermission()?(FilePickerAndroidProvider.permissionGranted=!0,Promise.resolve(!0)):new Promise(function(a){App.hostInterface?(FilePickerAndroidProvider.callbackPermissionResolve=a,App.hostInterface.requestFilePermission()):a(!1)}):Promise.resolve(!1)}editableRoots(){return!1}addNewRoot(){return Promise.resolve(!1)}removeRoot(a){return Promise.resolve(!1)}enumerate(a){this.enumerationVersion++;
FilePickerAndroidProvider.callback&&(FilePickerAndroidProvider.callback(0,null),FilePickerAndroidProvider.callback=null);return new Promise(b=>{if(!App.hostInterface)return null;FilePickerAndroidProvider.callback=(c,d)=>{c!==this.enumerationVersion?b(null):(FilePickerAndroidProvider.callback=null,b(d))};App.hostInterface.enumerateFiles(this.enumerationVersion,a)})}getDirectoryFiles(a,b,c){return __awaiter(this,void 0,void 0,function*(){const d=yield this.enumerate(c);if(d&&a===this.version){var e=
Strings.comparer;d.sort(function(f,g){const h=47===f.charCodeAt(0),k=47===g.charCodeAt(0);return h===k?e(f,g):h?1:-1});for(let f=0;f<d.length&&a===this.version;f++){const g=d[f];47===g.charCodeAt(0)?b.push(g):yield this.getDirectoryFiles(a,b,g)}}})}getFiles(a,b){return __awaiter(this,void 0,void 0,function*(){this.cancel();const c=[];var d=this.version,e=App.showOpenDialogWeb(!1,!0);if(a)for(var f=0;f<a.length&&d===this.version;f++)yield this.getDirectoryFiles(d,c,a[f].path);if(b&&d===this.version)for(f=
0;f<b.length;f++)c.push(b[f].path);if(!App.hostInterface||!c.length)return null;for(f=c.length-1;0<=f;f--)c[f]=FileUtils.fileURLPrefix+encodeURI(c[f]);App.hostInterface.setFileURLs(c);e=yield e;if(!e||e.length!==c.length||d!==this.version)return null;for(d=c.length-1;0<=d;d--)e[d]["data-path"]=c[d];return e})}navigate(a){return __awaiter(this,void 0,void 0,function*(){this.cancel();var b=this.version;if(!FilePickerAndroidProvider.permissionGranted&&!(yield FilePickerAndroidProvider.checkPermission())||
b!==this.version)return null;const c=yield this.enumerate(a);if(!c||!c.length||b!==this.version)return null;b=Array(c.length);if(a)for(var d=b.length-1;0<=d;d--){var e=c[d];b[d]={name:FileUtils.nameFromLinuxPath(e),dir:47===e.charCodeAt(0)?0:1,path:e}}else for(d=b.length-1;0<=d;d--){e=c[d];const g=e.indexOf(FileUtils.separator);b[d]={name:0<g?e.substring(0,g):Formatter.none,dir:1,path:0<g?e.substring(g+1):e,root:!0}}const f=Strings.comparer;b.sort(function(g,h){return g.dir===h.dir?f(g.name,h.name):
h.dir-g.dir});return b})}cancel(){this.version++;this.enumerationVersion++}destroy(){App.hostInterface&&App.hostInterface.cancelFileEnumeration(this.enumerationVersion);this.cancel()}}FilePickerAndroidProvider.callback=null;FilePickerAndroidProvider.callbackPermissionResolve=null;FilePickerAndroidProvider.permissionGranted=!1;
...
"GEMNUSEP";return h};c=function(h){const k=document.createElement("div");k.className="GEMNULBL";k.appendChild(document.createTextNode(h));return k};d=function(h,k,m,l,n,p){const q=document.createElement("div");q.className=p?"GEMNUIT GECLK "+p:"GEMNUIT GECLK";if(k)if(f&&(l&&f.radioHTML||!l&&f.checkHTML))q.innerHTML=l?f.radioHTML:f.checkHTML,k=q.firstChild,k.style.marginRight=l?f.radioMargin||"2px":f.checkMargin||"2px",m||(k.style.visibility="hidden");else{k=document.createElement("span");p=l?"\u25cf ":
"\u25a0 ";let t=null;if(f){let u=!1;l?f.radioCharacter&&(u=!0,p=f.radioCharacter,t=f.radioMargin||"2px"):f.checkCharacter&&(u=!0,p=f.checkCharacter,t=f.checkMargin||"2px");u&&(f.checkFontFamily&&(k.style.fontFamily=f.checkFontFamily),f.checkFontSize&&(k.style.fontSize=f.checkFontSize))}t&&(k.style.marginRight=t);k.appendChild(document.createTextNode(p));m||(k.style.visibility="hidden");q.appendChild(k)}q.appendChild(document.createTextNode(h));n&&(q.onclick=n);return q};this.element=a;a.className=
"GE";this.boundMouseMove=this.mouseMove.bind(this);this._scale=0;this.scale=f&&f.scale&&0<f.scale?f.scale:1;this.renderer=f&&f.svgRenderer?new GraphicalFilterEditorSVGRenderer(this):new GraphicalFilterEditorCanvasRenderer(this);this.renderer.element.addEventListener("mousemove",this.boundMouseMove);this.renderer.element.oncontextmenu=cancelEvent;a.appendChild(this.renderer.element);this.pointerHandler=new PointerHandler(this.renderer.element,this.mouseDown.bind(this),this.mouseMove.bind(this),this.mouseUp.bind(this));
a.oncontextmenu=cancelEvent;var g=document.createElement("div");g.className="GELBL";g.style.width="9em";g.appendChild(document.createTextNode(GraphicalFilterEditorStrings.Cursor));g.appendChild(this.lblCursor=document.createElement("span"));g.appendChild(document.createTextNode(" dB"));this.lblCursor.appendChild(document.createTextNode(GraphicalFilterEditorStrings.Minus0));a.appendChild(g);g=document.createElement("div");g.className="GELBL";g.style.width="9em";g.appendChild(document.createTextNode(GraphicalFilterEditorStrings.Curve));
...
!0,!1,!1,this.mnuNormalizeCurves_Click.bind(this),"GEFILTER"));g.appendChild(this.mnuShowZones=d(GraphicalFilterEditorStrings.ShowZones,!0,!1,!1,this.mnuShowZones_Click.bind(this)));g.appendChild(this.mnuShowActual=d(GraphicalFilterEditorStrings.ShowActualResponse,!0,!0,!1,this.mnuShowActual_Click.bind(this)));this.mnu.appendChild(g);a.appendChild(this.mnu);e&&this.loadSettings(e);this.drawCurve()}destroy(){this.filter&&this.filter.destroy();this.pointerHandler&&this.pointerHandler.destroy();this.renderer&&
this.renderer.destroy();zeroObject(this)}loadSettings(a){if(a){var b=this.filter;if(!1===a.showZones||!0===a.showZones)this._showZones=a.showZones;a.editMode&&a.editMode>=GraphicalFilterEditorControl.editModeFirst&&a.editMode<=GraphicalFilterEditorControl.editModeLast&&(b.iirSupported||a.editMode!==GraphicalFilterEditorControl.editModePeakingEq&&a.editMode!==GraphicalFilterEditorControl.editModeShelfEq)&&(this._editMode=a.editMode);if(!1===a.isActualChannelCurveNeeded||!0===a.isActualChannelCurveNeeded)this._isActualChannelCurveNeeded=
a.isActualChannelCurveNeeded;if(0===a.currentChannelIndex||1===a.currentChannelIndex)this._currentChannelIndex=a.currentChannelIndex;if(!1===a.isSameFilterLR||!0===a.isSameFilterLR)this.isSameFilterLR=a.isSameFilterLR;var c=GraphicalFilterEditor.decodeCurve(a.leftCurve),d=GraphicalFilterEditor.decodeCurve(a.rightCurve);c&&!d?d=c:d&&!c&&(c=d);if(c){var e=b.channelCurves[0];for(let f=GraphicalFilterEditor.visibleBinCount-1;0<=f;f--)e[f]=b.clampY(c[f])}if(d)for(c=b.channelCurves[1],e=GraphicalFilterEditor.visibleBinCount-
...
b.updateFilter(this._currentChannelIndex,this.isSameFilterLR,!0):b.changeIsNormalized(a,this._currentChannelIndex,this.isSameFilterLR);this._isActualChannelCurveNeeded&&this.filter.updateActualChannelCurve(this._currentChannelIndex);this.checkMenu(this.mnuShowZones,this._showZones);this.editMode=this._editMode;this.checkMenu(this.mnuNormalizeCurves,this.filter.isNormalized);this.checkMenu(this.mnuShowActual,this._isActualChannelCurveNeeded);this.drawCurve()}}saveSettings(){return{showZones:this._showZones,
editMode:this._editMode,isActualChannelCurveNeeded:this._isActualChannelCurveNeeded,currentChannelIndex:this._currentChannelIndex,isSameFilterLR:this.isSameFilterLR,isNormalized:this.filter.isNormalized,leftCurve:GraphicalFilterEditor.encodeCurve(this.filter.channelCurves[0]),rightCurve:GraphicalFilterEditor.encodeCurve(this.filter.channelCurves[1])}}get scale(){return this._scale}set scale(a){0>=a||this._scale===a||!this.element||(this._scale=a,this.element.style.fontSize=12*a+"px",this.element.style.lineHeight=
16*a+"px",this.renderer&&(this.renderer.scaleChanged(),this.drawCurve()))}get showZones(){return this._showZones}set showZones(a){this._showZones=a;this.checkMenu(this.mnuShowZones,a);this.drawCurve()}get editMode(){return this._editMode}set editMode(a){if(!(a<GraphicalFilterEditorControl.editModeFirst||a>GraphicalFilterEditorControl.editModeLast)&&(this.filter.iirSupported||a!==GraphicalFilterEditorControl.editModePeakingEq&&a!==GraphicalFilterEditorControl.editModeShelfEq)){this._editMode=a;this.checkMenu(this.mnuEditRegular,
...
a?GraphicalFilterEditorStrings.MinusInfinity:0>a?GraphicalFilterEditorStrings.toFixed(a,2):0===a?GraphicalFilterEditorStrings.Minus0:"+"+GraphicalFilterEditorStrings.toFixed(a,2)}static formatFrequency(a){return a[0].toFixed(0)+" Hz ("+(1E3>a[1]?a[1]+" Hz":a[1]/1E3+" kHz")+")"}static setFirstNodeText(a,b){a.firstChild&&(a.firstChild.nodeValue=b)}btnMnu_Click(a){a.button||("none"===this.mnu.style.display?(this.mnu.style.bottom=this.element.clientHeight-this.renderer.element.clientHeight+"px",this.mnu.style.display=
"inline-block",this.openMenuElement&&this.closeMenuElement?(this.openMenuElement.style.display="none",this.closeMenuElement.style.display=""):GraphicalFilterEditorControl.setFirstNodeText(this.btnMnu,this.closeMenuCharacter)):(this.mnu.style.display="none",this.openMenuElement&&this.closeMenuElement?(this.closeMenuElement.style.display="none",this.openMenuElement.style.display=""):GraphicalFilterEditorControl.setFirstNodeText(this.btnMnu,this.openMenuCharacter)));return!0}checkMenu(a,b){a.firstChild.style.visibility=
b?"visible":"hidden";return b}mnuChB_Click(a,b){b.button||this.isSameFilterLR&&this._currentChannelIndex===a||(this.isSameFilterLR?(this._currentChannelIndex=a,this.filter.updateFilter(a,!0,!0),this._isActualChannelCurveNeeded&&this.filter.updateActualChannelCurve(a),this.drawCurve()):(this.isSameFilterLR=!0,this.filter.copyFilter(a,1-a),this._currentChannelIndex!==a&&(this._currentChannelIndex=a,this._isActualChannelCurveNeeded&&this.filter.updateActualChannelCurve(a),this.drawCurve())),this.checkMenu(this.mnuChBL,
0===a),this.checkMenu(this.mnuChL,!1),this.checkMenu(this.mnuChBR,1===a),this.checkMenu(this.mnuChR,!1));return this.btnMnu_Click(b)}mnuChLR_Click(a,b){b.button||!this.isSameFilterLR&&this._currentChannelIndex===a||(this.isSameFilterLR&&(this.isSameFilterLR=!1,this.filter.updateFilter(1-this._currentChannelIndex,!1,!1)),this._currentChannelIndex!==a&&(this._currentChannelIndex=a,this._isActualChannelCurveNeeded&&this.filter.updateActualChannelCurve(a),this.drawCurve()),this.checkMenu(this.mnuChBL,
!1),this.checkMenu(this.mnuChL,0===a),this.checkMenu(this.mnuChBR,!1),this.checkMenu(this.mnuChR,1===a));return this.btnMnu_Click(b)}mnuResetCurve_Click(a){a.button||this.resetCurve();return this.btnMnu_Click(a)}mnuShowZones_Click(a){a.button||(this.showZones=!this._showZones);return this.btnMnu_Click(a)}mnuEditRegular_Click(a){a.button||(this.editMode=GraphicalFilterEditorControl.editModeRegular);return this.btnMnu_Click(a)}mnuEditZones_Click(a){a.button||(this.editMode=GraphicalFilterEditorControl.editModeZones);
...
class DelayControl{static delayShortCB(a){return setTimeout(a,DelayControl.shortDelayMS)}static delayUICB(a){return setTimeout(a,DelayControl.uiDelayMS)}static delayShort(){return new Promise(function(a){setTimeout(a,DelayControl.shortDelayMS)})}static delayUI(){return new Promise(function(a){setTimeout(a,DelayControl.uiDelayMS)})}}DelayControl.shortDelayMS=10;DelayControl.uiDelayMS=310;
...
(c+=" "+a.color);a.square&&(c+=" square");a.opaque||(c+=" transparent");a.className&&(c+=" "+a.className);b.className=c;b=CheckboxControl.prepare(b,a.icon,a.iconColor,a.checkbox0Color,a.checkbox1Color);!1===a.keyboardFocusable&&b.setAttribute("tabindex","-1");a.parent&&a.parent.appendChild(b);return b}static prepare(a,b,c,d,e){const f=document.createElement("button"),g=document.createElement("span"),h=a.parentNode,k=b?null:a.getAttribute("data-square"),m=Strings.translate(a.getAttribute("title")||
a.value);f.setAttribute("type","button");f.setAttribute("role","checkbox");f.setAttribute("aria-label",m);g.setAttribute("tabindex","-1");a.setAttribute("tabindex","-1");a.setAttribute("aria-label",m);f.className=a.className;a.className="";b||(b=a.getAttribute("data-icon"));b&&!c&&(c=a.getAttribute("data-icon-color"));d||(d=a.getAttribute("data-checkbox0-color"));e||(e=a.getAttribute("data-checkbox1-color"))||(e="orange");h&&(h.insertBefore(f,a),h.removeChild(a));f.appendChild(g);g.appendChild(a);
g.appendChild(Icon.createLarge("icon-checkbox-0",(k?"":"margin")+(d?" "+d:"")));g.appendChild(Icon.createLarge("icon-checkbox-1",k?e:"margin "+e));b&&g.appendChild(Icon.createLarge(b,c?"margin "+c:"margin"));k||g.appendChild(document.createTextNode(m));a.removeAttribute("value");a.removeAttribute("title");f.onclick=CheckboxControl.click;f.childInput=a;return f}static click(a){(a=this.childInput)&&a.click()}}
...
b-e)&&e<f)for(let l=e,n=0;l<f;l++,n++)e=c[n],c[n]=c[l],c[l]=e}else if((e-=b)&&e<f)for(let l=f-1,n=l-e;0<=n;n--,l--)e=c[l],c[l]=c[n],c[n]=e;var g=a.list,h=g.length,k=a.list.currentItem;e=this.container;var m=0;for(let l=b*d;m<f&&b<h;m++,b++,l+=d){const n=g.item(b),p=c[m];if(p.item!==n){const q=k===n;p.current!==q&&((p.current=q)?p.element.classList.add("current"):p.element.classList.remove("current"));a.prepareElement(n,b,h,p.element);p.item||e.appendChild(p.element);p.item=n}p.index!==b&&(p.index=
...
var h=0,k=this.firstIndex,m=this.visibleCount+(0===k?2:1===k?3:4);m>b.length&&(m=b.length);0>(k-=2)&&(k=0);for(let l=k*c;h<m&&k<e;h++,k++,l+=c){const n=d.item(k),p=b[h],q=f===n;p.current!==q&&((p.current=q)?p.element.classList.add("current"):p.element.classList.remove("current"));a.prepareElement(n,k,e,p.element);p.item||g.appendChild(p.element);p.item=n;p.index!==k&&(p.index=k,p.element.style.transform="translateY("+l+"px)")}for(a=b.length;h<a;h++)d=b[h],d.item&&(d.item=null,g.removeChild(d.element))}else{g=
...
(this.refreshVisibleItemsEnqueued=!0,queueMicrotask(this.boundRefreshVisibleItemsInternal))}notifyCleared(){this.clear()}notifyItemsAdded(a,b){if(this.useVirtualItems)this.refreshVisibleItems();else{const m=this.adapter,l=b-a+1;if(m&&!(0>=l)){var c=this.container,d=this.items,e=m.list,f=e.length,g=e.currentItem,h=Array(l);for(var k=b;k>=a;k--){const n=e.item(k),p=new ListControlItem(m.createEmptyElement("list-item real"));m.prepareElement(n,k,f,p.element);h[k-a]=p;g===n&&(this.currentItem&&(this.currentItem.current=
...
class FilePickerListAdapter extends ListAdapter{constructor(a){super(a)}get itemHeight(){return AppUI.simpleListItemPX}createEmptyElement(a){const b=document.createElement("div");b.className=a;CheckboxControl.create({stringKey:"Selected",checkbox0Color:"pink",checkbox1Color:"green",square:!0,keyboardFocusable:!1,className:"right-margin",parent:b}).setAttribute("data-check","1");ButtonControl.create({icon:"icon-clear",text:Strings.Delete,color:"red",square:!0,keyboardFocusable:!1,className:"no-left-margin right-margin",
parent:b}).setAttribute("data-delete","1");b.appendChild(Icon.createLarge("icon-folder","orange button-top-margin margin"));b.appendChild(Icon.createLarge("icon-add-folder","green button-top-margin margin"));b.appendChild(document.createTextNode(Formatter.none));return b}prepareElement(a,b,c,d){b=d.childNodes;a.root?(b[0].classList.add("hidden"),a.path?b[3].classList.add("hidden"):b[3].classList.remove("hidden")):(c=b[0],c.classList.remove("hidden"),CheckboxControl.setChecked(c,a.selected),b[3].classList.add("hidden"));
a.deletable?b[1].classList.remove("hidden"):b[1].classList.add("hidden");a.dir?b[2].classList.remove("hidden"):b[2].classList.add("hidden");b[4].nodeValue=a.name}prepareElementIndexOrLengthChanged(a,b,c,d){}}
...
AppUI.monoDownMixerControlEnabled.checked=b.monoDownMixerControlEnabled||!1,AppUI.monoDownMixerControlEnabled.onclick=AppUI.monoDownMixerControlEnabledClicked):(b=document.getElementById("mono-down-mixer-control-container"),b.parentNode.removeChild(b));App.frameless||AppUI.panelContainer.classList.add("web");window.addEventListener("resize",AppUI.adjustWindow,{passive:!0});AppUI.adjustWindow()}static init(a){App.player&&(AppUI.volumeSlider.value=App.player.volume,App.player.onsongchanged=AppUI.playerSongChanged,
...
AppUI.playlistControl.centerItemIntoView(App.player.playlist.currentIndex)}static destroy(){zeroObject(AppUI)}static get loading(){return AppUI._loading}static get thinBorderPX(){return AppUI._thinBorderPX}static get thickBorderPX(){return AppUI._thickBorderPX}static get simpleListItemPX(){return AppUI._simpleListItemPX}static addZoomHandler(a){a&&AppUI.zoomHandlers.push(a)}static removeZoomHandler(a){if(a)for(let b=AppUI.zoomHandlers.length-1;0<=b;b--)if(AppUI.zoomHandlers[b]===a){AppUI.zoomHandlers.splice(b,
1);break}}static factorToZoom(a){return 1>=a?0:Math.round(4*a)-4}static zoomToFactor(a){return 0>=a?1:.25*(a+4)}static remToPX(a){return a*AppUI.rem}static pxToREM(a){return a/AppUI.rem}static playerSongChanged(a){App.player&&AppUI.playlistControl&&(Strings.changeText(AppUI.titleLabel,a&&a.title||Formatter.none),Strings.changeText(AppUI.artistLabel,a&&a.artist||Formatter.none),App.player.playlist&&0<=App.player.playlist.currentIndex&&!AppUI.playlistControl.deleteMode&&AppUI.playlistControl.bringItemIntoView(App.player.playlist.currentIndex),
...
(App.monoDownMixerControl.enabled=AppUI.monoDownMixerControlEnabled.checked)}static hideTopMessage(){0>AppUI.topMessageFading||!AppUI.topMessage||(AppUI.topMessageTimeout&&clearTimeout(AppUI.topMessageTimeout),AppUI.topMessageFading=-1,AppUI.topMessage.classList.remove("in"),AppUI.topMessageTimeout=DelayControl.delayUICB(function(){0<=AppUI.topMessageFading||(AppUI.topMessageFading=0,AppUI.topMessageTimeout=0,AppUI.topMessage.style.visibility="",AppUI.topMessage.innerHTML="",AppUI.titleLabel&&AppUI.titleLabel.classList.remove("behind"),
...
1!==AppUI.topMessageFading||(AppUI.topMessageFading=2,AppUI.topMessage.classList.add("in"),AppUI.topMessageTimeout=DelayControl.delayUICB(function(){2===AppUI.topMessageFading&&(AppUI.topMessageFading=0,AppUI.topMessageTimeout=0)}))}))}static addFiles(a){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading&&!Modal.visible&&!FilePicker.visible){var b=App.hostType===App.hostTypeElectron,c=FilePicker.isSupported()?yield FilePicker.show():yield App.showOpenDialogWeb(a);if(c&&
App.player)try{AppUI._loading=!0;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const d=App.player.playlist,e=b?null:new Uint8Array(BufferedFileHandle.minBufferLength<<1),f=b?null:[new Uint8Array(256)];let g=!1;for(let h=0;h<c.length&&App.player;h++)if(b)yield d.addSong(c[h]);else{const k=d.length;(yield d.addSongWeb(c[h],e,f))&&k===d.length&&(g=!0)}g&&AppUI.playlistControl&&AppUI.playlistControl.refreshVisibleItems()}catch(d){Modal.show({text:"addFiles error: "+
(d.message||d.toString())})}finally{AppUI._loading=!1,App.updateLoadingIcon()}}})}static toggleDeleteMode(a){AppUI.playlistControl&&(AppUI.playlistControl.deleteMode?(AppUI.playlistControl.deleteMode=!1,AppUI.hideTopMessage(),a&&HistoryHandler.popState()):(AppUI.playlistControl.deleteMode=!0,AppUI.showTopMessage(Strings.DeleteModeHTML,function(b){const c=document.createElement("div");ButtonControl.create({color:"red",icon:"icon-delete-all",text:Strings.DeleteAllSongs,parent:c,onclick:function(){App.player&&
App.player.playlist&&App.player.playlist.clear();AppUI.playlistControl&&AppUI.playlistControl.deleteMode&&AppUI.toggleDeleteMode(!0)}});ButtonControl.create({color:"green",icon:"icon-check",text:Strings.Done,parent:c,onclick:function(){AppUI.playlistControl&&AppUI.playlistControl.deleteMode&&AppUI.toggleDeleteMode(!0)}});b.appendChild(c)}),HistoryHandler.pushState()))}static switchView(){AppUI.panelContainer&&AppUI.panelContainer.classList.toggle("toggled")}static showAbout(){Modal.show({html:Strings.AboutHTML+
...
AppUI.baseThickBorderPX=2;AppUI._simpleListItemPX=AppUI.remToPX(AppUI.buttonSizeREM+2);AppUI.zoomHandlers=[];AppUI.rootVariables=document.getElementById("root-variables");AppUI.panelContainer=document.getElementById("panel-container");AppUI.fixedPanel=document.getElementById("fixed-panel");AppUI.optionalPanel=document.getElementById("optional-panel");AppUI.iconPlay=document.getElementById("i-play");AppUI.iconPause=document.getElementById("i-pause");AppUI.artistLabel=document.getElementById("artist-label");
AppUI.titleLabel=document.getElementById("title-label");AppUI.topMessage=document.getElementById("top-message");AppUI._loading=!1;AppUI.devicePixelRatio=-1;AppUI._smallIconSizePX=Icon.baseSizePX;AppUI._largeIconSizePX=Icon.baseSizePX<<1;AppUI._thinBorderPX=AppUI.baseThinBorderPX;AppUI._thickBorderPX=AppUI.baseThickBorderPX;AppUI.directoryCancelled=!1;AppUI.topMessageFading=0;AppUI.topMessageTimeout=0;
...
App.mainWindowClosing),App.triggerMainWindowStateChanged()):App.hostInterface||window.addEventListener("onpagehide"in window?"pagehide":"unload",App.mainWindowClosing);App.frameless&&(App.titleBar=document.createElement("h1"),App.titleBar.innerHTML=`
			...
			`,document.body.insertBefore(App.titleBar,document.body.firstChild),App.iconRestore=document.getElementById("icon-restore"),App.iconMaximize=document.getElementById("icon-maximize"),document.body.removeChild(App.iconLoading),App.iconLoading.className="",App.titleBar.insertBefore(App.iconLoading,document.getElementById("icon-main").nextSibling));App.hostType!==App.hostTypeAndroid&&FilePicker.isSupported()||(c=document.createElement("div"),c.style.display="none",App.fileInput=document.createElement("input"),
App.fileInput.setAttribute("type","file"),App.fileInput.setAttribute("multiple","multiple"),App.fileInput.setAttribute("aria-label",Strings.AddSongs),App.fileInput.onchange=function(){if(App.fileInput&&App.fileInputPromiseResolve&&parseInt(App.fileInput.getAttribute("data-version"))===App.fileInputVersion){App.fileInputVersion++;var d=App.fileInputPromiseResolve;App.fileInputPromiseResolve=null;if(App.fileInput.files&&App.fileInput.files.length&&(1!==App.fileInput.files.length||FileUtils.isTypeSupported(App.fileInput.files[0].name))){var e=
Array(App.fileInput.files.length);for(let f=e.length-1;0<=f;f--)e[f]=App.fileInput.files[f];App.fileInput.value="";d(App.fileInputSkipSortResults?e:App.sortFiles(e))}else d(null)}},c.appendChild(App.fileInput),document.getElementById("top-panel").appendChild(c));AppUI.preInit(b,a);App.init(a)})}static init(a){if(!App.player){App.player=new Player(a.playerVolume,function(c){App.graphicalFilterControl=new GraphicalFilterControl(document.getElementById("filter-container"),document.getElementById("optional-panel-container"),
c,a.graphicalFilterControlSimpleMode);App.graphicalFilterControl.enabled=!!a.graphicalFilterControlEnabled;return App.graphicalFilterControl},function(c){App.stereoPannerControl=new StereoPannerControl(document.getElementById("stereo-panner-slider"),c);App.stereoPannerControl.enabled=!!a.stereoPannerControlEnabled;App.stereoPannerControl.onappliedgainchanged=function(d){App.monoDownMixerControl&&(App.monoDownMixerControl.multiplier=1/(1+d))};return App.stereoPannerControl},function(c){App.monoDownMixerControl=
new MonoDownMixerControl(c);App.monoDownMixerControl.enabled=MonoDownMixerControl.isSupported()&&!!a.monoDownMixerControlEnabled;return App.monoDownMixerControl});var b=InternalStorage.loadPlaylistWeb();b&&(App.player.playlist=b);AppUI.init(a);(b=document.getElementById("cover"))&&b.classList.remove("in");DelayControl.delayShortCB(function(){AppUI.centerCurrentSongIntoView();DelayControl.delayUICB(function(){window.removeEventListener("resize",App.adjustCover);const c=document.getElementById("cover");
c&&document.body.removeChild(c)})});App.loading=!1}}static triggerMainWindowStateChanged(){App.ipcRenderer&&App.ipcRenderer.invoke("triggerMainWindowStateChanged")}static mainWindowMinimize(){App.ipcRenderer&&App.ipcRenderer.invoke("mainWindowMinimize")}static mainWindowRestoreOrMaximize(){App.ipcRenderer&&App.ipcRenderer.invoke(App.isMaximized?"mainWindowRestore":"mainWindowMaximize")}static mainWindowClose(){App.ipcRenderer&&App.ipcRenderer.invoke("mainWindowClose")}static extractMetadata(a,b){return App.ipcRenderer.invoke("extractMetadata",
a,b)}static showOpenDialogWeb(a,b){if(!App.fileInput)return Promise.resolve(null);App.fileInputPromiseResolve&&(App.fileInputPromiseResolve(null),App.fileInputPromiseResolve=null);App.fileInputSkipSortResults=!!b;App.fileInput.setAttribute("data-version",(++App.fileInputVersion).toString());a?(App.fileInput.setAttribute("webkitdirectory","webkitdirectory"),App.fileInput.setAttribute("directory","directory"),App.fileInput.removeAttribute("accept")):(App.fileInput.removeAttribute("webkitdirectory"),
App.fileInput.removeAttribute("directory"),App.fileInput.setAttribute("accept",FileUtils.concatenatedSupportedExtensions));a=new Promise(function(c){App.fileInputPromiseResolve=c});App.fileInput.click();return a}}App.hostTypeElectron="Electron";App.hostTypeAndroid="Android";App.ipcRenderer=window.electronIpcRenderer||null;App.closeHandlers=[];App.frameless=!!window.electronIpcRenderer;App.hostInterface=window.hostInterface||null;App.hostType=App.hostInterface&&App.hostInterface.getHostType()||null;
App.iconLoading=document.getElementById("icon-loading");App.titleBar=null;App.iconRestore=null;App.iconMaximize=null;App.installPrompt=null;App.fileInputVersion=0;App.fileInputSkipSortResults=!0;App._loading=!0;App.isMinimized=!1;App.isMaximized=!1;function main(){App.preInit().catch(console.error)}let cLib;
