...
this.label.className="db-label large small-left-margin";this.slider.rightChild=this.label;this.slider.ondragended=b;this.slider.onkeyboardchanged=b;this.slider.onvaluechanged=this.sliderValueChanged.bind(this);this.sliderValueChanged(this._pan);this.commitChanges(this._pan)}sliderValueChanged(a){AppUI.changeText(this.label,(a?(0>a?Strings.RightAbbrev:Strings.LeftAbbrev)+" "+((a=Math.abs(a))>=StereoPannerControl.maxAbsoluteValue?GraphicalFilterEditorStrings.MinusInfinity:"-"+a):"-0")+" dB")}commitChanges(a){const b=
this.audioContext,c=this.channelSplitter,d=this.gain,e=this.channelMerger;if(c&&d&&e)if(this._pan=a){const f=0<a,g=!f;a=Math.abs(a);d.gain.setValueAtTime(a>=StereoPannerControl.maxAbsoluteValue?0:Math.pow(10,-a/20),b.currentTime+.1);if(f!==this.leftGain||g!==this.rightGain)c.disconnect(),d.disconnect(),this.leftGain=f,this.rightGain=g,f?(c.connect(d,0,0),d.connect(e,0,0),c.connect(e,1,1)):(c.connect(d,1,0),d.connect(e,0,1),c.connect(e,0,0))}else this.rightGain=this.leftGain=!1,d.disconnect(),c.disconnect(),
...
class GraphicalFilterControl{constructor(a,b,c,d,e,f){this.filterChangedCallback=f;this._enabled=!!d;this._simpleMode=!!e;this.container=a;this.outerContainer=b;d=document.createElement("div");this._simpleMode||(b.style.minWidth=GraphicalFilterControl.advancedModeMinWidth,a.appendChild(d));e=InternalStorage.loadGraphicalFilterEditorSettings();this._simpleMode&&(e.editMode=GraphicalFilterEditorControl.editModeShelfEq);this.editor=new GraphicalFilterEditorControl(d,2048,c,this.filterChanged.bind(this),e,
{svgRenderer:!0,radioHTML:Icon.createHTML("icon-radio",!1,"menu-icon"),checkHTML:Icon.createHTML("icon-check",!1,"menu-icon"),menuWidth:Icon.outerSizeREM+"rem",menuPadding:"0",openMenuHTML:"<span>"+Icon.createHTML("icon-expand-less",!0)+"</span>",closeMenuHTML:"<span>"+Icon.createHTML("icon-expand-more",!0)+"</span>"});d=document.createElement("div");d.className="button-bottom-margin";this._simpleMode&&(b.style.minWidth=GraphicalFilterControl.simpleModeMinWidth,a.appendChild(d));this.simpleEditor=
new SimpleFilterEditorControl(d,this.editor);a=this.editor.element.querySelectorAll("div.GELBL");for(b=a.length-1;0<=b;b--)c=a[b],c.style.height="var(--button-size)",c.style.lineHeight="6rem",c.style.paddingTop="var(--button-padding-top)",c.style.paddingBottom="0";a=this.editor.element.querySelector("div.GEBTN.GECLK");a.className="btn square white transparent";a.style.width="";a.style.padding="";a.style.float="right"}filterChanged(){this.filterChangedCallback&&this.filterChangedCallback()}get enabled(){return this._enabled}set enabled(a){this._enabled!==
a&&(this._enabled=a,this.filterChangedCallback&&this.editor&&this.editor.filter&&this.filterChangedCallback())}get simpleMode(){return this._simpleMode}set simpleMode(a){if(this._simpleMode!==a){this._simpleMode=a;if(a){this.outerContainer.style.minWidth=GraphicalFilterControl.simpleModeMinWidth;this.editor.editMode=GraphicalFilterEditorControl.editModeShelfEq;this.simpleEditor.updateSliders();a=this.editor.element;var b=this.simpleEditor.element}else this.outerContainer.style.minWidth=GraphicalFilterControl.advancedModeMinWidth,
...
this._alive=!0;this._loading=!1;this._paused=!0;this._currentSong=this._playlist=null;this._volume=Player.maxVolume;this.lastObjectURL=null;this.volume=void 0===d?Player.maxVolume:d;this.filterOutput=this.source;this.stereoPannerInput=this.audioContext.destination;this.filterChanged();this.stereoPannerChanged();(this.mediaSession=d="mediaSession"in navigator&&"MediaMetadata"in window?navigator.mediaSession:null)&&"setActionHandler"in d&&(a=this.playPause.bind(this),d.setActionHandler("previoustrack",
this.previous.bind(this)),d.setActionHandler("seekbackward",this.seekBackward.bind(this)),d.setActionHandler("pause",a),d.setActionHandler("play",a),d.setActionHandler("seekforward",this.seekForward.bind(this)),d.setActionHandler("nexttrack",this.next.bind(this)))}destroy(a){this.stop();a?(this._alive=!1,this.graphicalFilterControl&&this.graphicalFilterControl.saveSettings(),this.stereoPannerControl&&this.stereoPannerControl.saveSettings()):(this._playlist&&this._playlist.destroy(),(a=this.mediaSession)&&
...
if(b)return b}}catch(a){}return{playerVolume:100,graphicalFilterControlEnabled:!1,graphicalFilterControlSimpleMode:544>window.innerWidth,stereoPannerControlEnabled:!1}}static saveAppSettings(a){a?localStorage.setItem("appSettings",JSON.stringify(a)):localStorage.removeItem("appSettings")}static loadGraphicalFilterEditorSettings(){try{const a=localStorage.getItem("graphicalFilterEditorSettings");if(a){const b=JSON.parse(a);if(b)return b}}catch(a){}return{currentChannelIndex:0,editMode:GraphicalFilterEditorControl.editModeShelfEq,
...
document.getElementById("stereo-panner-control-enabled");AppUI.stereoPannerControlEnabled.checked=b.stereoPannerControlEnabled||!1;AppUI.stereoPannerControlEnabled.onclick=AppUI.stereoPannerControlEnabledClicked;App.frameless||AppUI.panelContainer.classList.add("web");window.addEventListener("resize",AppUI.adjustWindow,{passive:!0});AppUI.adjustWindow()}static init(a){App.player&&(AppUI.volumeSlider.value=App.player.volume,App.player.onsongchanged=AppUI.playerSongChanged,App.player.onloadingchanged=
