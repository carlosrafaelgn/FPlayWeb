...
this._paused=!0;this._currentSong=this._playlist=null;this._volume=100;this.lastObjectURL=null;this.volume=void 0===b?100:b;this.filterChanged();(this.mediaSession=b="mediaSession"in navigator&&"MediaMetadata"in window?navigator.mediaSession:null)&&"setActionHandler"in b&&(a=this.playPause.bind(this),b.setActionHandler("previoustrack",this.previous.bind(this)),b.setActionHandler("seekbackward",this.seekBackward.bind(this)),b.setActionHandler("pause",a),b.setActionHandler("play",a),b.setActionHandler("seekforward",
this.seekForward.bind(this)),b.setActionHandler("nexttrack",this.next.bind(this)))}destroy(a){this.stop();a?(this._alive=!1,this.graphicalFilterControl&&this.graphicalFilterControl.saveSettings()):(this._playlist&&this._playlist.destroy(),(a=this.mediaSession)&&"setActionHandler"in a&&(a.setActionHandler("previoustrack",null),a.setActionHandler("seekbackward",null),a.setActionHandler("pause",null),a.setActionHandler("play",null),a.setActionHandler("seekforward",null),a.setActionHandler("nexttrack",
...
(App.player.graphicalFilterControl.enabled=AppUI.graphicalFilterControlEnabled.checked)}static addFiles(a){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading){var b=App.electron?yield App.showOpenFileDialog():yield App.showOpenDialogWeb(a);if(b&&App.player)try{AppUI._loading=!0;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const c=App.electron?null:new Uint8Array(BufferedFileHandle.minBufferLength<<1),d=App.electron?
null:[new Uint8Array(256)];for(let e=0;e<b.length&&App.player&&App.player.playlist;e++)App.electron?yield App.player.playlist.addSong(b[e]):yield App.player.playlist.addSongWeb(b[e],c,d)}catch(c){alert("addFiles error: "+(c.message||c.toString()))}finally{AppUI._loading=!1,App.updateLoadingIcon()}}})}static addDirectory(a){return __awaiter(this,void 0,void 0,function*(){const b=yield App.listDirectory(a);if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;if(!b)return!0;for(let c=
0;c<b.length;c++)if(b[c].isDirectory){if(!(yield AppUI.addDirectory(b[c].fileURL)))return!1}else{if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;yield App.player.playlist.addSong(b[c])}return!0})}static addDirectories(){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading){if(!App.electron)return AppUI.addFiles(!0);var a=yield App.showOpenDirectoryDialog();if(a&&App.player)try{AppUI._loading=!0;AppUI.directoryCancelled=!1;App.updateLoadingIcon();App.player.playlist||
(App.player.playlist=new Playlist,AppUI.preparePlaylist());for(let b=0;b<a.length&&(yield AppUI.addDirectory(a[b]));b++);}catch(b){alert("addDirectories error: "+(b.message||b.toString()))}finally{AppUI._loading=!1,AppUI.directoryCancelled=!1,App.updateLoadingIcon()}}})}static switchView(){AppUI.panelContainer&&AppUI.panelContainer.classList.toggle("toggled")}}AppUI.primaryInputIsTouch="matchMedia"in window?window.matchMedia("(pointer: coarse)").matches:0<navigator.maxTouchPoints||0<navigator.msMaxTouchPoints;
AppUI.rem=4;AppUI.buttonRegularSizeREM=8;AppUI.buttonRegularSizePX=AppUI.remToPX(AppUI.buttonRegularSizeREM);AppUI.buttonSizeREM=AppUI.buttonRegularSizeREM*(AppUI.primaryInputIsTouch?1.5:1);AppUI.buttonSizePX=AppUI.remToPX(AppUI.buttonSizeREM);AppUI.optionalPanelClientWidthREM=128;AppUI.optionalPanelClientWidthPX=AppUI.remToPX(AppUI.optionalPanelClientWidthREM);AppUI.scrollbarSizeREM=AppUI.primaryInputIsTouch?5:3;AppUI.minWidthREM=216;AppUI.minWidthPX=AppUI.remToPX(AppUI.minWidthREM);
AppUI.baseThinBorderPX=1;AppUI.baseThickBorderPX=2;AppUI.zoomHandlers=[];AppUI.rootVariables=document.getElementById("root-variables");AppUI.panelContainer=document.getElementById("panel-container");AppUI.fixedPanel=document.getElementById("fixed-panel");AppUI.optionalPanel=document.getElementById("optional-panel");AppUI.iconPlay=document.getElementById("icon-play");AppUI.iconPause=document.getElementById("icon-pause");AppUI.artistLabel=document.getElementById("artist-label");AppUI.titleLabel=document.getElementById("title-label");
...
delete window.electronIpcRenderer;delete window.electronWebFrame;if("serviceWorker"in navigator){window.addEventListener("beforeinstallprompt",App.beforeInstallPrompt);var c=navigator.serviceWorker.register("sw.js");c&&"then"in c&&c.then(function(d){d&&"onupdatefound"in d&&d.active&&(d.onupdatefound=function(){Modal.show({html:Strings.PleaseRefresh,title:Strings.UpdateAvailable,oktext:Strings.Refresh,okcancel:!0,onok:function(){window.location.reload()}})})})}window.addEventListener("resize",App.adjustCover);
App.adjustCover();App.ipcRenderer?(App.ipcRenderer.on("mainWindowStateChanged",App.mainWindowStateChanged),App.ipcRenderer.on("mainWindowClosing",App.mainWindowClosing),App.triggerMainWindowStateChanged()):window.addEventListener("beforeunload",App.mainWindowClosing);App.frameless&&(App.titleBar=document.createElement("h1"),App.titleBar.innerHTML=`
				<div></div>
				<i id="icon-main"></i>
				FPlay
