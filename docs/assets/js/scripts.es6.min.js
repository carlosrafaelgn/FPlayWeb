...
var __awaiter=this&&this.__awaiter||function(a,b,c,d){function e(f){return f instanceof c?f:new c(function(g){g(f)})}return new (c||(c=Promise))(function(f,g){function h(l){try{m(d.next(l))}catch(p){g(p)}}function k(l){try{m(d["throw"](l))}catch(p){g(p)}}function m(l){l.done?f(l.value):e(l.value).then(h,k)}m((d=d.apply(a,b||[])).next())})};
...
(p=b.fillBuffer(1024))&&(yield p);if(4<l){m=b.readByte();var n=b.readByte();p=b.readByte();if(null===m||null===n||null===p)break a;e-=3;l-=3;if(73===m||68===n||51===p)if(d=b.fileLength-e,f&&g){d=-1;break a}}b.skip(l);(p=b.fillBuffer(1024))&&(yield p);continue;default:l=b.readUInt32LE();if(null===l)break a;e-=8;if(0>l||l>e)break a;b.skip(l);(p=b.fillBuffer(1024))&&(yield p);continue}for((p=b.fillBuffer(1024))&&(yield p);m&&(!f||!g)||0<l;){var q=b.readUInt32BE();n=0;if(null===q)break a;e-=4;l-=4;if(1229866575!==
q){n=b.readUInt32LE();if(null===n||0>n||n>e||!m&&n>l)break a;e-=4;l-=4}1684108385!==q&&(p=b.fillBuffer(Math.min(264,8+n)))&&(yield p);switch(q){case 1718449184:if(!m)break;p=b.readUInt16LE();q=b.readUInt16LE();var t=b.readUInt32LE(),u=b.readUInt32LE(),r=b.readUInt16LE();const y=b.readUInt16LE();n-=16;e-=16;l-=16;if(null===p||null===q||null===t||null===u||null===r||null===y)break a;f=!0;k||(k=u);break;case 1684108385:m&&(g=!0,h||(h=n));break;case 1229866575:for(q=function(x){b.read(c,0,x);return MetadataExtractor.finishReadingV2Frame(0,
x,c)};0<l;){(p=b.fillBuffer(8))&&(yield p);t=b.readUInt32BE();e-=4;l-=4;if(null===t)break a;u=b.readUInt32LE();e-=4;l-=4;if(null===u||0>u||u>l)break a;if(1>=u)b.skip(u);else{r=Math.min(257,u)-1;(p=b.fillBuffer(r))&&(yield p);switch(t){case 1229865293:a.title=q(r);break;case 1230000708:a.album=q(r);break;case 1229017684:a.artist=q(r);break;case 1229148740:a.year=parseInt(q(r))||0;break;case 1230262859:a.track=parseInt(q(r))||0;break;default:r=0}b.skip(u-r);e-=u;l-=u}}}0<n&&(b.skip(n),e-=n,l-=n);e&&
...
App.player.currentSong&&App.player.seekTo(a)}static volumeStr(a){return(a>=Player.maxVolume?"-0":a?a-Player.maxVolume:GraphicalFilterEditorStrings.MinusInfinity)+" dB"}static volumeSliderValueChanged(a){App.player&&(App.player.volume=a);AppUI.volumeLabel&&AppUI.changeText(AppUI.volumeLabel,AppUI.volumeStr(a))}static playlistControlItemClicked(a,b){App.player&&App.player.play(b)}static playlistControlItemContextMenu(a,b){App.player&&App.player.play(b)}static graphicalFilterControlTypeClicked(){App.player&&
(App.player.graphicalFilterControl.simpleMode=!App.player.graphicalFilterControl.simpleMode,ButtonControl.setText(AppUI.graphicalFilterControlType,App.player.graphicalFilterControl.simpleMode?Strings.TraditionalFilter:Strings.AdvancedFilter))}static graphicalFilterControlEnabledClicked(){App.player&&(App.player.graphicalFilterControl.enabled=AppUI.graphicalFilterControlEnabled.checked)}static addFiles(a){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading){var b=App.electron?
yield App.showOpenFileDialog():yield App.showOpenDialogWeb(a);if(b&&App.player)try{AppUI._loading=!0;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const c=App.electron?null:new Uint8Array(BufferedFileHandle.minBufferLength<<1),d=App.electron?null:[new Uint8Array(256)];for(let e=0;e<b.length&&App.player&&App.player.playlist;e++)App.electron?yield App.player.playlist.addSong(b[e]):yield App.player.playlist.addSongWeb(b[e],c,d)}catch(c){alert("addFiles error: "+
(c.message||c.toString()))}finally{AppUI._loading=!1,App.updateLoadingIcon()}}})}static addDirectory(a){return __awaiter(this,void 0,void 0,function*(){const b=yield App.listDirectory(a);if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;if(!b)return!0;for(let c=0;c<b.length;c++)if(b[c].isDirectory){if(!(yield AppUI.addDirectory(b[c].fileURL)))return!1}else{if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;yield App.player.playlist.addSong(b[c])}return!0})}static addDirectories(){return __awaiter(this,
void 0,void 0,function*(){if(App.player&&!AppUI._loading){if(!App.electron)return AppUI.addFiles(!0);var a=yield App.showOpenDirectoryDialog();if(a&&App.player)try{AppUI._loading=!0;AppUI.directoryCancelled=!1;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());for(let b=0;b<a.length&&(yield AppUI.addDirectory(a[b]));b++);}catch(b){alert("addDirectories error: "+(b.message||b.toString()))}finally{AppUI._loading=!1,AppUI.directoryCancelled=!1,App.updateLoadingIcon()}}})}static switchView(){AppUI.panelContainer&&
AppUI.panelContainer.classList.toggle("toggled")}static showAbout(){Modal.show({html:Strings.AboutHTML,title:Strings.About})}}AppUI.primaryInputIsTouch="matchMedia"in window?window.matchMedia("(pointer: coarse)").matches:0<navigator.maxTouchPoints||0<navigator.msMaxTouchPoints;AppUI.rem=4;AppUI.buttonRegularSizeREM=8;AppUI.buttonRegularSizePX=AppUI.remToPX(AppUI.buttonRegularSizeREM);AppUI.buttonSizeREM=AppUI.buttonRegularSizeREM*(AppUI.primaryInputIsTouch?1.5:1);AppUI.buttonSizePX=AppUI.remToPX(AppUI.buttonSizeREM);
AppUI.optionalPanelClientWidthREM=128;AppUI.optionalPanelClientWidthPX=AppUI.remToPX(AppUI.optionalPanelClientWidthREM);AppUI.scrollbarSizeREM=AppUI.primaryInputIsTouch?5:3;AppUI.minWidthREM=216;AppUI.minWidthPX=AppUI.remToPX(AppUI.minWidthREM);AppUI.baseThinBorderPX=1;AppUI.baseThickBorderPX=2;AppUI.zoomHandlers=[];AppUI.rootVariables=document.getElementById("root-variables");AppUI.panelContainer=document.getElementById("panel-container");AppUI.fixedPanel=document.getElementById("fixed-panel");
AppUI.optionalPanel=document.getElementById("optional-panel");AppUI.iconPlay=document.getElementById("icon-play");AppUI.iconPause=document.getElementById("icon-pause");AppUI.artistLabel=document.getElementById("artist-label");AppUI.titleLabel=document.getElementById("title-label");AppUI._loading=!1;AppUI.devicePixelRatio=-1;AppUI._smallIconSizePX=Icon.baseSizePX;AppUI._largeIconSizePX=Icon.baseSizePX<<1;AppUI._thinBorderPX=AppUI.baseThinBorderPX;AppUI._thickBorderPX=AppUI.baseThickBorderPX;
AppUI.directoryCancelled=!1;
class App{static mainWindowStateChanged(a,b,c){if(App.isMinimized!==b||App.isMaximized!==c)App.isMinimized=b,App.isMaximized=c,b||(App.titleBar&&(App.titleBar.className=c?"maximized":""),App.iconRestore&&(App.iconRestore.className=c?"":"hidden"),App.iconMaximize&&(App.iconMaximize.className=c?"hidden":""))}static mainWindowClosing(){if(App.player&&App.player.alive){const a=App.closeHandlers,b=[],c=App.player.playlist;InternalStorage.saveAppSettings({devicePixelRatio,playerVolume:App.player.volume,graphicalFilterControlEnabled:App.player.graphicalFilterControl.enabled,
graphicalFilterControlSimpleMode:App.player.graphicalFilterControl.simpleMode});App.player.destroy(!0);if(a&&a.length)for(let d=a.length-1;0<=d;d--)if(a[d]){const e=a[d]();e&&b.push(e)}App.electron&&(c&&b.push(InternalStorage.savePlaylist(c)),b.length?Promise.all(b).then(App.destroy,App.destroy):App.destroy())}}static destroy(){App.player&&App.player.destroy();AppUI.destroy();const a=App.ipcRenderer;zeroObject(App);a&&a.invoke("mainWindowFinalClose")}static updateLoadingIcon(){App.iconLoading&&(App.iconLoading.style.display=
