...
a,b&&b(AppUI.topMessage),AppUI.topMessage.style.visibility="visible",AppUI.topMessageTimeout=setTimeout(function(){0>=AppUI.topMessageFading||1!==AppUI.topMessageFading||(AppUI.topMessageFading=2,AppUI.topMessage.classList.add("in"),AppUI.topMessageTimeout=setTimeout(function(){2===AppUI.topMessageFading&&(AppUI.topMessageFading=0,AppUI.topMessageTimeout=0)},320))},20))}static addFiles(a){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading){var b=App.hostType===App.hostTypeElectron,
c=b?yield App.showOpenFileDialog():yield App.showOpenDialogWeb(a);if(c&&App.player)try{AppUI._loading=!0;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const d=App.player.playlist,e=b?null:new Uint8Array(BufferedFileHandle.minBufferLength<<1),f=b?null:[new Uint8Array(256)];let g=!1;for(let h=0;h<c.length&&App.player;h++)if(b)yield d.addSong(c[h]);else{const k=d.length;(yield d.addSongWeb(c[h],e,f))&&k===d.length&&(g=!0)}g&&AppUI.playlistControl&&
AppUI.playlistControl.refreshVisibleItems()}catch(d){Modal.show({text:"addFiles error: "+(d.message||d.toString())})}finally{AppUI._loading=!1,App.updateLoadingIcon()}}})}static addDirectory(a){return __awaiter(this,void 0,void 0,function*(){const b=yield App.listDirectory(a);if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;if(!b)return!0;for(let c=0;c<b.length;c++)if(b[c].isDirectory){if(!(yield AppUI.addDirectory(b[c].fileURL)))return!1}else{if(!App.player||!App.player.playlist||
AppUI.directoryCancelled)return!1;yield App.player.playlist.addSong(b[c])}return!0})}static addDirectories(){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading){if(App.hostType!==App.hostTypeElectron)return AppUI.addFiles(!0);var a=yield App.showOpenDirectoryDialog();if(a&&App.player)try{AppUI._loading=!0;AppUI.directoryCancelled=!1;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());for(let b=0;b<a.length&&(yield AppUI.addDirectory(a[b]));b++);
}catch(b){Modal.show({text:"addDirectories error: "+(b.message||b.toString())})}finally{AppUI._loading=!1,AppUI.directoryCancelled=!1,App.updateLoadingIcon()}}})}static toggleDeleteMode(a){AppUI.playlistControl&&(AppUI.playlistControl.deleteMode?(AppUI.playlistControl.deleteMode=!1,AppUI.hideTopMessage(),a&&HistoryHandler.popState()):(AppUI.playlistControl.deleteMode=!0,AppUI.showTopMessage(Strings.DeleteModeHTML,function(b){const c=document.createElement("div");ButtonControl.create({color:"red",
icon:"icon-delete-all",stringKey:"DeleteAllSongs",parent:c,onclick:function(){App.player&&App.player.playlist&&App.player.playlist.clear();AppUI.playlistControl&&AppUI.playlistControl.deleteMode&&AppUI.toggleDeleteMode(!0)}});ButtonControl.create({color:"green",icon:"icon-check",stringKey:"Done",parent:c,onclick:function(){AppUI.playlistControl&&AppUI.playlistControl.deleteMode&&AppUI.toggleDeleteMode(!0)}});b.appendChild(c)}),HistoryHandler.pushState()))}static switchView(){AppUI.panelContainer&&
...
AppUI.scrollbarSizeREM=AppUI.primaryInputIsTouch?5:3;AppUI.minWidthREM=216;AppUI.minWidthPX=AppUI.remToPX(AppUI.minWidthREM);AppUI.baseThinBorderPX=1;AppUI.baseThickBorderPX=2;AppUI.zoomHandlers=[];AppUI.rootVariables=document.getElementById("root-variables");AppUI.panelContainer=document.getElementById("panel-container");AppUI.fixedPanel=document.getElementById("fixed-panel");AppUI.optionalPanel=document.getElementById("optional-panel");AppUI.iconPlay=document.getElementById("i-play");
AppUI.iconPause=document.getElementById("i-pause");AppUI.artistLabel=document.getElementById("artist-label");AppUI.titleLabel=document.getElementById("title-label");AppUI.topMessage=document.getElementById("top-message");AppUI._loading=!1;AppUI.devicePixelRatio=-1;AppUI._smallIconSizePX=Icon.baseSizePX;AppUI._largeIconSizePX=Icon.baseSizePX<<1;AppUI._thinBorderPX=AppUI.baseThinBorderPX;AppUI._thickBorderPX=AppUI.baseThickBorderPX;AppUI.directoryCancelled=!1;AppUI.topMessageFading=0;
AppUI.topMessageTimeout=0;
class App{static mainWindowStateChanged(a,b,c){if(App.isMinimized!==b||App.isMaximized!==c)App.isMinimized=b,App.isMaximized=c,b||(App.titleBar&&(App.titleBar.className=c?"maximized":""),App.iconRestore&&(App.iconRestore.className=c?"":"hidden"),App.iconMaximize&&(App.iconMaximize.className=c?"hidden":""))}static mainWindowClosing(){if(App.player&&App.player.alive){const a=App.closeHandlers,b=[],c=App.player.playlist;InternalStorage.saveAppSettings({devicePixelRatio,playerVolume:App.player.volume,graphicalFilterControlEnabled:App.graphicalFilterControl?
App.graphicalFilterControl.enabled:!1,graphicalFilterControlSimpleMode:App.graphicalFilterControl?App.graphicalFilterControl.simpleMode:!0,stereoPannerControlEnabled:App.stereoPannerControl?App.stereoPannerControl.enabled:!1,monoDownMixerControlEnabled:App.monoDownMixerControl?App.monoDownMixerControl.enabled:!1});App.player.destroy(!0);App.graphicalFilterControl&&App.graphicalFilterControl.saveSettings();App.stereoPannerControl&&App.stereoPannerControl.saveSettings();if(a&&a.length)for(let d=a.length-
1;0<=d;d--)if(a[d]){const e=a[d]();e&&b.push(e)}c&&InternalStorage.savePlaylistWeb(c);App.ipcRenderer&&(b.length?Promise.all(b).then(App.destroy,App.destroy):App.destroy())}}static destroy(){App.player&&App.player.destroy();AppUI.destroy();const a=App.ipcRenderer;zeroObject(App);a&&a.invoke("mainWindowFinalClose")}static updateLoadingIcon(){App.iconLoading&&(App.iconLoading.style.display=App.loading?"":"none")}static get loading(){return(App.player?App.player.loading:!1)||AppUI.loading||App._loading}static set loading(a){App._loading!==
a&&(App._loading=a,App.updateLoadingIcon())}static get installPromptReady(){return!!App.installPrompt}static showInstallPrompt(){const a=App.installPrompt;if(a&&a.prompt)try{App.installPrompt=null,a.prompt()}catch(b){}}static beforeInstallPrompt(a){"preventDefault"in a&&a.preventDefault();App.installPrompt=a}static adjustCover(){const a=document.getElementById("cover");if(a){const b=document.body.getBoundingClientRect();a.style.transform=`scale(${Math.ceil(.5*b.right)}, ${Math.ceil(.5*b.bottom)})`}}static sortFiles(a){a.length&&
(a[0]["data-path"]?a.sort(function(b,c){return b["data-path"].localeCompare(c["data-path"])}):a[0].webkitRelativePath?a.sort(function(b,c){return b.webkitRelativePath.localeCompare(c.webkitRelativePath)}):a.sort(function(b,c){return b.name.localeCompare(c.name)}));return a}static preInit(){Strings.init();Icon.init();GraphicalFilterEditorStrings.init(Strings.language);Strings.toFixed=GraphicalFilterEditorStrings.toFixed;const a=InternalStorage.loadAppSettings(),b=window.electronWebFrame;App.ipcRenderer&&
