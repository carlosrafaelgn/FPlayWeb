...
c||c>=g?e.appendChild(document.createTextNode(Strings.translate(f))):(b=f.substring(b,c),c=f.substring(c+1,g),e.setAttribute(b,Strings.translate(c)));b=g+1}while(b<f.length)}}}static changeText(a,b){if(a){null===b&&(b="");var c=a.lastChild;c?c.nodeValue=b:a.appendChild(document.createTextNode(b))}}}Strings.language="en";Strings.DecimalSeparator=".";Strings.OppositeDecimalSeparator=",";Strings.Oops="Oops\u2026";Strings.AppName="FPlay";Strings.Menu="Menu";Strings.About="About";Strings.Edit="Edit";
...
(App.player.monoDownMixerControl.enabled=AppUI.monoDownMixerControlEnabled.checked)}static addFiles(a){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading){var b=App.hostType===App.hostTypeElectron,c=b?yield App.showOpenFileDialog():yield App.showOpenDialogWeb(a);if(c&&App.player)try{AppUI._loading=!0;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const d=App.player.playlist,e=b?null:new Uint8Array(BufferedFileHandle.minBufferLength<<
1),f=b?null:[new Uint8Array(256)];let g=!1;for(let h=0;h<c.length&&App.player;h++)if(b)yield d.addSong(c[h]);else{const k=d.length;(yield d.addSongWeb(c[h],e,f))&&k===d.length&&(g=!0)}g&&AppUI.playlistControl&&AppUI.playlistControl.refreshVisibleItems()}catch(d){Modal.show({text:"addFiles error: "+(d.message||d.toString())})}finally{AppUI._loading=!1,App.updateLoadingIcon()}}})}static addDirectory(a){return __awaiter(this,void 0,void 0,function*(){const b=yield App.listDirectory(a);if(!App.player||
!App.player.playlist||AppUI.directoryCancelled)return!1;if(!b)return!0;for(let c=0;c<b.length;c++)if(b[c].isDirectory){if(!(yield AppUI.addDirectory(b[c].fileURL)))return!1}else{if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;yield App.player.playlist.addSong(b[c])}return!0})}static addDirectories(){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading){if(App.hostType!==App.hostTypeElectron)return AppUI.addFiles(!0);var a=yield App.showOpenDirectoryDialog();
if(a&&App.player)try{AppUI._loading=!0;AppUI.directoryCancelled=!1;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());for(let b=0;b<a.length&&(yield AppUI.addDirectory(a[b]));b++);}catch(b){Modal.show({text:"addDirectories error: "+(b.message||b.toString())})}finally{AppUI._loading=!1,AppUI.directoryCancelled=!1,App.updateLoadingIcon()}}})}static switchView(){AppUI.panelContainer&&AppUI.panelContainer.classList.toggle("toggled")}static showAbout(){Modal.show({html:Strings.AboutHTML+
