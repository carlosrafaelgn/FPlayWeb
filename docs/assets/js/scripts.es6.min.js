...
Strings.AdvancedFilter="Advanced Filter";Strings.TraditionalFilter="Traditional Filter";Strings.OK="OK";Strings.Cancel="Cancel";Strings.Clear="Clear";Strings.Back="Back";Strings.Close="Close";Strings.Refresh="Refresh";Strings.Exit="Exit";Strings.Previous="Previous";Strings.Play="Play";Strings.Pause="Pause";Strings.PlayPause="Play / Pause";Strings.Stop="Stop";Strings.Next="Next";Strings.Volume="Volume";Strings.UnknownError="Unknown error";Strings.AddFiles="Add Files";Strings.AddFolders="Add Folders";
Strings.ShowFilter="Show Filter";Strings.ShowPlaylist="Show Playlist";Strings.UpdateAvailable="Update Available!";Strings.PleaseRefresh="Please, refresh the page to update the app. \ud83d\ude0a";Strings.AboutHTML='FPlay for web is an experimental audio player. \ud83d\ude0a<br />\n<br />\nFor more information about the project, its source code and dependencies, check out its repository at <a target="_blank" href="https://github.com/carlosrafaelgn/FPlayWeb">github.com/carlosrafaelgn/FPlayWeb</a>.<br />\n<br />\nThis project is licensed under the <a target="_blank" href="https://github.com/carlosrafaelgn/FPlayWeb/blob/master/LICENSE">MIT License</a>.';
...
class StereoPannerControl extends Filter{constructor(a,b,c,d){super(d);this._enabled=!!c;this._pan=InternalStorage.loadStereoPannerSettings();this.audioContext=b;this.channelSplitter=b.createChannelSplitter(2);this.gain=b.createGain();this.channelMerger=b.createChannelMerger(2);this.rightGain=this.leftGain=!1;b=this.commitChanges.bind(this);this.slider=new SliderControl(a,!0,!1,-StereoPannerControl.maxAbsoluteValue,StereoPannerControl.maxAbsoluteValue,this._pan);this.slider.ondragended=b;this.slider.onkeyboardchanged=
b;this.commitChanges(this._pan)}commitChanges(a){const b=this.audioContext,c=this.channelSplitter,d=this.gain,e=this.channelMerger;if(c&&d&&e)if(this._pan=a){const f=0<a,g=!f;a=Math.abs(a);d.gain.setValueAtTime(a>=StereoPannerControl.maxAbsoluteValue?0:Math.pow(10,-a/20),b.currentTime+.1);if(f!==this.leftGain||g!==this.rightGain)c.disconnect(),d.disconnect(),this.leftGain=f,this.rightGain=g,f?(c.connect(d,0,0),d.connect(e,0,0),c.connect(e,1,1)):(c.connect(d,1,0),d.connect(e,0,1),c.connect(e,0,0))}else this.rightGain=
this.leftGain=!1,d.disconnect(),c.disconnect(),c.connect(e,0,0),c.connect(e,1,1)}get enabled(){return this._enabled}set enabled(a){this._enabled!==a&&(this._enabled=a,this.channelSplitter&&this.filterChangedCallback&&this.filterChangedCallback())}get pan(){return this._pan}set pan(a){a=Math.max(-StereoPannerControl.maxAbsoluteValue,Math.min(StereoPannerControl.maxAbsoluteValue,a|0));this._pan!==a&&(this.slider.value=a,this.commitChanges(a))}saveSettings(){InternalStorage.saveStereoPannerSettings(this._pan)}get inputNode(){return this.channelSplitter}get outputNode(){return this.channelMerger}destroy(){this.channelSplitter&&
(super.destroy(),zeroObject(this))}}StereoPannerControl.maxAbsoluteValue=20;var LibMikModMessageId;(function(a){a[a.INIT=1]="INIT";a[a.LOAD_MODULE_BUFFER=2]="LOAD_MODULE_BUFFER";a[a.CHANGE_GENERAL_OPTIONS=3]="CHANGE_GENERAL_OPTIONS";a[a.STOP_MODULE=4]="STOP_MODULE";a[a.PLAYBACK_ERROR=5]="PLAYBACK_ERROR";a[a.PLAYBACK_ENDED=6]="PLAYBACK_ENDED"})(LibMikModMessageId||(LibMikModMessageId={}));
...
class CheckboxControl{static init(){const a=document.querySelectorAll("input[type=checkbox]");if(a)for(let b=a.length-1;0<=b;b--)CheckboxControl.prepare(a[b])}static create(a){var b=document.createElement("input");b.setAttribute("type","checkbox");b.setAttribute("value",a.stringKey);a.id&&b.setAttribute("id",a.id);a.className&&(b.className=a.className);b=CheckboxControl.prepare(b);a.parent&&a.parent.appendChild(b);return b}static prepare(a){const b=document.createElement("button"),c=document.createElement("span"),
d=a.parentNode,e=a.getAttribute("title")||Strings.translate(a.value);b.setAttribute("type","button");b.setAttribute("role","checkbox");b.setAttribute("aria-label",e);c.setAttribute("tabindex","-1");a.setAttribute("tabindex","-1");a.setAttribute("aria-label",e);b.className=a.className;a.className="";d&&(d.insertBefore(b,a),d.removeChild(a));b.appendChild(c);c.appendChild(a);c.appendChild(Icon.createLarge("icon-checkbox-0","margin"));c.appendChild(Icon.createLarge("icon-checkbox-1","margin orange"));
c.appendChild(document.createTextNode(e));a.removeAttribute("value");a.removeAttribute("title");b.onclick=CheckboxControl.click;return b}static click(a){(a=(a.target||this).getElementsByTagName("input"))&&a[0]&&a[0].click()}}
...
break;case 3:e.innerHTML="500<br/>1k";break;case 4:e.innerHTML="2k<br/>4k";break;case 5:e.innerHTML="<br/>8k";break;case 6:e.innerHTML="<br/>16k"}f.textContent=SimpleFilterEditorControl.sliderToDB(g);f.className="small-top-margin";d.className="simple-filter-slider";a.appendChild(d);b[c]=new SliderControl(d,!0,!0,-30,30,g,f,e);this.createHandler(b[c],c)}}static sliderToDB(a){return Formatter.formatDB(.5*a)}static sliderToY(a){return GraphicalFilterEditor.zeroChannelValueY-(a<<1)}static yToSlider(a){return Math.max(-30,
Math.min(30,-(a-GraphicalFilterEditor.zeroChannelValueY)>>1))}filterToSlider(a){return SimpleFilterEditorControl.yToSlider(this.editor.getShelfZoneY(a))}createHandler(a,b){const c=this.editor;a.onvaluechanged=function(e){a.leftChild&&(a.leftChild.textContent=SimpleFilterEditorControl.sliderToDB(e))};const d=function(e){c.changeShelfZoneY(b,SimpleFilterEditorControl.sliderToY(e));c.commitChanges()};a.ondragended=d;a.onkeyboardchanged=d}updateSliders(){const a=this.sliders;for(let b=0;b<a.length;b++)a[b].value=
this.filterToSlider(b)}}
class GraphicalFilterControl{constructor(a,b,c,d,e){this.filterChangedCallback=e;this._enabled=!!c;this._simpleMode=!!d;this.container=a;c=document.createElement("div");this._simpleMode||a.appendChild(c);d=InternalStorage.loadGraphicalFilterEditorSettings();this._simpleMode&&(d.editMode=GraphicalFilterEditorControl.editModeShelfEq);this.editor=new GraphicalFilterEditorControl(c,2048,b,this.filterChanged.bind(this),d,{svgRenderer:!0,radioHTML:Icon.createHTML("icon-radio",!1,"menu-icon"),checkHTML:Icon.createHTML("icon-check",
!1,"menu-icon"),menuWidth:Icon.outerSizeREM+"rem",menuPadding:"0",openMenuHTML:"<span>"+Icon.createHTML("icon-expand-less",!0)+"</span>",closeMenuHTML:"<span>"+Icon.createHTML("icon-expand-more",!0)+"</span>"});c=document.createElement("div");c.className="bottom-margin";this._simpleMode&&a.appendChild(c);this.simpleEditor=new SimpleFilterEditorControl(c,this.editor);a=this.editor.element.querySelectorAll("div.GELBL");for(b=a.length-1;0<=b;b--)c=a[b],c.style.height="var(--button-size)",c.style.lineHeight=
"6rem",c.style.paddingTop="var(--button-padding-top)",c.style.paddingBottom="0";a=this.editor.element.querySelector("div.GEBTN.GECLK");a.className="btn square white transparent";a.style.width="";a.style.padding="";a.style.float="right"}filterChanged(){this.filterChangedCallback&&this.filterChangedCallback()}get enabled(){return this._enabled}set enabled(a){this._enabled!==a&&(this._enabled=a,this.filterChangedCallback&&this.editor&&this.editor.filter&&this.filterChangedCallback())}get simpleMode(){return this._simpleMode}set simpleMode(a){if(this._simpleMode!==
a){this._simpleMode=a;if(a){this.editor.editMode=GraphicalFilterEditorControl.editModeShelfEq;this.simpleEditor.updateSliders();a=this.editor.element;var b=this.simpleEditor.element}else a=this.simpleEditor.element,b=this.editor.element;a&&a.parentNode&&a.parentNode.removeChild(a);this.container&&b&&!b.parentNode&&this.container.appendChild(b);this.filterChangedCallback&&this.editor&&this.editor.filter&&this.filterChangedCallback()}}saveSettings(){this.editor&&InternalStorage.saveGraphicalFilterEditorSettings(this.editor.saveSettings())}}
GraphicalFilterControl.defaultPresets={Powerful:""};
...
class Player{constructor(a,b,c,d,e,f){this.audioContextTimeout=0;this.audioContextSuspended=!0;this.audioContext=window.AudioContext?new window.AudioContext({latencyHint:"playback"}):new window.webkitAudioContext({latencyHint:"playback"});this.audioContext.suspend();this.audioContext.onstatechange=this.audioContextStateChanged.bind(this);this.graphicalFilterControl=new GraphicalFilterControl(a,this.audioContext,d,e,this.filterChanged.bind(this));this.stereoPannerControl=new StereoPannerControl(b,
this.audioContext,f,this.stereoPannerChanged.bind(this));this.onerror=this.oncurrenttimeschanged=this.onpausedchanged=this.onloadingchanged=this.onsongchanged=null;this.boundPlaybackError=this.playbackError.bind(this);this.boundNotifySongChanged=this.notifySongChanged.bind(this);this.boundCheckAudioContext=this.checkAudioContext.bind(this);this.audio=new Audio;this.audio.loop=!1;this.audio.controls=!1;this.audio.volume=1;a=this.playbackLoadStarted.bind(this);this.audio.onwaiting=a;this.audio.onloadstart=
a;this.audio.oncanplay=this.playbackLoadEnded.bind(this);this.audio.onended=this.playbackEnded.bind(this);this.audio.onerror=this.boundPlaybackError;this.audio.onpause=this.playbackPaused.bind(this);a=this.playbackStarted.bind(this);this.audio.onplay=a;this.audio.onplaying=a;a=this.playbackAborted.bind(this);this.audio.onabort=a;this.audio.onemptied=a;this.audio.ondurationchange=this.playbackLengthChanged.bind(this);this.audio.ontimeupdate=this.currentTimeChanged.bind(this);this.source=this.audioContext.createMediaElementSource(this.audio);
this._alive=!0;this._loading=!1;this._paused=!0;this._currentSong=this._playlist=null;this._volume=Player.maxVolume;this.lastObjectURL=null;this.volume=void 0===c?Player.maxVolume:c;this.filterOutput=this.source;this.stereoPannerInput=this.audioContext.destination;this.filterChanged();this.stereoPannerChanged();(this.mediaSession=c="mediaSession"in navigator&&"MediaMetadata"in window?navigator.mediaSession:null)&&"setActionHandler"in c&&(a=this.playPause.bind(this),c.setActionHandler("previoustrack",
this.previous.bind(this)),c.setActionHandler("seekbackward",this.seekBackward.bind(this)),c.setActionHandler("pause",a),c.setActionHandler("play",a),c.setActionHandler("seekforward",this.seekForward.bind(this)),c.setActionHandler("nexttrack",this.next.bind(this)))}destroy(a){this.stop();a?(this._alive=!1,this.graphicalFilterControl&&this.graphicalFilterControl.saveSettings(),this.stereoPannerControl&&this.stereoPannerControl.saveSettings()):(this._playlist&&this._playlist.destroy(),(a=this.mediaSession)&&
"setActionHandler"in a&&(a.setActionHandler("previoustrack",null),a.setActionHandler("seekbackward",null),a.setActionHandler("pause",null),a.setActionHandler("play",null),a.setActionHandler("seekforward",null),a.setActionHandler("nexttrack",null)),zeroObject(this))}filterChanged(){if(this._alive){var a=this.source,b=this.stereoPannerInput;this.filterOutput=a;this.graphicalFilterControl.enabled&&this.graphicalFilterControl.editor.filter.connectSourceAndDestination(a,b)?this.filterOutput=this.graphicalFilterControl.editor.filter.outputNode:
(a.disconnect(),this.graphicalFilterControl.editor.filter.disconnectSourceAndDestination(),a.connect(b,0,0))}}stereoPannerChanged(){if(this._alive){var a=this.filterOutput,b=this.audioContext.destination;this.stereoPannerInput=b;this.stereoPannerControl.enabled&&this.stereoPannerControl.connectSourceAndDestination(a,b)?this.stereoPannerInput=this.stereoPannerControl.inputNode:(a.disconnect(),this.stereoPannerControl.disconnectSourceAndDestination(),a.connect(b,0,0))}}get alive(){return this._alive}get loading(){return this._loading}get paused(){return this._paused}get volume(){return this._volume}set volume(a){a|=
...
this.audio.duration:0>=a?0:a)}}Player.maxVolume=40;Player.nop=function(){};
...
if(b)return b}}catch(a){}return{playerVolume:100,graphicalFilterControlEnabled:!1,graphicalFilterControlSimpleMode:!1,stereoPannerControlEnabled:!1}}static saveAppSettings(a){a?localStorage.setItem("appSettings",JSON.stringify(a)):localStorage.removeItem("appSettings")}static loadGraphicalFilterEditorSettings(){try{const a=localStorage.getItem("graphicalFilterEditorSettings");if(a){const b=JSON.parse(a);if(b)return b}}catch(a){}return{currentChannelIndex:0,editMode:GraphicalFilterEditorControl.editModeShelfEq,
isActualChannelCurveNeeded:!0,isNormalized:!1,isSameFilterLR:!0,showZones:!0}}static saveGraphicalFilterEditorSettings(a){a?localStorage.setItem("graphicalFilterEditorSettings",JSON.stringify(a)):localStorage.removeItem("graphicalFilterEditorSettings")}static loadStereoPannerSettings(){return Math.max(-StereoPannerControl.maxAbsoluteValue,Math.min(StereoPannerControl.maxAbsoluteValue,parseInt(localStorage.getItem("stereoPannerSettings"))|0))}static saveStereoPannerSettings(a){localStorage.setItem("stereoPannerSettings",
...
(App.player.stereoPannerControl.enabled=AppUI.stereoPannerControlEnabled.checked)}static addFiles(a){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading){var b=App.electron?yield App.showOpenFileDialog():yield App.showOpenDialogWeb(a);if(b&&App.player)try{AppUI._loading=!0;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const c=App.electron?null:new Uint8Array(BufferedFileHandle.minBufferLength<<1),d=App.electron?null:
[new Uint8Array(256)];for(let e=0;e<b.length&&App.player&&App.player.playlist;e++)App.electron?yield App.player.playlist.addSong(b[e]):yield App.player.playlist.addSongWeb(b[e],c,d)}catch(c){alert("addFiles error: "+(c.message||c.toString()))}finally{AppUI._loading=!1,App.updateLoadingIcon()}}})}static addDirectory(a){return __awaiter(this,void 0,void 0,function*(){const b=yield App.listDirectory(a);if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;if(!b)return!0;for(let c=0;c<
b.length;c++)if(b[c].isDirectory){if(!(yield AppUI.addDirectory(b[c].fileURL)))return!1}else{if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;yield App.player.playlist.addSong(b[c])}return!0})}static addDirectories(){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading){if(!App.electron)return AppUI.addFiles(!0);var a=yield App.showOpenDirectoryDialog();if(a&&App.player)try{AppUI._loading=!0;AppUI.directoryCancelled=!1;App.updateLoadingIcon();App.player.playlist||
(App.player.playlist=new Playlist,AppUI.preparePlaylist());for(let b=0;b<a.length&&(yield AppUI.addDirectory(a[b]));b++);}catch(b){alert("addDirectories error: "+(b.message||b.toString()))}finally{AppUI._loading=!1,AppUI.directoryCancelled=!1,App.updateLoadingIcon()}}})}static switchView(){AppUI.panelContainer&&AppUI.panelContainer.classList.toggle("toggled")}static showAbout(){Modal.show({html:Strings.AboutHTML,title:Strings.About+" (v"+window.CACHE_VERSION+")"})}}
AppUI.primaryInputIsTouch="matchMedia"in window?window.matchMedia("(pointer: coarse)").matches:0<navigator.maxTouchPoints||0<navigator.msMaxTouchPoints;AppUI.rem=4;AppUI.buttonRegularSizeREM=8;AppUI.buttonRegularSizePX=AppUI.remToPX(AppUI.buttonRegularSizeREM);AppUI.buttonSizeREM=AppUI.buttonRegularSizeREM*(AppUI.primaryInputIsTouch?1.5:1);AppUI.buttonSizePX=AppUI.remToPX(AppUI.buttonSizeREM);AppUI.optionalPanelClientWidthREM=128;AppUI.optionalPanelClientWidthPX=AppUI.remToPX(AppUI.optionalPanelClientWidthREM);
AppUI.scrollbarSizeREM=AppUI.primaryInputIsTouch?5:3;AppUI.minWidthREM=216;AppUI.minWidthPX=AppUI.remToPX(AppUI.minWidthREM);AppUI.baseThinBorderPX=1;AppUI.baseThickBorderPX=2;AppUI.zoomHandlers=[];AppUI.rootVariables=document.getElementById("root-variables");AppUI.panelContainer=document.getElementById("panel-container");AppUI.fixedPanel=document.getElementById("fixed-panel");AppUI.optionalPanel=document.getElementById("optional-panel");AppUI.iconPlay=document.getElementById("icon-play");
AppUI.iconPause=document.getElementById("icon-pause");AppUI.artistLabel=document.getElementById("artist-label");AppUI.titleLabel=document.getElementById("title-label");AppUI._loading=!1;AppUI.devicePixelRatio=-1;AppUI._smallIconSizePX=Icon.baseSizePX;AppUI._largeIconSizePX=Icon.baseSizePX<<1;AppUI._thinBorderPX=AppUI.baseThinBorderPX;AppUI._thickBorderPX=AppUI.baseThickBorderPX;AppUI.directoryCancelled=!1;
...
graphicalFilterControlEnabled:App.player.graphicalFilterControl.enabled,graphicalFilterControlSimpleMode:App.player.graphicalFilterControl.simpleMode,stereoPannerControlEnabled:App.player.stereoPannerControl.enabled});App.player.destroy(!0);if(a&&a.length)for(let d=a.length-1;0<=d;d--)if(a[d]){const e=a[d]();e&&b.push(e)}App.electron&&(c&&b.push(InternalStorage.savePlaylist(c)),b.length?Promise.all(b).then(App.destroy,App.destroy):App.destroy())}}static destroy(){App.player&&App.player.destroy();
AppUI.destroy();const a=App.ipcRenderer;zeroObject(App);a&&a.invoke("mainWindowFinalClose")}static updateLoadingIcon(){App.iconLoading&&(App.iconLoading.style.display=App.loading?"":"none")}static get loading(){return(App.player?App.player.loading:!1)||AppUI.loading||App._loading}static set loading(a){App._loading!==a&&(App._loading=a,App.updateLoadingIcon())}static get installPromptReady(){return!!App.installPrompt}static showInstallPrompt(){const a=App.installPrompt;if(a&&a.prompt)try{App.installPrompt=
null,a.prompt()}catch(b){}}static beforeInstallPrompt(a){"preventDefault"in a&&a.preventDefault();App.installPrompt=a}static adjustCover(){const a=document.getElementById("cover");if(a){const b=document.body.getBoundingClientRect();a.style.transform=`scale(${Math.ceil(.5*b.right)}, ${Math.ceil(.5*b.bottom)})`}}static sortFiles(a){a.length&&(a[0].webkitRelativePath?a.sort(function(b,c){return b.webkitRelativePath.localeCompare(c.webkitRelativePath)}):a.sort(function(b,c){return b.name.localeCompare(c.name)}));
return a}static preInit(){Strings.init();Icon.init();GraphicalFilterEditorStrings.init(Strings.language);Strings.toFixed=GraphicalFilterEditorStrings.toFixed;const a=InternalStorage.loadAppSettings(),b=window.electronWebFrame;delete window.electronIpcRenderer;delete window.electronWebFrame;if("serviceWorker"in navigator){window.addEventListener("beforeinstallprompt",App.beforeInstallPrompt);var c=navigator.serviceWorker.register("sw.js");c&&"then"in c&&c.then(function(d){d&&"onupdatefound"in d&&d.active&&
(d.onupdatefound=function(){Modal.show({html:Strings.PleaseRefresh,title:Strings.UpdateAvailable,oktext:Strings.Refresh,okcancel:!0,onok:function(){window.location.reload()}})})})}window.addEventListener("resize",App.adjustCover);App.adjustCover();App.ipcRenderer?(App.ipcRenderer.on("mainWindowStateChanged",App.mainWindowStateChanged),App.ipcRenderer.on("mainWindowClosing",App.mainWindowClosing),App.triggerMainWindowStateChanged()):window.addEventListener("onpagehide"in window?"pagehide":"unload",
App.mainWindowClosing);App.frameless&&(App.titleBar=document.createElement("h1"),App.titleBar.innerHTML=`
				<div></div>
				<i id="icon-main"></i>
				FPlay
