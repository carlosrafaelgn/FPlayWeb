...
c||c>=g?e.appendChild(document.createTextNode(Strings.translate(f))):(b=f.substring(b,c),c=f.substring(c+1,g),e.setAttribute(b,Strings.translate(c)));b=g+1}while(b<f.length)}}}}Strings.language="en";Strings.DecimalSeparator=".";Strings.OppositeDecimalSeparator=",";Strings.Oops="Oops\u2026";Strings.Menu="Menu";Strings.About="About";Strings.Edit="Edit";Strings.Delete="Delete";Strings.Enable="Enable";Strings.Disable="Disable";Strings.Enabled="Enabled";Strings.Disabled="Disabled";Strings.OK="OK";
Strings.Cancel="Cancel";Strings.Clear="Clear";Strings.Back="Back";Strings.Close="Close";Strings.Refresh="Refresh";Strings.Exit="Exit";Strings.Previous="Previous";Strings.Play="Play";Strings.Pause="Pause";Strings.PlayPause="Play / Pause";Strings.Stop="Stop";Strings.Next="Next";Strings.Volume="Volume";Strings.UnknownError="Unknown error";Strings.AddFiles="Add Files";Strings.AddFolders="Add Folders";Strings.ShowFilter="Show Filter";Strings.ShowPlaylist="Show Playlist";Strings.UpdateAvailable="Update Available!";
Strings.PleaseRefresh="Please, refresh the page to update the app. \ud83d\ude0a";Strings.AboutHTML='FPlay for web is an experimental audio player. \ud83d\ude0a<br />\n<br />\nFor more information about the project, its source code and dependencies, check out its repository at <a target="_blank" href="https://github.com/carlosrafaelgn/FPlayWeb">github.com/carlosrafaelgn/FPlayWeb</a>.<br />\n<br />\nThis project is licensed under the <a target="_blank" href="https://github.com/carlosrafaelgn/FPlayWeb/blob/master/LICENSE">MIT License</a>.';
...
Icon.createHTML("icon-expand-more",!0)+"</span>"});a=this.editor.element.querySelectorAll("div.GELBL");for(b=a.length-1;0<=b;b--)c=a[b],c.style.height="var(--button-size)",c.style.lineHeight="6rem",c.style.paddingTop="var(--button-padding-top)",c.style.paddingBottom="0";a=this.editor.element.querySelector("div.GEBTN.GECLK");a.className="btn square white transparent";a.style.width="";a.style.padding="";a.style.float="right"}filterChanged(){this.filterChangedCallback&&this.filterChangedCallback()}get enabled(){return this._enabled}set enabled(a){this._enabled!==
...
!this._element)return!1;this.focusContainer.focus();this.container.classList.add("active");this.mouseMove(a);return!0}mouseMove(a){const b=this.innerContainer.getBoundingClientRect();let c;this.vertical?(c=b.bottom-b.top,a=c-(a.clientY-(b.top-3))):(c=b.right-b.left,a=a.clientX-(b.left+4));this.value=0>=c?this._min:this._min+this.delta*a/c}mouseUp(a){this.container.classList.remove("active");if(this.ondragended)this.ondragended(this._value)}keyDown(a){switch(a.key){case "ArrowDown":case "ArrowLeft":this.value=
this._value-1;break;case "ArrowUp":case "ArrowRight":this.value=this._value+1;break;case "PageDown":this.value=this._value-.1*this.delta;break;case "PageUp":this.value=this._value+.1*this.delta}}focus(){this.container.classList.add("active")}blur(){this.container.classList.remove("active")}}
class Player{constructor(a,b,c){this.audioContextTimeout=0;this.audioContextSuspended=!0;this.audioContext=window.AudioContext?new window.AudioContext({latencyHint:"playback"}):new window.webkitAudioContext({latencyHint:"playback"});this.audioContext.suspend();this.audioContext.onstatechange=this.audioContextStateChanged.bind(this);this.graphicalFilterControl=new GraphicalFilterControl(a,this.audioContext,c);this.graphicalFilterControl.filterChangedCallback=this.filterChanged.bind(this);this.onerror=
...
if(b)return b}}catch(a){}return{playerVolume:100,graphicalFilterControlEnabled:!1}}static saveAppSettings(a){a?localStorage.setItem("appSettings",JSON.stringify(a)):localStorage.removeItem("appSettings")}static loadGraphicalFilterEditorSettings(){try{const a=localStorage.getItem("graphicalFilterEditorSettings");if(a){const b=JSON.parse(a);if(b)return b}}catch(a){}return{currentChannelIndex:0,editMode:GraphicalFilterEditorControl.editModeZones,isActualChannelCurveNeeded:!0,isNormalized:!1,isSameFilterLR:!0,
...
b,new Response(a.serialize().trimmedArrayBuffer))})}}InternalStorage.cachePrefix="https://fplay.com.br/";InternalStorage.cacheNamePlaylists="playlists";
...
b){App.player&&App.player.play(b)}static playlistControlItemContextMenu(a,b){App.player&&App.player.play(b)}static graphicalFilterControlEnabledClicked(){App.player&&(App.player.graphicalFilterControl.enabled=AppUI.graphicalFilterControlEnabled.checked)}static addFiles(a){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading){var b=App.electron?yield App.showOpenFileDialog():yield App.showOpenDialogWeb(a);if(b&&App.player)try{AppUI._loading=!0;App.updateLoadingIcon();App.player.playlist||
(App.player.playlist=new Playlist,AppUI.preparePlaylist());const c=App.electron?null:new Uint8Array(BufferedFileHandle.minBufferLength<<1),d=App.electron?null:[new Uint8Array(256)];for(let e=0;e<b.length&&App.player&&App.player.playlist;e++)App.electron?yield App.player.playlist.addSong(b[e]):yield App.player.playlist.addSongWeb(b[e],c,d)}catch(c){alert("addFiles error: "+(c.message||c.toString()))}finally{AppUI._loading=!1,App.updateLoadingIcon()}}})}static addDirectory(a){return __awaiter(this,
void 0,void 0,function*(){const b=yield App.listDirectory(a);if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;if(!b)return!0;for(let c=0;c<b.length;c++)if(b[c].isDirectory){if(!(yield AppUI.addDirectory(b[c].fileURL)))return!1}else{if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;yield App.player.playlist.addSong(b[c])}return!0})}static addDirectories(){return __awaiter(this,void 0,void 0,function*(){if(App.player&&!AppUI._loading){if(!App.electron)return AppUI.addFiles(!0);
...
App.adjustCover);App.adjustCover();App.ipcRenderer?(App.ipcRenderer.on("mainWindowStateChanged",App.mainWindowStateChanged),App.ipcRenderer.on("mainWindowClosing",App.mainWindowClosing),App.triggerMainWindowStateChanged()):window.addEventListener("beforeunload",App.mainWindowClosing);App.frameless&&(App.titleBar=document.createElement("h1"),App.titleBar.innerHTML=`
				<div></div>
				<i id="icon-main"></i>
				FPlay
