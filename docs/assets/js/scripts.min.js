...
0<=App.player.playlist.currentIndex&&!AppUI._playlistControl.deleteMode&&a&&AppUI._playlistControl.bringItemIntoView(App.player.playlist.currentIndex),0>b&&(b=0),AppUI._currentTimeS=b|0,a?(Strings.changeText(AppUI._songLengthLabel,a.length),Strings.changeText(AppUI._assistiveSongLengthLabel,a.length),AppUI._seekSlider&&(0<a.lengthMS&&a.isSeekable?(AppUI._seekSlider.disabled=!1,AppUI._seekSlider.max=a.lengthMS):(AppUI._seekSlider.disabled=!0,AppUI._seekSlider.max=Math.max(1,a.lengthMS)),AppUI._seekSlider.manuallyChangeAll(1E3*
b,Formatter.formatTimeS(AppUI._currentTimeS),SliderControlValueChild.LeftChild))):(Strings.changeText(AppUI._songLengthLabel,Formatter.none),Strings.changeText(AppUI._assistiveSongLengthLabel,Formatter.none),AppUI._seekSlider&&(AppUI._seekSlider.disabled=!0,AppUI._seekSlider.manuallyChangeAll(0,Formatter.none,SliderControlValueChild.LeftChild))))}static playerLoadingChange(a){App.updateLoadingIcon()}static playerPausedChange(a){AppUI._playButton&&(AppUI._playButton.iconName=a?"icon-play":"icon-pause")}static playerCurrentTimeSChange(a){if(AppUI._seekSlider&&
...
App.player.currentSong&&(Strings.changeText(AppUI._songLengthLabel,a.length),Strings.changeText(AppUI._assistiveSongLengthLabel,a.length),AppUI._seekSlider&&(0<a.lengthMS&&a.isSeekable?(AppUI._seekSlider.disabled=!1,AppUI._seekSlider.max=a.lengthMS):(AppUI._seekSlider.disabled=!0,AppUI._seekSlider.value=0,AppUI._seekSlider.max=Math.max(1,a.lengthMS)))),AppUI._playlistControl&&AppUI._playlistControl.refreshVisibleItems())}static seekSliderValueChange(a){AppUI._seekSlider&&AppUI._seekSlider.dragging&&
(a=a/1E3|0,AppUI._currentTimeS!==a&&(AppUI._currentTimeS=a,AppUI._seekSlider.manuallyChangeAria(Formatter.formatTimeS(a),SliderControlValueChild.LeftChild)))}static seekSliderDragChangeCommit(a){App.player&&App.player.seekTo(a)}static volumeSliderValueChange(a){App.player&&(App.player.volume=a)}static playlistControlItemClick(a,b,c){App.player&&!c&&App.player.play(b)}static playlistControlItemContextMenu(a,b){App.player&&a&&AppUI._playlistControl&&!AppUI._playlistControl.deleteMode&&AppUI.showSongInfo(a)}static playlistControlItemControlClick(a,
...
!FilePicker.visible){var b=App.hostType===App.hostTypeElectron;if(FilePicker.isSupported()){a=null;var c=FilePicker.showAddPlay(AppUI._playlistControl)}else a=await App.showOpenDialogWeb(a),c=null;if((a||c)&&App.player)try{App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const d=App.player.playlist,e=b?null:new Uint8Array(BufferedReader.minBufferLength),f=b?null:new ResizeableBuffer(2048);let g=!1;do{let h=!1;if(c){const k=await c();if(!k)break;h=k.play;a=k.files}if(!a)break;
