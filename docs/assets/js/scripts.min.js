...
App.player&&App.player.previous(),cancelEvent(a);case "F2":return!a.repeat&&App.player&&App.player.playPause(),cancelEvent(a);case "F3":return!a.repeat&&App.player&&App.player.stop(),cancelEvent(a);case "F4":return!a.repeat&&App.player&&App.player.next(),cancelEvent(a)}}static preInit(a,b){AppUI.webFrame=a;window.addEventListener("keydown",AppUI.globalKeyHandler,!0);a&&b.devicePixelRatio&&b.devicePixelRatio!==devicePixelRatio&&a.setZoomFactor(b.devicePixelRatio);App.hostType===App.hostTypeAndroid&&
(a=document.getElementById("add-folder-button"),a.parentNode.removeChild(a));ButtonControl.init();CheckboxControl.init();AppUI.seekSlider=new SliderControl("seek-slider",!1,!1,0,1);AppUI.seekSlider.disabled=!0;AppUI.seekSlider.onvaluechanged=AppUI.seekSliderValueChanged;AppUI.seekSlider.ondragended=AppUI.seekSliderDragEnded;AppUI.currentTimeLabel=document.createElement("span");AppUI.currentTimeLabel.className="seek-label";AppUI.currentTimeS=0;Strings.changeText(AppUI.currentTimeLabel,Formatter.none);
AppUI.seekSlider.leftChild=AppUI.currentTimeLabel;AppUI.songLengthLabel=document.createElement("span");AppUI.songLengthLabel.className="seek-label";Strings.changeText(AppUI.songLengthLabel,Formatter.none);AppUI.seekSlider.rightChild=AppUI.songLengthLabel;AppUI.volumeSlider=new SliderControl("volume-slider",!0,!1,0,Player.maxVolume,b.playerVolume);AppUI.volumeSlider.leftChild=Icon.createLarge("icon-volume","green small-right-margin");AppUI.volumeLabel=document.createElement("span");AppUI.volumeLabel.className=
"small-left-margin";AppUI.volumeLabel.setAttribute("id","volume-label");Strings.changeText(AppUI.volumeLabel,AppUI.volumeStr(AppUI.volumeSlider.value));AppUI.volumeSlider.rightChild=AppUI.volumeLabel;AppUI.volumeSlider.onvaluechanged=AppUI.volumeSliderValueChanged;AppUI.playlistControl=new ListControl("playlist-control",!0);AppUI.playlistControl.onitemclicked=AppUI.playlistControlItemClicked;AppUI.playlistControl.onitemcontextmenu=AppUI.playlistControlItemContextMenu;AppUI.graphicalFilterControlType=
document.getElementById("graphical-filter-control-type");ButtonControl.setText(AppUI.graphicalFilterControlType,b.graphicalFilterControlSimpleMode?Strings.TraditionalFilter:Strings.AdvancedFilter);AppUI.graphicalFilterControlType.onclick=AppUI.graphicalFilterControlTypeClicked;AppUI.graphicalFilterControlEnabled=document.getElementById("graphical-filter-control-enabled");AppUI.graphicalFilterControlEnabled.checked=b.graphicalFilterControlEnabled||!1;AppUI.graphicalFilterControlEnabled.onclick=AppUI.graphicalFilterControlEnabledClicked;
...
(b=document.getElementById("mono-down-mixer-control-container"),b.parentNode.removeChild(b));App.frameless||AppUI.panelContainer.classList.add("web");window.addEventListener("resize",AppUI.adjustWindow,{passive:!0});AppUI.adjustWindow()}static init(a){App.player&&(AppUI.volumeSlider.value=App.player.volume,App.player.onsongchanged=AppUI.playerSongChanged,App.player.onloadingchanged=AppUI.playerLoadingChanged,App.player.onpausedchanged=AppUI.playerPausedChanged,App.player.oncurrenttimeschanged=AppUI.playerCurrentTimeSChanged,
App.player.onerror=AppUI.playerError,AppUI.preparePlaylist(),AppUI.playerSongChanged(App.player.currentSong),App.player.playlist&&0<=App.player.playlist.currentIndex&&AppUI.playlistControl.centerItemIntoView(App.player.playlist.currentIndex),HistoryHandler.init(Menu.historyStatePopped,Modal.historyStatePopped))}static destroy(){zeroObject(AppUI)}static get loading(){return AppUI._loading}static get thinBorderPX(){return AppUI._thinBorderPX}static get thickBorderPX(){return AppUI._thickBorderPX}static addZoomHandler(a){a&&
AppUI.zoomHandlers.push(a)}static removeZoomHandler(a){if(a)for(let b=AppUI.zoomHandlers.length-1;0<=b;b--)if(AppUI.zoomHandlers[b]===a){AppUI.zoomHandlers.splice(b,1);break}}static factorToZoom(a){return 1>=a?0:Math.round(4*a)-4}static zoomToFactor(a){return 0>=a?1:.25*(a+4)}static remToPX(a){return a*AppUI.rem}static pxToREM(a){return a/AppUI.rem}static playerSongChanged(a){App.player&&AppUI.playlistControl&&(Strings.changeText(AppUI.artistLabel,a&&a.artist||Formatter.none),Strings.changeText(AppUI.titleLabel,
a&&a.title||Formatter.none),App.player.playlist&&0<=App.player.playlist.currentIndex&&AppUI.playlistControl.bringItemIntoView(App.player.playlist.currentIndex),AppUI.currentTimeS=0,a?(Strings.changeText(AppUI.currentTimeLabel,Formatter.zero),Strings.changeText(AppUI.songLengthLabel,a.length),AppUI.seekSlider&&(AppUI.seekSlider.value=0,0<a.lengthMS?(AppUI.seekSlider.disabled=!1,AppUI.seekSlider.max=a.lengthMS):AppUI.seekSlider.disabled=!0)):(Strings.changeText(AppUI.currentTimeLabel,Formatter.none),
Strings.changeText(AppUI.songLengthLabel,Formatter.none),AppUI.seekSlider&&(AppUI.seekSlider.disabled=!0,AppUI.seekSlider.value=0)))}static playerLoadingChanged(a){App.updateLoadingIcon()}static playerPausedChanged(a){AppUI.iconPlay&&AppUI.iconPause&&(a?(AppUI.iconPause.classList.add("hidden"),AppUI.iconPlay.classList.remove("hidden")):(AppUI.iconPlay.classList.add("hidden"),AppUI.iconPause.classList.remove("hidden")))}static playerCurrentTimeSChanged(a){if(AppUI.seekSlider&&!AppUI.seekSlider.dragging){var b=
...
AppUI.baseThickBorderPX=2;AppUI.zoomHandlers=[];AppUI.rootVariables=document.getElementById("root-variables");AppUI.panelContainer=document.getElementById("panel-container");AppUI.fixedPanel=document.getElementById("fixed-panel");AppUI.optionalPanel=document.getElementById("optional-panel");AppUI.iconPlay=document.getElementById("i-play");AppUI.iconPause=document.getElementById("i-pause");AppUI.artistLabel=document.getElementById("artist-label");AppUI.titleLabel=document.getElementById("title-label");
...
a.style.transform=`scale(${Math.ceil(.5*b.right)}, ${Math.ceil(.5*b.bottom)})`}}static sortFiles(a){a.length&&(a[0]["data-path"]?a.sort(function(b,c){return b["data-path"].localeCompare(c["data-path"])}):a[0].webkitRelativePath?a.sort(function(b,c){return b.webkitRelativePath.localeCompare(c.webkitRelativePath)}):a.sort(function(b,c){return b.name.localeCompare(c.name)}));return a}static preInit(){Strings.init();Icon.init();GraphicalFilterEditorStrings.init(Strings.language);Strings.toFixed=GraphicalFilterEditorStrings.toFixed;
const a=InternalStorage.loadAppSettings(),b=window.electronWebFrame;App.ipcRenderer&&delete window.electronIpcRenderer;b&&delete window.electronWebFrame;if("serviceWorker"in navigator&&!location.href.startsWith("file://")){window.addEventListener("beforeinstallprompt",App.beforeInstallPrompt);var c=navigator.serviceWorker.register("sw.js");c&&"then"in c&&c.then(function(d){d&&"onupdatefound"in d&&d.active&&(d.onupdatefound=function(){Modal.show({html:Strings.PleaseRefresh,title:Strings.UpdateAvailable,
oktext:Strings.Refresh,okcancel:!0,onok:function(){window.location.reload()}})})})}window.addEventListener("resize",App.adjustCover);App.adjustCover();App.ipcRenderer?(App.ipcRenderer.on("mainWindowStateChanged",App.mainWindowStateChanged),App.ipcRenderer.on("mainWindowClosing",App.mainWindowClosing),App.triggerMainWindowStateChanged()):App.hostInterface||window.addEventListener("onpagehide"in window?"pagehide":"unload",App.mainWindowClosing);App.frameless&&(App.titleBar=document.createElement("h1"),
