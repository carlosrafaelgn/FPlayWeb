...
(App.player.graphicalFilterControl.simpleMode=!App.player.graphicalFilterControl.simpleMode,ButtonControl.setText(AppUI.graphicalFilterControlType,App.player.graphicalFilterControl.simpleMode?Strings.TraditionalFilter:Strings.AdvancedFilter))}static graphicalFilterControlEnabledClicked(){App.player&&(App.player.graphicalFilterControl.enabled=AppUI.graphicalFilterControlEnabled.checked)}static async addFiles(a){if(App.player&&!AppUI._loading&&(a=App.electron?await App.showOpenFileDialog():await App.showOpenDialogWeb(a))&&
App.player)try{AppUI._loading=!0;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());const b=App.electron?null:new Uint8Array(BufferedFileHandle.minBufferLength<<1),c=App.electron?null:[new Uint8Array(256)];for(let d=0;d<a.length&&App.player&&App.player.playlist;d++)App.electron?await App.player.playlist.addSong(a[d]):await App.player.playlist.addSongWeb(a[d],b,c)}catch(b){alert("addFiles error: "+(b.message||b.toString()))}finally{AppUI._loading=
!1,App.updateLoadingIcon()}}static async addDirectory(a){a=await App.listDirectory(a);if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;if(!a)return!0;for(let b=0;b<a.length;b++)if(a[b].isDirectory){if(!await AppUI.addDirectory(a[b].fileURL))return!1}else{if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;await App.player.playlist.addSong(a[b])}return!0}static async addDirectories(){if(App.player&&!AppUI._loading){if(!App.electron)return AppUI.addFiles(!0);
var a=await App.showOpenDirectoryDialog();if(a&&App.player)try{AppUI._loading=!0;AppUI.directoryCancelled=!1;App.updateLoadingIcon();App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());for(let b=0;b<a.length&&await AppUI.addDirectory(a[b]);b++);}catch(b){alert("addDirectories error: "+(b.message||b.toString()))}finally{AppUI._loading=!1,AppUI.directoryCancelled=!1,App.updateLoadingIcon()}}}static switchView(){AppUI.panelContainer&&AppUI.panelContainer.classList.toggle("toggled")}static showAbout(){Modal.show({html:Strings.AboutHTML,
