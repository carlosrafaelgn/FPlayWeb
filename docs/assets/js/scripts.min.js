...
(a.track=d[126]&255)}}catch(g){}}static async extractID3v2Andv1(a,b,c){a={url:Formatter.pathToURL(a.path)||Formatter.empty,file:a};var d=b.readByte()<<24|b.readByte()<<16|b.readByte()<<8;if(1229206272!==d)if(d|=b.readByte(),1380533830===d){d=await MetadataExtractor.extractRIFF(a,b,c[0]);if(!d)return null;if(!d[1])return a}else return await MetadataExtractor.extractID3v1(a,b,0,c[0]),a;var e=b.readByte(),f=b.readByte();d=b.readByte();var g=b.readByte();const h=b.readByte(),k=b.readByte(),m=b.readByte();
d=(d&16?10:0)+(m&127|(k&127)<<7|(h&127)<<14|(g&127)<<21);if(2<(e&255)||f){for(e=0;0<d&&e!==MetadataExtractor.ALL_B;){(f=b.fillBuffer(1024))&&await f;g=b.readByte()<<24|b.readByte()<<16|b.readByte()<<8|b.readByte();f=b.readByte()<<24|b.readByte()<<16|b.readByte()<<8|b.readByte();b.skip(2);if(!g||0>=f||f>d)break;switch(g){case 1414091826:e&MetadataExtractor.TITLE_B?b.skip(f):(g=MetadataExtractor.readV2Frame(b,f,c),a.title=g&&g.then?await g:g,a.title&&(e|=MetadataExtractor.TITLE_B));break;case 1414546737:e&
MetadataExtractor.ARTIST_B?b.skip(f):(g=MetadataExtractor.readV2Frame(b,f,c),a.artist=g&&g.then?await g:g,a.artist&&(e|=MetadataExtractor.ARTIST_B));break;case 1413565506:e&MetadataExtractor.ALBUM_B?b.skip(f):(g=MetadataExtractor.readV2Frame(b,f,c),a.album=g&&g.then?await g:g,a.album&&(e|=MetadataExtractor.ALBUM_B));break;case 1414677323:e&MetadataExtractor.TRACK_B?b.skip(f):(g=MetadataExtractor.readV2Frame(b,f,c),a.track=g&&g.then?parseInt(await g):parseInt(g),a.track?e|=MetadataExtractor.TRACK_B:
a.track=0);break;case 1414284622:e&MetadataExtractor.LENGTH_B?b.skip(f):(g=MetadataExtractor.readV2Frame(b,f,c),a.lengthMS=g&&g.then?parseInt(await g):parseInt(g),a.lengthMS?e|=MetadataExtractor.LENGTH_B:a.lengthMS=0);break;case 1415136594:case 1413763651:e&MetadataExtractor.YEAR_B?b.skip(f):(g=MetadataExtractor.readV2Frame(b,f,c),a.year=g&&g.then?parseInt(await g):parseInt(g),a.year?e|=MetadataExtractor.YEAR_B:a.year=0);break;default:b.skip(f)}d-=10+f}(e&MetadataExtractor.ALL_BUT_LENGTH_B)!==MetadataExtractor.ALL_BUT_LENGTH_B&&
await MetadataExtractor.extractID3v1(a,b,e,c[0])}return a}static async extract(a,b,c){if(!a)return null;if(!a.name.endsWith(".mp3")&&!a.name.endsWith(".aac")&&!a.name.endsWith(".wav")){const d=a.name.toLowerCase();if(!d.endsWith(".mp3")&&!d.endsWith(".aac")&&!d.endsWith(".wav"))return null}b||(b=new Uint8Array(BufferedFileHandle.minBufferLength));c||(c=[new Uint8Array(256)]);try{const d=new BufferedFileHandle(a,b);await d.fillBuffer(-1);return await MetadataExtractor.extractID3v2Andv1(a,d,c)}catch(d){return null}}}
...
m&&k<e;h++,k++,l+=c){const p=d.item(k),n=b[h],q=f===p;n.current!==q&&((n.current=q)?n.element.classList.add("current"):n.element.classList.remove("current"));a.prepareElement(p,k,e,n.element);n.item||g.appendChild(n.element);n.item=p;n.index!==k&&(n.index=k,n.element.style.transform="translateY("+l+"px)")}for(a=b.length;h<a;h++)d=b[h],d.item&&(d.item=null,g.removeChild(d.element))}else{g=this.visibleCount;for(let l=0,p=this.indexFromY(0);l<=g&&p<e;l++,p++)a.prepareElement(d.item(p),p,e,b[p].element)}}}resize(){const a=
...
(this.clientHeight-this.itemHeight>>1)}refreshVisibleItems(){this.refreshVisibleItemsEnqueued||(this.refreshVisibleItemsEnqueued=!0,queueMicrotask(this.boundRefreshVisibleItemsInternal))}notifyCleared(){this.clear()}notifyItemsAdded(a,b){if(this.useVirtualItems)this.refreshVisibleItems();else{const m=this.adapter,l=b-a+1;if(m&&!(0>=l)){var c=this.container,d=this.items,e=m.list,f=e.length,g=e.currentItem,h=Array(l);for(var k=b;k>=a;k--){const p=e.item(k),n=new ListControlItem(m.createEmptyElement("list-item real"));
m.prepareElement(p,k,f,n.element);h[k-a]=n;g===p&&(this.currentItem&&(this.currentItem.current=!1,this.currentItem.element.classList.remove("current")),this.currentItem=n,n.current=!0,n.element.classList.add("current"))}g=a>=d.length?null:d[a].element;a>=d.length?d.push(...h):d.splice(a,0,...h);if(this.items.length!==e.length)throw Error("Assertion error: this.items.length !== list.length");this.adjustContainerHeight();for(k=0;k<l;k++)c.insertBefore(h[k].element,g);for(c=d.length-1;c>b;c--)m.prepareElementIndexOrLengthChanged(e.item(c),
...
App.ipcRenderer.invoke("mainWindowMinimize")}static mainWindowRestoreOrMaximize(){App.ipcRenderer&&App.ipcRenderer.invoke(App.isMaximized?"mainWindowRestore":"mainWindowMaximize")}static mainWindowClose(){App.ipcRenderer&&App.ipcRenderer.invoke("mainWindowClose")}static async showOpenFileDialog(){if(App.fileInput||App._showingDialog)return null;try{return App._showingDialog=!0,await App.ipcRenderer.invoke("showOpenFileDialog")}finally{App._showingDialog=!1}}static convertFilePathsToAppFiles(a){return App.ipcRenderer.invoke("convertFilePathsToAppFiles",
a)}static async showOpenDirectoryDialog(){if(App._showingDialog)return null;try{return App._showingDialog=!0,await App.ipcRenderer.invoke("showOpenDirectoryDialog")}finally{App._showingDialog=!1}}static listDirectory(a){return App.ipcRenderer.invoke("listDirectory",a)}static extractMetadata(a,b){return App.ipcRenderer.invoke("extractMetadata",a,b)}static async showOpenDialogWeb(a){if(!App.fileInput)return null;App.fileInputPromiseResolve&&(App.fileInputPromiseResolve(null),App.fileInputPromiseResolve=
