...
0),c;let e=0;d&&(a.set(this.buffer.subarray(this.bufferPosition,this.bufferPosition+d),b),e+=d,b+=d,c-=d,this._bufferAvailable=this.bufferPosition=0);return this._eof?e:new Promise((g,f)=>{this.file.slice(this.position,this.position+c).arrayBuffer().then(h=>{h.byteLength?(this.position+=h.byteLength,this.position>=this.fileLength&&(this.position=this.fileLength,this._eof=!0)):this._eof=!0;a.set(new Uint8Array(h),b);g(h.byteLength+e)},f)})}}BufferedFileHandle.minBufferLength=32768;
class MetadataExtractor{static async extractRIFF(a,b,c){if(44>b.fileLength)return null;var d=b.readUInt32LE();if(null===d||b.fileLength<8+d)return null;var e=d;let g=!1,f=!1,h=d=0,k=0;a:for(;0<e;){var m=!1,l=0,p=void 0;switch(b.readUInt32BE()){case null:break a;case 1463899717:m=!0;e-=4;break;case 1279873876:l=b.readUInt32LE();if(null===l)break a;e-=8;if(0>l||l>e)break a;break;case 1768174368:l=b.readUInt32LE();if(null===l)break a;e-=8;if(0>l||l>e)break a;(p=b.fillBuffer(1024))&&await p;if(4<l){m=
...
(a.track=d[126]&255)}}catch(f){}}static async extractID3v2Andv1(a,b,c){const d={url:Formatter.pathToURL(a["data-path"])||Formatter.empty};d.url||(d.file=a);a=b.readByte()<<24|b.readByte()<<16|b.readByte()<<8;if(1229206272!==a)if(a|=b.readByte(),1380533830===a){a=await MetadataExtractor.extractRIFF(d,b,c[0]);if(!a)return null;if(!a[1])return d}else return await MetadataExtractor.extractID3v1(d,b,0,c[0]),d;var e=b.readByte(),g=b.readByte();a=b.readByte();var f=b.readByte();const h=b.readByte(),k=b.readByte(),
m=b.readByte();a=(a&16?10:0)+(m&127|(k&127)<<7|(h&127)<<14|(f&127)<<21);if(2<(e&255)||g){for(e=0;0<a&&e!==MetadataExtractor.ALL_B;){(g=b.fillBuffer(1024))&&await g;f=b.readByte()<<24|b.readByte()<<16|b.readByte()<<8|b.readByte();g=b.readByte()<<24|b.readByte()<<16|b.readByte()<<8|b.readByte();b.skip(2);if(!f||0>=g||g>a)break;switch(f){case 1414091826:e&MetadataExtractor.TITLE_B?b.skip(g):(f=MetadataExtractor.readV2Frame(b,g,c),d.title=f&&f.then?await f:f,d.title&&(e|=MetadataExtractor.TITLE_B));break;
case 1414546737:e&MetadataExtractor.ARTIST_B?b.skip(g):(f=MetadataExtractor.readV2Frame(b,g,c),d.artist=f&&f.then?await f:f,d.artist&&(e|=MetadataExtractor.ARTIST_B));break;case 1413565506:e&MetadataExtractor.ALBUM_B?b.skip(g):(f=MetadataExtractor.readV2Frame(b,g,c),d.album=f&&f.then?await f:f,d.album&&(e|=MetadataExtractor.ALBUM_B));break;case 1414677323:e&MetadataExtractor.TRACK_B?b.skip(g):(f=MetadataExtractor.readV2Frame(b,g,c),d.track=f&&f.then?parseInt(await f):parseInt(f),d.track?e|=MetadataExtractor.TRACK_B:
d.track=0);break;case 1414284622:e&MetadataExtractor.LENGTH_B?b.skip(g):(f=MetadataExtractor.readV2Frame(b,g,c),d.lengthMS=f&&f.then?parseInt(await f):parseInt(f),d.lengthMS?e|=MetadataExtractor.LENGTH_B:d.lengthMS=0);break;case 1415136594:case 1413763651:e&MetadataExtractor.YEAR_B?b.skip(g):(f=MetadataExtractor.readV2Frame(b,g,c),d.year=f&&f.then?parseInt(await f):parseInt(f),d.year?e|=MetadataExtractor.YEAR_B:d.year=0);break;default:b.skip(g)}a-=10+g}(e&MetadataExtractor.ALL_BUT_LENGTH_B)!==MetadataExtractor.ALL_BUT_LENGTH_B&&
await MetadataExtractor.extractID3v1(d,b,e,c[0])}return d}static async extract(a,b,c){if(!a)return null;if(!a.name.endsWith(".mp3")&&!a.name.endsWith(".aac")&&!a.name.endsWith(".wav")){const d=a.name.toLowerCase();if(!d.endsWith(".mp3")&&!d.endsWith(".aac")&&!d.endsWith(".wav"))return null}b||(b=new Uint8Array(BufferedFileHandle.minBufferLength));c||(c=[new Uint8Array(256)]);try{const d=new BufferedFileHandle(a,b);await d.fillBuffer(-1);return await MetadataExtractor.extractID3v2Andv1(a,d,c)}catch(d){return null}}}
MetadataExtractor.ALL_B=63;MetadataExtractor.ALL_BUT_LENGTH_B=31;MetadataExtractor.TITLE_B=1;MetadataExtractor.ARTIST_B=2;MetadataExtractor.ALBUM_B=4;MetadataExtractor.TRACK_B=8;MetadataExtractor.YEAR_B=16;MetadataExtractor.LENGTH_B=32;MetadataExtractor.textDecoderIso88591=new TextDecoder("iso-8859-1");MetadataExtractor.textDecoderUtf16=new TextDecoder("utf-16");MetadataExtractor.textDecoderUtf16be=new TextDecoder("utf-16be");MetadataExtractor.textDecoderUtf8=new TextDecoder("utf-8");
...
g=Array(a);f=Array(a);const l=Array(a);var k=.5*Math.log(2),m=this.audioContext.sampleRate;const p=2*Math.PI;for(let n=a-1;0<=n;n--){const q=p*d[n]/m;l[n]=1/(2*Math.sinh(q/Math.sin(q)*k))}for(k=a-1;0<=k;k--)m=b.createBiquadFilter(),m.type="peaking",m.frequency.value=d[k],m.Q.value=l[k],g[k]=m,k<a-1&&g[k+1].connect(g[k]);this.biquadFilters=g;this.biquadFilterInput=g[a-1];this.biquadFilterOutput=g[0]}for(b=a-1;0<=b;b--)f[b]=Math.max(-40,this.yToDB(c[e[b]]));for(b=a-1;0<=b;b--)g[b].gain.value=f[b]+(b<
...
for(let r=n-1;0<=r;r--)d.lineTo(t-p,u),t-=l,d.moveTo(t,u);d.stroke();d.beginPath();t=k+(p>>1);u=(GraphicalFilterEditor.validYRangeHeight*e|0)+h;d.moveTo(t,u);for(k=n-1;0<=k;k--)d.lineTo(t-p,u),t-=l,d.moveTo(t,u);d.stroke();if(a)for(a=f.equivalentZonesFrequencyCount.length-2;0<a;a--){t=((f.equivalentZonesFrequencyCount[a]+g)*e|0)+h;u=q;d.beginPath();for(d.moveTo(t,u);u<m;)d.lineTo(t,u+p),u+=l,d.moveTo(t,u);d.stroke()}t=1>e?2*e:2*e|0;h=.5*t;d.lineWidth=t;a=GraphicalFilterEditor.visibleBinCount-1;for(m=
b?1:0;0<=m;m--){l=m||!b?f.channelCurves[c]:f.actualChannelCurve;d.strokeStyle=m?"#707070":this.rangeImage;d.beginPath();d.moveTo(g*e|0,l[0]*e+h);for(t=1;t<a;t++)d.lineTo((g+t)*e|0,l[t]*e+h);d.lineTo((g+t+1)*e|0,l[t]*e+h);d.stroke()}}}}
...
d:2*d|0);const g=this.leftMargin,f=GraphicalFilterEditor.visibleBinCount;for(let h=b?1:0;0<=h;h--){const k=h||!b?a.channelCurves[c]:a.actualChannelCurve,m=h?this.svgGrayCurve:this.svgCurve;let l=1,p=0,n=k[0],q="M "+(g*d|0)+","+(n*d+e);for(;l<f;l++)k[l]!==n&&(l>p+1&&1<l&&(q+=" "+((g+l-1)*d|0)+","+(k[l-1]*d+e)),p=l,n=k[l],q+=" "+((g+p)*d|0)+","+(n*d+e));q+=" "+((g+l)*d|0)+","+(k[l-1]*d+e);m.setAttribute("d",q)}this.isActualChannelCurveNeeded!==b&&(this.isActualChannelCurveNeeded=b,this.svgGrayCurve.style.display=
b?"":"none")}}
...
["webkit-3d","moz-webgl","webgl","experimental-webgl"];for(let g=0;g<e.length;g++)try{const f=a.getContext(e[g],b);return new Program(f,c,d)}catch(f){}return null}getAttribsAndUniforms(a,b,c,d){a=a.split("\n");for(let m=0;m<a.length;m++){var e=a[m].trim();if("uniform"===e.substr(0,7)){e=e.split(" ");var g=e[1];for(var f=2;f<e.length;f++){var h=e[f];if(";"===h)break;var k=h.charAt(h.length-1);const l=";"===k;if(","===k||l)h=h.substr(0,h.length-1);","!==h&&h.length&&(c.push(h),d.push(g));if(l)break}}else if("attribute"===
e.substr(0,9))for(e=e.split(" "),g=2;g<e.length;g++){f=e[g];if(";"===f)break;h=f.charAt(f.length-1);k=";"===h;if(","===h||k)f=f.substr(0,f.length-1);","!==f&&f.length&&b.push(f);if(k)break}}}prepareUniform(a,b){const c=this.gl,d=c.getUniformLocation(this.program,a);if(!this[a])if("bool"===b||"int"===b||"sampler2D"===b)this[a]=function(e){c.uniform1i(d,e)};else if("float"===b)this[a]=function(e){c.uniform1f(d,e)};else if("vec2"===b)this[a]=function(e,g){c.uniform2f(d,e,g)},this[a+"v"]=function(e){c.uniform2fv(d,
...
2048,e=this.visibleFrequencies,g=Analyzer.colors;var f;let h,k,m,l;this.analyzerL.getByteTimeDomainData(this.data);cLib._plainAnalyzer(this.fft4gfPtr,this.windowPtr,this.dataPtr,this.tmpPtr);let p=this.prevL;c.lineWidth=1;c.fillStyle="#000000";c.fillRect(0,0,512,512);for(k=h=0;511>k&&1024>h&&d>e[k+1]-e[k];){for(f=d*h;1024>h&&f+d<e[k];)h++,f=d*h;f=(4*p[k]+a[k]*lerp(f,b[h],f+d,b[h+1],e[k]))/2.5>>1;255<f?f=255:0>f&&(f=0);p[k]=f;c.beginPath();c.strokeStyle=g[f];c.moveTo(k-.5,256.5-f);c.lineTo(k-.5,256.5);
c.stroke();k++}for(h++;1024>h&&512>k;){l=m=0;do m+=b[h],l++,h++,f=d*h;while(f<e[k]&&1024>h);f=(4*p[k]+a[k]*m/l)/2.5>>1;255<f?f=255:0>f&&(f=0);p[k]=f;c.beginPath();c.strokeStyle=g[f];c.moveTo(k-.5,256.5-f);c.lineTo(k-.5,256.5);c.stroke();k++}this.analyzerR.getByteTimeDomainData(this.data);cLib._plainAnalyzer(this.fft4gfPtr,this.windowPtr,this.dataPtr,this.tmpPtr);p=this.prevR;for(k=h=0;511>k&&1024>h&&d>e[k+1]-e[k];){for(f=d*h;1024>h&&f+d<e[k];)h++,f=d*h;f=(4*p[k]+a[k]*lerp(f,b[h],f+d,b[h+1],e[k]))/
2.5>>1;255<f?f=255:0>f&&(f=0);p[k]=f;c.beginPath();c.strokeStyle=g[f];c.moveTo(k-.5,256.5);c.lineTo(k-.5,256.5+f);c.stroke();k++}for(h++;1024>h&&512>k;){l=m=0;do m+=b[h],l++,h++,f=d*h;while(f<e[k]&&1024>h);f=(4*p[k]+a[k]*m/l)/2.5>>1;255<f?f=255:0>f&&(f=0);p[k]=f;c.beginPath();c.strokeStyle=g[f];c.moveTo(k-.5,256.5);c.lineTo(k-.5,256.5+f);c.stroke();k++}}cleanUp(){this.ptr&&cLib._freeBuffer(this.ptr)}}
...
p=l-e;0<=p;p--,l--)e=c[l],c[l]=c[p],c[p]=e;var f=a.list,h=f.length,k=a.list.currentItem;e=this.container;var m=0;for(let l=b*d;m<g&&b<h;m++,b++,l+=d){const p=f.item(b),n=c[m];if(n.item!==p){const q=k===p;n.current!==q&&((n.current=q)?n.element.classList.add("current"):n.element.classList.remove("current"));a.prepareElement(p,b,h,n.element);n.item||e.appendChild(n.element);n.item=p}n.index!==b&&(n.index=b,n.element.style.transform="translateY("+l+"px)")}for(a=c.length;m<a;m++)d=c[m],d.item&&(d.item=
null,e.removeChild(d.element));this.lastIndex=b-1;this.lastIndex<this.firstIndex&&(this.lastIndex=this.firstIndex)}}refreshVisibleItemsInternal(){this.refreshVisibleItemsEnqueued=!1;var a=this._adapter,b=this.element;if(a&&b){this.adjustContainerHeight();b=this.items;var c=this.itemHeightAndMargin,d=a.list,e=d.length,g=a.list.currentItem,f=this.container;if(this.useVirtualItems){this.currentListItem=g;var h=0,k=this.firstIndex,m=this.visibleCount+(0===k?2:1===k?3:4);0>(k-=2)&&(k=0);for(let l=k*c;h<
m&&k<e;h++,k++,l+=c){const p=d.item(k),n=b[h],q=g===p;n.current!==q&&((n.current=q)?n.element.classList.add("current"):n.element.classList.remove("current"));a.prepareElement(p,k,e,n.element);n.item||f.appendChild(n.element);n.item=p;n.index!==k&&(n.index=k,n.element.style.transform="translateY("+l+"px)")}for(a=b.length;h<a;h++)d=b[h],d.item&&(d.item=null,f.removeChild(d.element))}else{f=this.visibleCount;for(let l=0,p=this.indexFromY(0);l<=f&&p<e;l++,p++)a.prepareElement(d.item(p),p,e,b[p].element)}}}resize(){const a=
...
(this.clientHeight-this.itemHeight>>1)}refreshVisibleItems(){this.refreshVisibleItemsEnqueued||(this.refreshVisibleItemsEnqueued=!0,queueMicrotask(this.boundRefreshVisibleItemsInternal))}notifyCleared(){this.clear()}notifyItemsAdded(a,b){if(this.useVirtualItems)this.refreshVisibleItems();else{const m=this.adapter,l=b-a+1;if(m&&!(0>=l)){var c=this.container,d=this.items,e=m.list,g=e.length,f=e.currentItem,h=Array(l);for(var k=b;k>=a;k--){const p=e.item(k),n=new ListControlItem(m.createEmptyElement("list-item real"));
m.prepareElement(p,k,g,n.element);h[k-a]=n;f===p&&(this.currentItem&&(this.currentItem.current=!1,this.currentItem.element.classList.remove("current")),this.currentItem=n,n.current=!0,n.element.classList.add("current"))}f=a>=d.length?null:d[a].element;a>=d.length?d.push(...h):d.splice(a,0,...h);if(this.items.length!==e.length)throw Error("Assertion error: this.items.length !== list.length");this.adjustContainerHeight();for(k=0;k<l;k++)c.insertBefore(h[k].element,f);for(c=d.length-1;c>b;c--)m.prepareElementIndexOrLengthChanged(e.item(c),
c,g,d[c].element);for(--a;0<=a;a--)m.prepareElementIndexOrLengthChanged(e.item(a),a,g,d[a].element)}}}notifyItemsRemoved(a,b){if(this.useVirtualItems)this.refreshVisibleItems();else{const f=this.adapter;var c=b-a+1;if(f&&!(0>=c)){var d=this.container;b=this.items;var e=f.list,g=e.length;a=b.splice(a,c);for(c=a.length-1;0<=c;c--){const h=a[c];h.current&&(this.currentItem=null);d.removeChild(h.element);h.zero()}if(this.items.length!==e.length)throw Error("Assertion error: this.items.length !== list.length");
...
AppUI.preparePlaylist());const c=b?null:new Uint8Array(BufferedFileHandle.minBufferLength<<1),d=b?null:[new Uint8Array(256)];for(let e=0;e<a.length&&App.player&&App.player.playlist;e++)b?await App.player.playlist.addSong(a[e]):await App.player.playlist.addSongWeb(a[e],c,d)}catch(c){alert("addFiles error: "+(c.message||c.toString()))}finally{AppUI._loading=!1,App.updateLoadingIcon()}}}static async addDirectory(a){a=await App.listDirectory(a);if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;
if(!a)return!0;for(let b=0;b<a.length;b++)if(a[b].isDirectory){if(!await AppUI.addDirectory(a[b].fileURL))return!1}else{if(!App.player||!App.player.playlist||AppUI.directoryCancelled)return!1;await App.player.playlist.addSong(a[b])}return!0}static async addDirectories(){if(App.player&&!AppUI._loading){if(App.hostType!==App.hostTypeElectron)return AppUI.addFiles(!0);var a=await App.showOpenDirectoryDialog();if(a&&App.player)try{AppUI._loading=!0;AppUI.directoryCancelled=!1;App.updateLoadingIcon();
App.player.playlist||(App.player.playlist=new Playlist,AppUI.preparePlaylist());for(let b=0;b<a.length&&await AppUI.addDirectory(a[b]);b++);}catch(b){alert("addDirectories error: "+(b.message||b.toString()))}finally{AppUI._loading=!1,AppUI.directoryCancelled=!1,App.updateLoadingIcon()}}}static switchView(){AppUI.panelContainer&&AppUI.panelContainer.classList.toggle("toggled")}static showAbout(){Modal.show({html:Strings.AboutHTML+(App.player&&App.player.audioContext?"<br/><br/><small>Base latency: "+
(App.player.audioContext.baseLatency||0).toFixed(4)+" s<br/>Output latency: "+(isNaN(App.player.audioContext.outputLatency)?"-":App.player.audioContext.outputLatency.toFixed(4)+" s")+"</small>":""),title:Strings.About+" (v"+window.CACHE_VERSION+")"})}}AppUI.primaryInputIsTouch="matchMedia"in window?window.matchMedia("(pointer: coarse)").matches:0<navigator.maxTouchPoints||0<navigator.msMaxTouchPoints;AppUI.rem=4;AppUI.buttonRegularSizeREM=8;AppUI.buttonRegularSizePX=AppUI.remToPX(AppUI.buttonRegularSizeREM);
...
null;f&&(g=JSON.parse(f))}catch(h){}if(g&&g.length===e.length)for(f=e.length-1;0<=f;f--)e[f]=App.fileInput.files[f],e[f]["data-path"]=g[f];else for(g=e.length-1;0<=g;g--)e[g]=App.fileInput.files[g];App.fileInput.value="";d(App.sortFiles(e))}else d(null)}},c.appendChild(App.fileInput),document.getElementById("top-panel").appendChild(c));AppUI.preInit(b,a)}static async init(){if(!App.player){var a=InternalStorage.loadAppSettings();App.player=new Player(document.getElementById("filter-container"),document.getElementById("optional-panel-container"),
document.getElementById("stereo-panner-slider"),a.playerVolume,a.graphicalFilterControlEnabled,a.graphicalFilterControlSimpleMode,a.stereoPannerControlEnabled,a.monoDownMixerControlEnabled);try{const b=await InternalStorage.loadPlaylist();if(!App.player||!App.player.alive)return;b&&(App.player.playlist=b)}catch(b){}App.player&&App.player.alive&&(AppUI.init(a),(a=document.getElementById("cover"))&&a.classList.remove("in"),setTimeout(function(){window.removeEventListener("resize",App.adjustCover);const b=
document.getElementById("cover");b&&document.body.removeChild(b)},320),App.loading=!1)}}static triggerMainWindowStateChanged(){App.ipcRenderer&&App.ipcRenderer.invoke("triggerMainWindowStateChanged")}static mainWindowMinimize(){App.ipcRenderer&&App.ipcRenderer.invoke("mainWindowMinimize")}static mainWindowRestoreOrMaximize(){App.ipcRenderer&&App.ipcRenderer.invoke(App.isMaximized?"mainWindowRestore":"mainWindowMaximize")}static mainWindowClose(){App.ipcRenderer&&App.ipcRenderer.invoke("mainWindowClose")}static async showOpenFileDialog(){if(App.fileInput||
