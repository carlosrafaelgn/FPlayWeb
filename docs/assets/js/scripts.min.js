...
(App.monoDownMixerControl.multiplier=1/(1+d))};return App.stereoPannerControl},function(c){App.monoDownMixerControl=new MonoDownMixerControl(c);App.monoDownMixerControl.enabled=MonoDownMixerControl.isSupported()&&!!a.monoDownMixerControlEnabled;return App.monoDownMixerControl});var b=InternalStorage.loadPlaylistWeb();b&&(App.player.playlist=b);AppUI.init(a);(b=document.getElementById("cover"))&&b.classList.remove("in");setTimeout(function(){window.removeEventListener("resize",App.adjustCover);const c=
document.getElementById("cover");c&&document.body.removeChild(c)},320);App.loading=!1}}static triggerMainWindowStateChanged(){App.ipcRenderer&&App.ipcRenderer.invoke("triggerMainWindowStateChanged")}static mainWindowMinimize(){App.ipcRenderer&&App.ipcRenderer.invoke("mainWindowMinimize")}static mainWindowRestoreOrMaximize(){App.ipcRenderer&&App.ipcRenderer.invoke(App.isMaximized?"mainWindowRestore":"mainWindowMaximize")}static mainWindowClose(){App.ipcRenderer&&App.ipcRenderer.invoke("mainWindowClose")}static async showOpenFileDialog(){if(App.fileInput||
