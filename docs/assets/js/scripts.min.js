...
if(!this._paused&&(this._paused=!0,this.suspendAudioContext(!0),this.onpausedchanged))this.onpausedchanged(!0)}}playbackLengthChanged(){this._alive&&this.audio&&this._playlist&&this._currentSong&&(this._playlist.songLengthChanged(this._currentSong,this.audio.duration),this.mediaSession&&0<this.audio.duration&&"setPositionState"in this.mediaSession&&this.mediaSession.setPositionState({duration:this.audio.duration}))}currentTimeChanged(){if(this._alive&&this.audio&&this._currentSong&&this.oncurrenttimeschanged)this.oncurrenttimeschanged(this.audio.currentTime)}notifySongChanged(){if(this._alive){const a=
this._currentSong,b=this.mediaSession;b&&(a?(b.metadata=new window.MediaMetadata({title:a.title,artist:a.artist,album:a.album,artwork:[{src:"assets/images/albumArts/64x64.png",sizes:"64x64",type:"image/png"},{src:"assets/images/albumArts/96x96.png",sizes:"96x96",type:"image/png"},{src:"assets/images/albumArts/192x192.png",sizes:"192x192",type:"image/png"},{src:"assets/images/albumArts/256x256.png",sizes:"256x256",type:"image/png"},{src:"assets/images/albumArts/512x512.png",sizes:"512x512",type:"image/png"}]}),
0<a.lengthMS&&"setPositionState"in b&&b.setPositionState({duration:a.lengthMS/1E3})):b.playbackState="none");if(this.onsongchanged)this.onsongchanged(a)}}previous(){this._alive&&this._playlist&&(this._playlist.moveCurrentToPrevious(),this.play(-1))}seekBackward(){this._alive&&this.seekTo(-1E4,!0)}pause(){this._alive&&(this.suspendAudioContext(!1),this._currentSong&&this.audio.pause())}play(a){if(this._alive){if(this._currentSong&&void 0===a){this.resumeAudioContext();var b=this.audio.play()}else if(this._playlist){void 0!==
a&&0<=a&&(this._playlist.currentIndex=a);a=this._playlist.currentItem;!a&&this._playlist.length&&(this._playlist.moveCurrentToNext(),a=this._playlist.currentItem);if(!a){this.stop();return}this._currentSong=a;queueMicrotask(this.boundNotifySongChanged);for(this.resumeAudioContext();this.audio.firstChild;)this.audio.removeChild(this.audio.firstChild);b=document.createElement("source");if(a.url)b.src=a.url;else if(a.file)this.lastObjectURL&&URL.revokeObjectURL(this.lastObjectURL),this.lastObjectURL=
URL.createObjectURL(a.file),b.src=this.lastObjectURL;else{this.stop();if(this.onerror)this.onerror(Strings.MissingSongError);return}b.onerror=this.boundPlaybackError;this.audio.appendChild(b);this.audio.load();b=this.audio.play()}b&&b.catch(Player.nop)}}playPause(){this._alive&&(!this._currentSong||this.audio.paused?this.play():this.pause())}stop(){if(this._alive){if(this._currentSong){for(;this.audio.firstChild;)this.audio.removeChild(this.audio.firstChild);this.audio.load();this.lastObjectURL&&
(URL.revokeObjectURL(this.lastObjectURL),this.lastObjectURL=null);if(this._loading&&(this._loading=!1,this.onloadingchanged))this.onloadingchanged(!1);if(!this._paused&&(this._paused=!0,this.onpausedchanged))this.onpausedchanged(!0);this._currentSong=null;this.notifySongChanged()}this.suspendAudioContext(!1)}}seekForward(){this._alive&&this.seekTo(1E4,!0)}next(){this._alive&&this._playlist&&(this._playlist.moveCurrentToNext(),this.play(-1))}seekTo(a,b){this._alive&&this._currentSong&&!(0>=this._currentSong.lengthMS)&&
this.audio.seekable&&this.audio.seekable.length&&(a=a/1E3+(b?this.audio.currentTime:0),this.audio.currentTime=a>this.audio.duration?this.audio.duration:0>=a?0:a)}}Player.maxVolume=40;Player.nop=function(){};
...
AppUI.panelContainer=document.getElementById("panel-container");AppUI.fixedPanel=document.getElementById("fixed-panel");AppUI.optionalPanel=document.getElementById("optional-panel");AppUI.iconPlay=document.getElementById("i-play");AppUI.iconPause=document.getElementById("i-pause");AppUI.artistLabel=document.getElementById("artist-label");AppUI.titleLabel=document.getElementById("title-label");AppUI._loading=!1;AppUI.devicePixelRatio=-1;AppUI._smallIconSizePX=Icon.baseSizePX;
...
graphicalFilterControlEnabled:App.graphicalFilterControl?App.graphicalFilterControl.enabled:!1,graphicalFilterControlSimpleMode:App.graphicalFilterControl?App.graphicalFilterControl.simpleMode:!0,stereoPannerControlEnabled:App.stereoPannerControl?App.stereoPannerControl.enabled:!1,monoDownMixerControlEnabled:App.monoDownMixerControl?App.monoDownMixerControl.enabled:!1});App.player.destroy(!0);App.graphicalFilterControl&&App.graphicalFilterControl.saveSettings();App.stereoPannerControl&&App.stereoPannerControl.saveSettings();
if(a&&a.length)for(let d=a.length-1;0<=d;d--)if(a[d]){const e=a[d]();e&&b.push(e)}App.insideNativeHost?(c&&b.push(InternalStorage.savePlaylist(c)),b.length?Promise.all(b).then(App.destroy,App.destroy):App.destroy()):c&&InternalStorage.savePlaylistWeb(c)}}static destroy(){App.player&&App.player.destroy();AppUI.destroy();const a=App.ipcRenderer;zeroObject(App);a&&a.invoke("mainWindowFinalClose")}static updateLoadingIcon(){App.iconLoading&&(App.iconLoading.style.display=App.loading?"":"none")}static get loading(){return(App.player?
App.player.loading:!1)||AppUI.loading||App._loading}static set loading(a){App._loading!==a&&(App._loading=a,App.updateLoadingIcon())}static get installPromptReady(){return!!App.installPrompt}static showInstallPrompt(){const a=App.installPrompt;if(a&&a.prompt)try{App.installPrompt=null,a.prompt()}catch(b){}}static beforeInstallPrompt(a){"preventDefault"in a&&a.preventDefault();App.installPrompt=a}static adjustCover(){const a=document.getElementById("cover");if(a){const b=document.body.getBoundingClientRect();
a.style.transform=`scale(${Math.ceil(.5*b.right)}, ${Math.ceil(.5*b.bottom)})`}}static sortFiles(a){a.length&&(a[0].webkitRelativePath?a.sort(function(b,c){return b.webkitRelativePath.localeCompare(c.webkitRelativePath)}):a.sort(function(b,c){return b.name.localeCompare(c.name)}));return a}static preInit(){Strings.init();Icon.init();GraphicalFilterEditorStrings.init(Strings.language);Strings.toFixed=GraphicalFilterEditorStrings.toFixed;const a=InternalStorage.loadAppSettings(),b=window.electronWebFrame;
App.ipcRenderer&&delete window.electronIpcRenderer;b&&delete window.electronWebFrame;if("serviceWorker"in navigator){window.addEventListener("beforeinstallprompt",App.beforeInstallPrompt);var c=navigator.serviceWorker.register("sw.js");c&&"then"in c&&c.then(function(d){d&&"onupdatefound"in d&&d.active&&(d.onupdatefound=function(){Modal.show({html:Strings.PleaseRefresh,title:Strings.UpdateAvailable,oktext:Strings.Refresh,okcancel:!0,onok:function(){window.location.reload()}})})})}window.addEventListener("resize",
App.adjustCover);App.adjustCover();App.ipcRenderer?(App.ipcRenderer.on("mainWindowStateChanged",App.mainWindowStateChanged),App.ipcRenderer.on("mainWindowClosing",App.mainWindowClosing),App.triggerMainWindowStateChanged()):App.hostInterface||window.addEventListener("onpagehide"in window?"pagehide":"unload",App.mainWindowClosing);App.frameless&&(App.titleBar=document.createElement("h1"),App.titleBar.innerHTML=`
				<div></div>
				<i id="icon-main"></i>
				FPlay
...
null;f&&(g=JSON.parse(f))}catch(k){}if(g&&g.length===e.length)for(f=e.length-1;0<=f;f--)e[f]=App.fileInput.files[f],e[f]["data-path"]=g[f];else for(g=e.length-1;0<=g;g--)e[g]=App.fileInput.files[g];App.fileInput.value="";d(App.sortFiles(e))}else d(null)}},c.appendChild(App.fileInput),document.getElementById("top-panel").appendChild(c));AppUI.preInit(b,a)}static async init(){if(!App.player){var a=InternalStorage.loadAppSettings();App.player=new Player(a.playerVolume,function(c){App.graphicalFilterControl=
new GraphicalFilterControl(document.getElementById("filter-container"),document.getElementById("optional-panel-container"),c,a.graphicalFilterControlSimpleMode);App.graphicalFilterControl.enabled=!!a.graphicalFilterControlEnabled;return App.graphicalFilterControl},function(c){App.stereoPannerControl=new StereoPannerControl(document.getElementById("stereo-panner-slider"),c);App.stereoPannerControl.enabled=!!a.stereoPannerControlEnabled;App.stereoPannerControl.onappliedgainchanged=function(d){App.monoDownMixerControl&&
(App.monoDownMixerControl.multiplier=1/(1+d))};return App.stereoPannerControl},function(c){App.monoDownMixerControl=new MonoDownMixerControl(c);App.monoDownMixerControl.enabled=MonoDownMixerControl.isSupported()&&!!a.monoDownMixerControlEnabled;return App.monoDownMixerControl});try{var b=App.insideNativeHost?await InternalStorage.loadPlaylist():InternalStorage.loadPlaylistWeb();if(!App.player||!App.player.alive)return;b&&(App.player.playlist=b)}catch(c){}App.player&&App.player.alive&&(AppUI.init(a),
(b=document.getElementById("cover"))&&b.classList.remove("in"),setTimeout(function(){window.removeEventListener("resize",App.adjustCover);const c=document.getElementById("cover");c&&document.body.removeChild(c)},320),App.loading=!1)}}static triggerMainWindowStateChanged(){App.ipcRenderer&&App.ipcRenderer.invoke("triggerMainWindowStateChanged")}static mainWindowMinimize(){App.ipcRenderer&&App.ipcRenderer.invoke("mainWindowMinimize")}static mainWindowRestoreOrMaximize(){App.ipcRenderer&&App.ipcRenderer.invoke(App.isMaximized?
"mainWindowRestore":"mainWindowMaximize")}static mainWindowClose(){App.ipcRenderer&&App.ipcRenderer.invoke("mainWindowClose")}static async showOpenFileDialog(){if(App.fileInput||App._showingDialog)return null;try{return App._showingDialog=!0,await App.ipcRenderer.invoke("showOpenFileDialog")}finally{App._showingDialog=!1}}static convertFilePathsToAppFiles(a){return App.ipcRenderer.invoke("convertFilePathsToAppFiles",a)}static async showOpenDirectoryDialog(){if(App._showingDialog)return null;try{return App._showingDialog=
!0,await App.ipcRenderer.invoke("showOpenDirectoryDialog")}finally{App._showingDialog=!1}}static listDirectory(a){return App.ipcRenderer.invoke("listDirectory",a)}static extractMetadata(a,b){return App.ipcRenderer.invoke("extractMetadata",a,b)}static showOpenDialogWeb(a){if(!App.fileInput)return Promise.resolve(null);App.fileInputPromiseResolve&&(App.fileInputPromiseResolve(null),App.fileInputPromiseResolve=null);App.fileInput.setAttribute("data-version",(++App.fileInputVersion).toString());a?(App.fileInput.setAttribute("webkitdirectory",
"webkitdirectory"),App.fileInput.setAttribute("directory","directory"),App.fileInput.removeAttribute("accept")):(App.fileInput.removeAttribute("webkitdirectory"),App.fileInput.removeAttribute("directory"),App.fileInput.setAttribute("accept",Playlist.concatenatedSupportedExtensions));return new Promise(b=>{App.fileInputPromiseResolve=b;App.fileInput.click()})}}App.hostTypeElectron="Electron";App.ipcRenderer=window.electronIpcRenderer||null;App.closeHandlers=[];App.frameless=!!window.electronIpcRenderer;
App.hostInterface=window.hostInterface||null;App.insideNativeHost=App.frameless||!!App.hostInterface;App.hostType=App.hostInterface&&App.hostInterface.getHostType()||null;App.iconLoading=document.getElementById("icon-loading");App.titleBar=null;App.iconRestore=null;App.iconMaximize=null;App.installPrompt=null;App.fileInputVersion=0;App._loading=!0;App._showingDialog=!1;App.isMinimized=!1;App.isMaximized=!1;function main(){App.init()}App.preInit();let cLib;
